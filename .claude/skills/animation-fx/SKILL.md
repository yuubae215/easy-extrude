...
## Step 1.5: モーション・アイデンティティ (ポートフォリオ単位の多様性)

品質ゲート4の「同じテイスト禁止」を作品単位からポートフォリオ単位へ持ち上げる。
美的方向は**方法(このスキルのカタログ)ではなくインスタンス**であり、
プロジェクト内では固定・プロジェクト間では意図的に変える。

1. **方向の決定 (プロジェクト初回のみ):** 5軸を明示的に選ぶ —
   イージング族 (spring/expo/back/…) / 時間スケール (きびきび↔ゆったり) /
   振付パターン (スタッガー形・位相) / アトモスフィア / 3色システム。
   選定前に auto memory の**直近方向の記録**を見て、前作と少なくとも2軸を意図的に逸らす
   (ブランド指定・既存トーン継承の指示がある場合はそちらが優先)。
2. **固定 (プロジェクト内):** 決めた5軸を project CLAUDE.md の
   「モーション・アイデンティティ」節へ記録(設計された契約)。以後この
   プロジェクトの全エフェクトはこの軸に従い、一貫性を作る。
3. **記帳 (プロジェクト間):** 決定した方向を auto memory に一行残す
   (`motion-directions: <日付> <project> = <5軸の要約>`)。これが次回の逸らし判定の台帳。

多様性は軸の**組合せ**で出す — 個々の実装品質は品質ゲートが守るので、
「変える」は奇抜化ではなく、カタログ内の未使用領域への計画的な移動。
...
- [ ] モーション・アイデンティティ5軸が project CLAUDE.md に固定され、直近方向の台帳と2軸以上離れている (Step 1.5)
