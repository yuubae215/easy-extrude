...
## 状態台帳・状態機械(§1.4)
<!-- 台帳: 状態(status/flag/mode/存在)を持つ実体は閾値未満でも全て記録する。
     §1.4 の発動判定はこの台帳の累積で行う(プロンプト単位ではなく)。
     状態が増えたら同じ変更の一部としてここを更新する。
     台帳が大きくなったら別ファイルへ出してよい — ただしここから **名指しする**
     (名指しがあれば源は一つ。名指しの無い別ファイルは第二の源 — §4)。 -->
### 台帳(全実体)
<!-- 基数は必ず埋める: mode/status は状態として認識されるが「0 個」「N 個」は
     状態に見えず、そこが空欄のまま「実体が存在しない場合」が未設計で残る。
     0 が正当なら「0 のとき何が起きるか」、N が正当なら「どれのことか」を書く。 -->
- [TODO 実体名]: 状態 = { [TODO S1, S2] } / 基数 = [TODO 1 | 0..1 | 0..N] / 権威 = [TODO どこが source か(§1.1)]
- [TODO 実体名]: 状態 = { [TODO] } / 基数 = [TODO] / 権威 = [TODO]

### 状態機械(台帳が閾値を跨いだ実体のみ)
<!-- 3状態以上 or 不正遷移が事故になる実体。遷移の正準は docs/state-<entity>.mermaid(図)、
     ここには意味(guard・禁止遷移の理由・権威)だけ。トポロジを再掲しない(§1.1)。 -->
- [TODO 実体名] → 図: `docs/state-[entity].mermaid`
  - guard / 禁止遷移の理由: [TODO]
  - 状態の権威: [TODO]
...
## モーション・アイデンティティ(animation-fx Step 1.5 / UI・演出があるプロジェクトのみ)
<!-- プロジェクト内で固定する美的方向の5軸。無関係なプロジェクトではこの節ごと削除。
     選定時は直近方向の台帳(auto memory)と2軸以上離すこと。 -->
- イージング族: [TODO] / 時間スケール: [TODO] / 振付: [TODO]
- アトモスフィア: [TODO] / 3色システム: [TODO 背景/実体/発光]
