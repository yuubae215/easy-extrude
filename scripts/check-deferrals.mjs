 ...
 */
// 2026-08-14: 1 → 4。ADR-129 の実装で DEF-031 / DEF-032 が決着し、登録簿から行を
// 消した。ところが**決着した残しを記述する散文**が 4 箇所に残る (ADR-127 / ADR-128 の
// 本文、ia-redesign の順序表、`GraspSampleView.js` の doc コメント) — どれも「まだ
// 残っている」ではなく「かつて残っていた」を書いた歴史である。この検査は言及と宣言を
// 区別できないので、散文を書き換えて数を下げるのは**検査に合わせて履歴を消す**行為に
// なる (この定数の doc が自分でそう書いている)。よって baseline を上げる。
// 粒度の問題そのものは、語彙か覆う範囲を上げる次の ADR の担当である。
const UNDECLARED_BASELINE = 4
 ...
 *
 * **2026-08-14: 13 → 14。** ADR-129 D6 が足した DEF-034 (名前をまだ持たない主張の
 * 載せ場所) の満期が「**それを決める ADR が起票されたとき**」で、DEF-020 と同じ形
 ...
 *
 * **2026-09-26: 15 → 11 (DEF 棚卸し)。** 散文の満期は来ても機械が知らないので、
 * 1 行ずつ実物と突き合わせた。**3 行は満期が既に来ていたのに残っていた** —
 * DEF-007 (UI の `DiagnosticsFunnel` が消費済み) / DEF-010 (`.gsn` 自身が「接続済み」と
 * 書いていた) / DEF-025 (`server/test/grasp.contract.test.js` が往復を検査済み)。
 * 満期切れ 0 件の緑は、散文の行については何も言っていなかったことになる。
 * 2 行 (DEF-004 → `GREP:` / DEF-006 → `PATH:`) は trigger を書けた。
 * DEF-025 を消したら同じ ADR-076 §Still deferred の残り 3 項目がファイル単位の被覆を失い、
 * DEF-050 として登録した (外部条件なので散文) — 15 − 3 − 2 + 1 = 11。
 * DEF-010 を消したら ADR-110/111/112 の木が「DEF-010 と同じレーン」に相乗りしていた
 * 接続の保留が受け皿を失ったので、DEF-051〜053 として `GREP:` trigger つきで登録した
 * (散文ではないのでこの数には入らない)。
 */
const PROSE_EXPIRY_BASELINE = 11
  ...
  errors.push(
    ...
    (proseExpiry.length > PROSE_EXPIRY_BASELINE
      ? '    満期欄に trigger を書けるなら書くこと — 4 形ある (ADR-123 D5):\n' +
        ...
        '    **上げる前に、上に並んだ既存の散文行の満期も 1 行ずつ確かめること** —\n' +
        '    散文の満期は来ても機械が知らないので、人が見る機会はこの瞬間しか無い\n' +
        '    (2026-09-26 の棚卸しで 15 行中 3 行が既に満期を過ぎていた)。\n'
