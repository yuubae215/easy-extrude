...
| 遊び心 / ワクワク感 / playfulness / 遊戯化 / モーション階層 / Motion Tier / Tier A / affordance motion / アフォーダンス・モーション / MotionGovernor / MotionMath / 演出予算 / transient 演出 / celebration / セレブレーション / デザイントークン / design tokens / tokens.js / パレット ドリフトテスト / `src/theme/` / motion.js 単一境界 / disabled-as-quest / オンボーディングツアー / コアモデリング演出 / 着地演出 / landing pulse / LandingEffects / CommandFeedbackMath / landingDescriptor / setLandingListener / undo 巻き戻しパルス / クローム刷新 / ロック状態 / locked / ゲート述語 reason / ChromeGates / ChromeMath / ChromePrimitives / useHoverPress / 押下バネ / press spring / hover 呼吸 / breathe / activeGlow / 呼吸グロー / enterMotion / トースト entry / disabled タップ理由 toast | ADR-065 (Accepted, Phase 0+1+2+3+4 実装済; Phase 5–6 後続 — 実装前に必読), PHILOSOPHY #30 (Motion Tier), `src/theme/tokens.js`+`tokens.test.js`, `src/theme/motion.js`+`motion.test.js` (grep 固定の単一境界), `src/view/MotionMath.js`, `src/view/MotionGovernor.js`, `src/view/CommandFeedbackMath.js`, `src/view/LandingEffects.js`, `src/service/CommandStack.js` (`setLandingListener`/`depth`), `src/view/ChromeGates.js`+`ChromeMath.js` (Phase 3 純粋層), `src/components/Chrome/ChromePrimitives.jsx`, `src/view/CelebrationMath.js`+`CelebrationMath.test.js`+`CelebrationField.js`, `src/components/Feedback/Celebration.jsx` (Phase 4 — 遷移述語/バースト/節目), ADR-062 (親方針・無改変継承) |
...
- **2026-07-11**: UX/Bugfix — **ADR-065 Phase 4 実装（セレブレーション）→ Accepted (Phase 5–6 後続) + Sketch 追加経路の回帰修正**。① Sketch: 真因は `_addObject('sketch')` の dangling call `_addProfileObject()`（クリックハンドラ内 TypeError = 無言死 #11; Object モードのままなのでタッチが orbit に見えた誤診）。新 `AddProfileCommand` + `_addProfileObject()`（自動 Edit Mode 2D 入場 #12・undo は setMode('object') 経由）+ ステール `_sketch` クリア + smoke E2E に sketch フロー追加（checkJs は types/domain スコープ = controller 配線の生死ガードは smoke のみ — Yellow Card 追加）。② Phase 4: 純粋 `CelebrationMath.js`（clearedTransition/allGreenTransition/commandMilestone/pickCelebration = 優先順位付き最大 1 記述子; **初期ロードは遷移ではない**を全述語テスト固定）+ `Celebration.jsx`（ContextLayer ルートにただ 1 つ = 予算 1 構造化; 勝った事実自身の tick で key = ステール再演防止）+ `CelebrationField.js`（InstancedMesh 1 draw call・Governor 経由・節目は label フィルタ前の `commandMilestone`）。`DURATION.celebration` のみ追加。Evidence: unit 537→558 / typecheck / contract 23 / E2E 6 pass / 実機で質問全消化バースト・reduced 静的バナー・節目 3D バースト確認 / build **+3.37KB gz**（累計 ≈+7.2KB、予算 ≤+20KB、依存 0）。契約・schema・DSL 版・BFF 無改変。
- **2026-07-10** (2): UX/Infra — **ADR-065 Phase 3 実装（クローム刷新 + disabled-as-quest）→ Accepted (Phase 4–6 後続)**。① 新純粋 `src/view/ChromeGates.js`: 各 disable 可能コントロールのゲートが `{enabled, reason}` を 1 つの戻り値で返し（locked ⇒ 非空 reason を機械テスト固定）、「enable 判定と理由の同一関数参照」（ADR-058/#25）を構造化 = 無言 disabled 表現不能。`UIStateManager` の inline boolean を全ゲート経由に置換、Header undo/redo も `gateUndo`/`gateRedo`。② 新純粋 `src/view/ChromeMath.js`: `tierAMotion`（押下バネ — CSS `EASING.spring`）/ `activeGlow`（呼吸グロー — keyframes を新設 `MotionMath.breathe` から生成し曲線と CSS のドリフト排除; reduced は静的グロー保持）/ `lockedStyle`（破線 + `cursor:help`）/ `enterMotion`（トースト・メニュー・InfoBar ヒントの entry; reduced は即時出現）。motion キーのみ = Tier A が Tier F 判定を偽装できない形（テストで key 集合拘束）。③ `ChromePrimitives.jsx`（`ChromeDefs`+`useHoverPress`）新設、ToolbarButton はロック様式 + タップ→理由 toast（native `disabled` 属性は不使用 = `aria-disabled`）、Header/InfoBar/Toast を in-place 刷新。トークン追加 `accentActive`/press系 DURATION/`EASING.spring`（双方向ドリフトテスト通過）。**副産物バグ修正**: boot 時 `setMode` が boot solid コマンド積載中に `canUndo` を store へスナップショット → 直後の `clear()` が再同期せずモバイル header Undo が起動直後から enabled（タップ無言 no-op #11）— `clear()` 直後の `_refreshUndoRedoState()` で修正 + 契約行追記。**escape hatch トリガ不成立**（JS 駆動中断可能バネ 0 面）= `@react-spring/web` 不採用のまま。CODE_CONTRACTS 行 + SCREEN_DESIGN 一般規則 + LAYOUT_DESIGN Animations 表更新。Evidence: unit 522→537 全 pass / typecheck / contract 23 / E2E 4 pass（reduced 含む）/ build **+2.27KB gz**（累計 +3.81KB、予算 ≤+20KB、依存 0）。契約・schema・DSL 版・BFF 無改変。
- **2026-07-10**: UX/Infra — **ADR-065 Phase 0+1+2 実装（トークン / MotionGovernor / コアモデリング着地演出）→ Accepted (Phase 3–6 後続)**。⓪ `src/theme/tokens.js`（COLOR/DURATION/EASING/Z + hexNumber/rgba、全 freeze・pure）新設、LAYOUT_DESIGN パレット表を 1行=1トークン=1hex に再構成し `tokens.test.js` が**双方向**ドリフト束縛（ADR-064 §1.1 と同手法）。① 単一 matchMedia を `src/theme/motion.js` へ移設（FeedbackPrimitives は再 export = 境界は移動しても分岐せず、`motion.test.js` の grep テストで固定）、純粋 `MotionMath.js`（easing/臨界減衰 spring/stagger）、`MotionGovernor.js` が transient 3D 演出の単独所有者（`spawn(reduced=>fx)` 注入・予算 8 の最古 evict + dispose #9・reduced は静的キュー保持 #11）— `_activeRipples` 廃止、RippleEffect+push 3 サイト移行。② `CommandStack.setLandingListener`（push/undo/redo 後の `{phase,label}` = 唯一の着地権威 #1、boot solid は clear() 後接続で除外、microtask 遅延で操作後アンカー）→ 純粋 `CommandFeedbackMath.js`（landingDescriptor: Add/Extrude/Face Extrude→緑ポップ、Move/Rotate→青 settle、undo→琥珀**収縮**巻き戻し、redo→青 replay; 未知/malformed/Add-undo(実体消滅)→null #11）+ `LandingEffects.js` LandingPulse（独自オーバーレイ球 — emissive 不可侵 #4・向き非依存・バウンズ比例 #27）。PHILOSOPHY **#30 Motion Tier** 鋳造 + CODE_CONTRACTS 3 行 + ADR-064 行の境界所在更新。Phase 2 残: スナップフラッシュ・選択パルス。Evidence: unit 495→522 全 pass / typecheck / contract 23 / E2E 4 pass / build **+1.54KB gz**（予算 ≤+20KB、依存 0）。契約・schema・DSL 版・BFF 無改変。
