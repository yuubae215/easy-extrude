...
| CI / テストゲート / PR ワークフロー / rigor 遡及 / prefers-reduced-motion / reduced motion / motion 削減 / flashStyle / useReducedMotion / E2E スモーク / Playwright / smoke / 配線の生死 / pnpm test glob 化 | ADR-064 (Accepted, Phase 1+2+3+4 実装済), ADR-062 (play 側の対), PHILOSOPHY #29/#20, `.github/workflows/ci.yml`, `.github/workflows/deploy.yml`, `src/view/FeedbackMath.js` (`flashStyle`), `src/components/Feedback/FeedbackPrimitives.jsx` (`prefersReducedMotion`/`useReducedMotion`), `playwright.config.js`, `e2e/smoke.spec.js` |
...
| 遊び心 / ワクワク感 / playfulness / 遊戯化 / モーション階層 / Motion Tier / Tier A / affordance motion / アフォーダンス・モーション / MotionGovernor / celebration / セレブレーション / デザイントークン / design tokens / `src/theme/` / disabled-as-quest / オンボーディングツアー / コアモデリング演出 / LandingEffects | ADR-065 (Proposed, Phase 0–6 未実装 — 実装前に必読), ADR-062 (親方針・無改変継承), ADR-064 Phase 4 (reduced-motion 境界の移設元) |
...
- **2026-07-08** (4): Infra — **ADR-064 Phase 3 実装（未契約ワイヤの二択確定）→ Accepted**。`/api/scenes` を rigor 対象化: `schema/scene-1.3.schema.json` を **Blender datablock レンズ**で設計 — グラフ骨格（`objects[]`・`links[]`・`transformGraph`）は `additionalProperties:false` で守り、バルクジオメトリ（`ImportedMesh` の base64 バッファ・`operationGraph`）は opaque blob 葉として封筒だけ検証。BFF `sceneContract.js` は schema を*読んで*検証子 + 版を導出（restate せず）、`scenes.js` POST/PUT が版を刻み検証（非適合→400、既存クライアント非破壊）。観測で判明した「wire に version 無し」の穴を BFF stamp で閉じる。`/api/ws`・`/api/import` は理由 + Phase C 期限つきで**対象外を明示宣言**（無言の無契約と区別）。テスト: `src/schema/SceneSchema.test.js` 12 + BFF `scenes.contract.test.js` 7、BFF `test:contract` を glob 化（Phase 1 と同じ silent-failure 除去）。glob 480→492、contract 16→23、typecheck・build green。ドメイン層は `KNOWN_TYPES` export のみ（挙動不変）。
