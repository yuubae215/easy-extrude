...
- **2026-04-07**: Feature — Coordinate Space Type Safety Phase 2: `WorldVector3`/`LocalVector3` branded JSDoc types in `src/types/spatial.js`; `corners` annotated on all domain entities; `_worldPoseCache` + computation locals annotated in SceneService; `tsconfig.json` + `pnpm typecheck` + CI gate. Bugfix — `UrbanPolyline/Polygon/Marker.fromPoints()` called `new Vertex(id)` without position, causing runtime TypeError; fixed to `new Vertex(id, p.clone())`.
