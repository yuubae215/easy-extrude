...
- **2026-07-11** (4): UX — **ADR-065 Phase 6 実装（オンボーディングツアー・デスクトップ）→ 全 7 フェーズ完了**。純粋 `TourMath.js`（add→select→grab→edit→extrude のクエストトレイル; committed 事実のみ・前進のみ・malformed は同一参照・誤ヒントより無ヒント #11）+ uiStore 判別共用体 `tour`（sole writer AppController; アプリ状態であって演出履歴ではない = ADR-062 §2 への意図的 carve-out を契約行化）+ `TourCard.jsx`（左下コーナー #26 回避・overlay 中は FSM 無変異で非表示）+ Outliner「+ Add」の Tier A 呼吸グロー（`tourAnchor`/`tourVisible` 単一源 §1.1）。done/dismiss のみ `localStorage.ee_tour` 永続（named rule 6 = 設定 vs 履歴; Widening 3 初適用）。モバイルは従来オーバーレイ維持。Evidence: unit 582→598 / typecheck / contract 23 / E2E 6 pass（+ツアーテスト; boot アサーション `exact:true` 化）/ 実機目視（boot グロー→4/5→TOUR COMPLETE→`ee_tour=done`）/ build **+1.83KB gz**（累計 ≈+10.6KB、予算 ≤+20KB、依存 0）。契約・schema・DSL 版・BFF 無改変。残候補はスナップ係合フラッシュのみ。
