...
- **2026-03-23**: STEP import end-to-end fix + unit conversion dialog — Fixed occt-import-js API mismatch (geometry is at `mesh.attributes`, not `mesh.faces`); added `SceneView.fitCameraToSphere()` + `geometryApplied` event to auto-frame imports; fixed `_finalizeRectSelection` and `_objDragging` crashes for `ImportedMesh` (missing `corners` guard); added unit-conversion modal (mm↔m, cm, inch) before import. MENTAL_MODEL updated.
- **2026-03-23**: Server startup fix + occt-import-js installation — Fixed `LibsqlBatchError: cannot change into wal mode from within a transaction` in `server/src/db/database.js`; moved `PRAGMA journal_mode = WAL` into a standalone `db.execute()` call. Added `occt-import-js@0.0.23` to `server/package.json`; `POST /api/import/step` now performs real STEP parsing.
- **2026-03-23**: Measure guide + snap display fix (2nd placement) — `MeasureLineView` was missing `showSnapCandidates`/`showSnapLocked`/`clearSnapLocked` no-ops; `TypeError` in `_onPointerMove` aborted before `_updateMeasurePreview()`, hiding the guide on all placements after the first. Snap candidates also invisible when MeasureLine is active (no `THREE.Points`); fixed with `_measure.snapMeshView` fallback. MENTAL_MODEL updated with two new rules.
