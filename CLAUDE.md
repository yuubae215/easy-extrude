...
- **2026-07-13**: UX — **ビューポート常設ステージ + 起動リビール（ADR-067 Accepted — ADR-066 Tier D の最初の適用）**。ギャップ分析で「全フレーム見続ける舞台そのもの（背景・奥行き・光・起動の瞬間）が素の Three.js のまま」= Tier D 開放後の最大の未開拓分野と特定。新純粋層 `StageMath.js`（14 テスト: 決定論ダスト/非整数比 2 周波ドリフト/層スタガー entry/フォグ密度 #27/フライト **p≥1 恒等**拘束）+ `SceneStage`（縦グラデ背景・FogExp2・フロアグロー・ダスト 2 層・リムライト; **SceneView 所有の永続ビュー**・`scene.background`/`fog` 単独書き手 #4・`_updateGridScale` 追従 #27・reduced ドリフト凍結 = 静的残存 #30/#11）+ `BootReveal`（`MotionGovernor.spawn` 経由の一回性カメラ進入 1.8s → 既定構図に恒等着地; pointerdown/wheel/context で即着地・external-write ガードで fitCameraToSphere に譲る・reduced=最終ポーズのみ）。`DURATION.bootReveal` 追加。CODE_CONTRACTS §1 新行・ADR-066 相互リンク。Evidence: unit 635 pass / typecheck clean / contract 23 / E2E 7 pass / build **+1.95KB gz**（累計 ≈+14KB・依存 0）。契約・schema・DSL 版・BFF 無改変。（全文 → SESSION_LOG）
