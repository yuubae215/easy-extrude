...
- **2026-07-08** (3): Infra — **ADR-064 Phase 2 実装（DSL スキーマ昇格）→ Accepted**。`layout/1.0` と `context/0.4` を in-repo JSON Schema 成果物へ昇格（`schema/{layout-1.0,context-0.4}.schema.json`、両者 `additionalProperties:false` + 版フィールド、grasp-contract と同形）。conformance テスト `src/schema/{Layout,Context}Schema.test.js` 37 本: 全 6 例 PASS・余剰/不正 enum/不正版 FAIL・hydrated `{$fact}` leaf/requirements-only doc PASS、**ドリフト束縛**=各 enum が `*DslSchema.js` 定数と機械一致（§1.1）。**形と意味の分離**: 意味検査（R1–R9・ref 解決）は `LayoutValidator`/`ContextValidator` に無改変で残す。CLI `schema/tools/validate.mjs`。glob 443→480、typecheck・build・`test:contract` 16/16 green。ajv を root devDep 追加。契約/BFF/ドメイン/DSL 版・JS バリデータ無改変。
