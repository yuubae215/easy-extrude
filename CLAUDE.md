...
- **2026-03-23**: Mobile Stack snap fix + footer keybinding update — Fixed `_applyStackSnap()` missing from `_objDragging` (touch-drag) path; added call to `_onPointerMove` touch branch. Updated `UIView._setInfoText()` footer: Object mode adds G>S Stack, Shift+D Duplicate, M Measure; Edit 3D shows 1/2/3 mode keys, E Extrude, Shift+Click Multi-select. MENTAL_MODEL confirmed.
