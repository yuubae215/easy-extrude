...
| 遊び心 / ワクワク感 / playfulness / 遊戯化 / モーション階層 / Motion Tier / Tier A / affordance motion / アフォーダンス・モーション / MotionGovernor / MotionMath / 演出予算 / transient 演出 / celebration / セレブレーション / デザイントークン / design tokens / tokens.js / パレット ドリフトテスト / `src/theme/` / motion.js 単一境界 / disabled-as-quest / オンボーディングツアー / コアモデリング演出 / 着地演出 / 音量設計 / volume corollary / voxel / VoxelBurst / dissolve / materialize / SAO 飛散 / グリッチ明滅 / lifecycleDescriptor / voxelFrame / _lifecycleAnchors / objectRemoved 第2引数 / pose 操作は無音 / LandingEffects / CommandFeedbackMath / setLandingListener / クローム刷新 / ロック状態 / locked / ゲート述語 reason / ChromeGates / ChromeMath / ChromePrimitives / useHoverPress / 押下バネ / press spring / hover 呼吸 / breathe / activeGlow / 呼吸グロー / enterMotion / トースト entry / disabled タップ理由 toast / 三拍リビール / three-beat reveal / revealFrame / REVEAL_TIMELINE / mixHex / RegionGhostMath / gapBandRects / regionResolveTransitions / resolveFrame / RegionResolveEffect / _refreshRegionGhosts / _buildRegionGhosts / 領域 recolor→dissolve / settled gap band / 不確実バンド extremes 収束 / 永続ゴースト reduced 対応 | ADR-065 (Accepted, Phase 0+1+2+3+4+5 実装済 + Phase 2 音量改訂 2026-07-11; Phase 6 後続 — 実装前に必読), PHILOSOPHY #30 (Motion Tier + volume corollary), `src/theme/tokens.js`+`tokens.test.js`, `src/theme/motion.js`+`motion.test.js` (grep 固定の単一境界), `src/view/MotionMath.js`, `src/view/MotionGovernor.js`, `src/view/CommandFeedbackMath.js`, `src/view/LandingEffects.js`, `src/service/CommandStack.js` (`setLandingListener`/`depth`), `src/view/ChromeGates.js`+`ChromeMath.js` (Phase 3 純粋層), `src/components/Chrome/ChromePrimitives.jsx`, `src/view/CelebrationMath.js`+`CelebrationMath.test.js`+`CelebrationField.js`, `src/components/Feedback/Celebration.jsx` (Phase 4 — 遷移述語/バースト/節目), `src/view/GraspGhostMath.js` (`revealFrame`/`mixHex`)+`GraspGhostView.js`, `src/view/RegionGhostMath.js`+`RegionGhostMath.test.js`+`RegionResolveEffect.js`, `src/view/UncertaintyGhostView.js` (Phase 5 — 振付 + reduced), ADR-062 (親方針・無改変継承) |
...
- **2026-07-11** (3): UX — **ADR-065 Phase 5 実装（Context/Grasp 深化 — 振付のみ）**。① grasp 三拍リビール: 純粋 `revealFrame`（`REVEAL_TIMELINE` 400/240/260ms — **合計 ≤1s の情報遅延上限をテスト拘束**）で 接近→閉指→スコア flood（`mixHex` でニュートラル灰→スコア色 + キャプションは beat 3; verdict 線種とパネルのスコアバーは即時）。hover は `Infinity` = 最終段（従来挙動）。② conflict セル解消の 3D 対応物: 新純粋 `RegionGhostMath.js`（`gapBandRects` = view と演出の単一幾何源 / `regionResolveTransitions` = **2 つの committed projection の diff**〔prev=null・変数消失は遷移でない #11〕/ `resolveFrame`）+ 新 transient `RegionResolveEffect`（Governor 経由・`DURATION.regionResolve` 700ms）。`_reproject` ghost 分岐 → `_refreshRegionGhosts()` で **ghost モード中の undo/redo が 3D をステールにする潜在バグも解消**（`_ghostData` は controller-local 演出履歴）。honesty 修正: resolved でも交差が空の gap バンドは settled 緑 + 「✓ settled by decision」（従来は赤 conflict のステール主張）。③ 不確実バンド: collapse 中 extremes が nominal へ**収束** + 永続ゴースト 3 view の reduced-motion 対応（単一境界を per-animation サンプル; 最終段ジャンプ/静的シェル、onSnapped 即発火）。Evidence: unit 561→582 / typecheck / contract 23 / E2E 5 pass / 実機目視（承認→settled 緑バンド・undo 赤復帰・redo recolor→dissolve・Tutorial collapse 収束・reduced 静的）/ build **+1.16KB gz**（累計 ≈+8.8KB、予算 ≤+20KB、依存 0）。契約・schema・DSL 版・BFF 無改変。Phase 6（ツアー）のみ後続。
- **2026-07-11** (2): UX — **ADR-065 Phase 2 音量改訂（毎操作パルス廃止 → 実体ライフサイクル voxel 演出）**。ユーザフィードバック「毎操作のポリゴン球花火はうるさい。削除は SAO 風 voxel 飛散、出現はその逆」→ **演出の音量設計**として規律化: pose 操作（Move/Rotate/Face Extrude + undo/redo）は**無音**（結果が既に可視 = 毎発火で #30 一文テスト不合格; テストで null を機械固定）、実体の appear/vanish のみ `VoxelBurst`（InstancedMesh 24 cube・dissolve = シアン飛散 / materialize = 緑逆再生 + 決定論的グリッチ明滅〔scale に乗せる — material 単一〕）、セレブレーションは無改変。`landingDescriptor`/`pulseFrame`/`LandingPulse` 廃止 → `lifecycleDescriptor`（undo は遷移反転）/`voxelFrame`/`voxelJitter`/`glitchGate`。アンカーは `objectRemoved` の**加算的第2引数 entity**（4 emit サイト; EVENTS.md 更新）+ controller-local `_lifecycleAnchors`（last-wins・毎着地 consume&reset = ロードのイベント洪水が漏れない）。トークン `voxelMaterialize/voxelDissolve`（COLOR 無変更）。PHILOSOPHY **#30 volume corollary** 鋳造 + CODE_CONTRACTS 行改訂。Phase 2 残: 選択パルスは不採用確定、スナップフラッシュは候補継続。Evidence: unit 558→561 / typecheck / contract 23 / E2E 5 pass / 実機目視（Add 緑収束・Move 無音・削除シアン飛散・undo 復元・reduced 静的シェル）/ build **+0.27KB gz**（累計 ≈+7.5KB、予算 ≤+20KB、依存 0）。契約・schema・DSL 版・BFF 無改変。
