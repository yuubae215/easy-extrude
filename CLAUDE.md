...
- **2026-07-15**: UX — **UX パリティ・パス Phase 2+3（ADR-070/071 Accepted・実装済）**。ユーザ指示「最新の ADR の意思決定に従って実装」= 両 Proposed ADR の推奨案承認とみなし実装。**ADR-070 エンティティ同一性**: 共有 `EntityLabel.js` 抽出（CF ラベル機構の一般化・各 1 owner・`Z.sceneLabel`）→ Solid/Profile/ImportedMesh に選択/ホバー段階開示の名前+IFC ラベル、CF ラベル情報化（`⌖`+名前+選択中 RPY）、`setIfcTint` = 基調色 owner（emissive は `_syncEmissive` 専有のまま #4）で inert だった IFC 24 クラスが見える declaration に; `SceneService._newMeshView`/`_syncIdentityVisuals` が単一構築/同期点。**ADR-071 配置既定**: `stackMode` 既定 ON（confirm/cancel で ON 復帰・start ではリセットしない=モバイル Free ボタン保護）、地面 Z=0 を暗黙の着地面（`max(hit ?? -∞, 0)`）、`axis!=='z'` で垂直 intent 保護、新述語 `checkGroundClearance`（#25）+ `warnIfBelowGrade` 1 ジェスチャ 1 warn（#11・クランプなし=基礎/杭可能）、S/モバイルボタンは無効化トグルへ意味反転（`Free (S: stack)` 可視）。docs: 両 ADR Accepted + README 索引 + CODE_CONTRACTS §1 新 2 行 + EVENTS/SCREEN_DESIGN/STATE_TRANSITIONS。Evidence: unit **640 pass** / typecheck clean / build clean / E2E **10 pass**（新規 2: stack 反転・ラベル liveness）。契約・schema・DSL 版・BFF 無改変。（全文 → SESSION_LOG）
- **2026-07-14**: UX — **UX パリティ・パス Phase 1（ADR-069 Accepted; Phase 2–4 計画済・未着手）**。ユーザの UI/UX 評価（10 観察）を「いきなり実装せず一段上げてゴール観測」の指示通り横断調査 → 10 個の無関係バグでなく**「3D 層が周辺面より一段先行、取り残された面が一段低く読める」単一根本**と特定。Phase 1 実装: **1a ギズモ→CameraFlight**: `GizmoView._onClick` の 1 フレーム直書きテレポート（唯一 motion 系を迂回する面 = 「急に視点が飛ぶ」原因）を廃し、ポーズ導出のみ→`onRequestView`→`AppController.flyToView`（`MotionGovernor.spawn`+`CameraFlight`, ADR-068 同契約）。軸ビューは軌道方向を変えるので `focusPose` 不使用＝自前導出・グリッド不変（#27）; up+`_quat` 同期は離散（reduced でも適用）; 配線断は `_applyInstant` へ graceful（#11）。**1b Link Network**: 全エッジ同時マーチングアント（「線があちこち飛んで見づらい」の literal 原因）を全廃（#30: 情報なき動き=ノイズ）→ focus+context（ホバー/選択で隣接エッジ前面化・残り後退; `_hoveredId` は presentation 専用・3D 選択不変）・運動学/位相で実線太/点線細・交差レイヤは右湾曲で A→B/B→A 分離・密モードでも焦点近傍ラベル維持（パネル拡大せず #26）。**1c 位相リンク切断**（#4）: `jointType===null`（adjacent 等）は Unfasten 経路なしだが `detachSpatialLink` は普遍 → 長押しメニューに「Disconnect」項目（N パネルと同じ undoable 経路）＝発見性ギャップ解消。**2a Frame 命名統一**（#9）: モバイル `promptAddFrame` の literal `'Frame'` override（連番回避で重複量産）を新 public `SceneService.nextEntityName` 経由へ＝全 add 経路が単一命名源（核 §1.1）。docs: ADR-069 + README 索引 + CODE_CONTRACTS §1 新 4 行。Evidence: unit **640 pass** / typecheck clean / build clean / E2E **9 pass**（新規: ギズモ +Z ドット→flyToView 配線 liveness — checkJs 対象外の controller 層の唯一のガード）。契約・schema・DSL 版・BFF 無改変（純クライアント体感層）。（全文 → SESSION_LOG）
- **2026-07-13** (2): UX — **カメラ・フォーカスフライト + 生きた選択 + 研磨掃引（ADR-068 Accepted）**。ユーザ指示「横断的に UX アイデア募集・全振りしてよい」→ ギャップ分析で「深く磨いた面の隙間の具体的 3 穴」を特定、ユーザが 3 方向 + フレームワーク内維持を選択（Sound 見送り）。**§1 カメラ（Tier D）**: 純粋 `CameraMath.focusPose`（端点恒等 machine-pin）が単一導出源 → `SceneView.focusPose()` を scene framing(`fitCameraToSphere`=グリッド込み #27)/selection framing(`CameraFlight`=グリッド不変)両方が消費; `CameraFlight`=BootReveal 同契約 transient（位置+target 補間・reduced 即着地・`_cameraStolen` ガード; OrbitControls damping 無効ゆえ直接書込み安全）; `focusSelection`/F・Home/`_onDblClick`。**§2 生きた選択**: `SelectPulse`（OBB アウトラインをポップ→フェード・overlay 専用 #4・選択遷移時のみ #30）+ `MeshView._syncEmissive` 第 3 フラグ `_hovered`（`setHovered()` 唯一書き手・fine ポインタ限定 #13）。**§3 研磨**: `exitMotion`+`eaChromeExit` トースト退場 / Grasp バー `transition:width`（契約 verbatim #29）/ Outliner scrollIntoView。docs: ADR-068 + CODE_CONTRACTS §1 新 2 行（Visual State Ownership 3 フラグ + Framing Has ONE Derivation）・EVENTS。Evidence: unit 635→**640 pass**（CameraMath 5）/ typecheck clean / E2E **8 pass**（focus 配線 liveness）/ build clean・依存 0 / 実機目視（reduced 全静的 degrade・pageerror 0）。契約・schema・DSL 版・BFF 無改変。
