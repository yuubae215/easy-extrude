...
- **2026-07-09**: Docs/UX — **ADR-065 起案（体感層の全面遊戯化プログラム — front は遊び場、back は証明のまま）→ Proposed**。ユーザ要望「フロントUIの遊び心とワクワク感の抜本強化（フロント=ゲーム、バック=堅い証明）」。探索で 5 空白を特定（コアモデリング操作の演出ゼロ / トークン基盤不在・hex 複製 / 無味乾燥クローム / 3D tick の reduced-motion 非対応 / 達成演出 1 箇所）。ADR-062 を廃止せず 3 Widening（Motion Tier 規則 = Tier F/Tier A/装飾禁止 + 一文テスト、セレブレーション = 事実**遷移**の一過性高演出・永続/ワイヤ禁止、設定 vs 履歴の永続線引き）+ 7 フェーズ（⓪トークン → ①MotionGovernor/reduced-motion 3D 延長 → ②コアモデリング演出【旗艦】→ ③クローム + disabled-as-quest → ④セレブレーション → ⑤Context/Grasp 振付 → ⑥事実駆動ツアー）。3D は依存ゼロ恒久、DOM は手書き + `@react-spring/web` のトリガ付き escape hatch（framer-motion/canvas-confetti は境界根拠で却下確定）。バンドル予算 ≤ +20KB gz、全フェーズ契約/スキーマ無改変。docs のみ、コード無改変。
