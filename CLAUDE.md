...
- **2026-07-16** (4): UX — **生成時に命名フォームを出さない（ADR-073・実装済）**。ユーザ feedback「Frame とかインスタンスがいくつも生成されるオブジェクトはポンポン作りたい。生成時に命名フォームが出てきてほしくない。後で変更する前提でいい」。Frame 生成の全経路のうち、ビューポート配置と Solid N パネル `onAddFrame` は既に無言自動命名だったが、モバイル長押し `ContextMenuHandler.promptAddFrame` と CF N パネル子フレーム追加（`_promptAddFrame` 共有）だけが生成前に `showRenameDialog` を割り込ませていた（ADR-069 はダイアログを *正しく連番シード* まで直したが是非は不問）＝入口でフォーム有無が分岐。**実装**: `promptAddFrame` から `showRenameDialog` 除去、`createCoordinateFrame(effectiveParentId, null)` 直呼びの即時生成に（`null`→内部 `_nextEntityName('Frame')` 採番＝単一命名源 #9）。undo/redo・親解決（ADR-037 Origin CF）・生成後選択（ADR-069 契約）不変。メソッド名据え置き（docstring に「もはや prompt でない」明記）、`showRenameDialog` は改名専用に一本化。改名は生成と切り離した別の意図的操作として存続。docs: **ADR-073** 新規 + ADR-069 Links 後続参照 + CODE_CONTRACTS「One Naming Source」を「生成時フォーム無し」まで拡張。Evidence: unit **653 pass** / typecheck clean / build clean / E2E **12 pass**。契約・schema・DSL 版・BFF 無改変。（全文 → SESSION_LOG）
