...
- **2026-04-15**: Feature — ADR-033 CoordinateFrame Phase C (C-1, C-2, C-4): auto-Origin removed from `createCuboid/extrudeProfile/duplicateSolid`; N-panel "Frames" section with "+ Add Frame" button + `CreateCoordinateFrameCommand` undo/redo; Outliner `⊡` unreferenced badge + `setFrameUnreferenced()`; `updateNPanelForFrame()` shows unreferenced notice. ADR-033 Accepted.
