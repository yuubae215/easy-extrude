...
- **2026-03-27**: ADR-022 Undo/Redo via Command Pattern — full 4-phase implementation. `CommandStack` + `MoveCommand` (Grab/FaceExtrude Ctrl+Z/Y); `ExtrudeSketchCommand` (Profile↔Solid swap); `AddSolidCommand` + `DeleteCommand` (soft-delete via `detachObject`/`reattachObject`); `RenameCommand` + `FrameRotateCommand` + `MoveCommand` extended to all entity types; `SceneService.invalidateWorldPose`; `extrudeProfile` now emits events. ADR-022 `Proposed`→`Accepted`.
- **2026-03-26**: Precision controls for G/R — Ctrl+Grab snaps to grid (default 1 unit, Ctrl+Wheel cycles [0.1…10]); Ctrl+Rotate snaps to degree step (default 1°, Ctrl+Wheel cycles [1, 5, 10, 15, 22.5, 45, 90]). Status bar shows "Step: X°  Scroll to change" during rotate. `_rotate.stepSize`, `ANGLE_STEPS`, updated `_applyRotate`/`_onWheel`/key handlers.
- **2026-03-26**: Dev environment startup order — added `pnpm dev:all` script using `concurrently` + `wait-on`. Starts the BFF (port 3001) first; Vite launches only after `tcp:localhost:3001` is ready, ensuring WebSocket connections succeed on startup. `concurrently` and `wait-on` added as devDependencies to root `package.json`.
