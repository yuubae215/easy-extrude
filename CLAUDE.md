...
- **2026-03-22**: Validation review & improvement plan — executed all 6 phases of the 24-item improvement plan. Phase A: `await` fixes in `sessionManager.js` (CRITICAL data loss). Phase B: guarded `JSON.parse` in `sceneStore`. Phase C: WsChannel listener cleanup, `openGeometryChannel()` re-entry guard, `AppController.dispose()`. Phase D: `deserializeScene` moved into `SceneService._deserializeEntities()` (layer fix), outliner type naming. Phase E: ADR-008/009/013 gaps filled. Phase F: `showToast` on ImportedMesh block, Tab guard, A11Y ARIA labels. Plan: `docs/validation/2026-03-22-improvement-plan.md`.
- **2026-03-22**: Validation review & improvement plan — reviewed full-repo + Phase C validation report (24 actionable items). Produced 6-phase prioritised improvement plan at `docs/validation/2026-03-22-improvement-plan.md`: Phase A server async bugs (3 critical), Phase B client state consistency, Phase C layer/QC, Phase D ADR gaps, Phase E UX/A11Y, Phase F low-severity.
