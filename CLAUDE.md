...
| 例を土台に編集 / fork & tweak / fork example / 類推オーサリング / シード・アンカー / authorSeed / forkExample / buildSeedIndex / seed chip / 埋まった手本 / onForkTemplate | ADR-058 (Accepted, Phase 1 実装済), `src/context/SeedAnchor.js`, `src/context/SeedAnchor.test.js`, `src/controller/ContextController.js` (`forkExample`), `src/components/Context/TemplateGallery.jsx` (fork action), `src/components/Context/IntakePanel.jsx` (RequirementForm seed chips), uiStore `context.authorSeed`/`contextSetSeed` |
