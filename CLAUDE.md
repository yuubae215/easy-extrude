...
| CI / テストゲート / PR ワークフロー / rigor 遡及 / スキーマ昇格 / DSL JSON Schema 化 / conformance / 未契約ワイヤ / prefers-reduced-motion / E2E スモーク / pnpm test glob 化 | ADR-064 (Accepted, Phase 1 実装済; Phase 2–4 後続), ADR-062 (play 側の対), PHILOSOPHY #29, `.github/workflows/ci.yml`, `.github/workflows/deploy.yml` |
...
- **2026-07-08** (2): Infra — **ADR-064 Phase 1 実装（CI ゲート化）→ Accepted (Phase 1 実装済; Phase 2–4 後続)**。`ci.yml` 新設（PR + main push、単一 `gate` ジョブ: unit → contract → typecheck → build; submodule は `vendor/grasp-contract` のみ、Rust/C++ 不要）。`pnpm test` = glob `node --test "src/**/*.test.js"`（443 = 手動列挙 434+9 と全件一致 — #11 の silent-failure 構造除去）。`build` から wasm 分離（`build:full` 新設）+ `deploy.yml` の Rust ステップ削除 = committed artifact 一本化（§1.1）。全ゲートローカル green。残: branch protection は管理者設定。
