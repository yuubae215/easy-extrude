...
- **2026-07-08** (5): Infra/UX — **ADR-064 Phase 4 実装（play 側の残欠: 検証と逃げ道）→ Accepted (Phase 1+2+3+4 実装済)**。(1) `prefers-reduced-motion` 対応: 純粋 `flashStyle(tone, reduced)` を `FeedbackMath.js` に新設（motion 許可=fade / 削減=静的 tint、情報は保持）、副作用境界 `prefersReducedMotion`/`useReducedMotion` を `FeedbackPrimitives.jsx` に集約（matchMedia を 1 箇所で読む・非ブラウザ env=false）。`flashAnim`/`LandingFlash` は委譲、`ContextInspector` Badge 無限パルス と `IntakePanel` Section バッジ scale ポップは reduced 時 `animation:none` へ退行（バッジは残す・#11）。ユニット 3 本追加（glob 492→495）。(2) Playwright スモーク E2E: `playwright.config.js`+`e2e/smoke.spec.js` 4 本（boot / box+undo / テンプレ→negotiate / reduced-motion 退行）、dev サーバ実行で COOP/COEP→SW 自己スキップ、CI は unit `gate` と別ジョブ `e2e`（#20・flakiness 隔離）。`@playwright/test` 1.56.x（sandbox chromium 1194 一致）を root devDep 追加、artifacts gitignore。ローカル 4 PASS 8.8s、typecheck・build green。
