...
- **2026-05-22**: Feature — ADR-043 Phase 2: Zone 包含チェック（AnnotatedRegion × Solid）。`_xyPointInPolygon()` を SceneService.js モジュールスコープに追加；`_evaluateClearanceLinks()` に `contains` 分岐を追加（全 8 コーナーが Zone ポリゴン内か判定、複数リンクは OR 合成）；`detachSpatialLink()` で tint クリア。`MeshView.setConstraintViolated(bool)` 追加・`_syncEmissive()` で赤 tint と選択 blue を合成。CODE_CONTRACTS §Visual State Ownership 更新。
- **2026-05-16**: Refactor — Phase 4: complete Preview Pipeline. Added `SceneService.applyPreviewEndpointMove(obj, endpointIndex, worldPoint)` — third method in the trilogy. `EndpointDragState.onPointerMove` now delegates to service instead of direct vertex mutation; `cancel()` uses service for view-consistent restore. `sceneService` added to `AppController._editCtx`. CODE_CONTRACTS §"Preview Pipeline" updated.
- **2026-05-16**: Refactor — Phase 3: runtime Operation State Machine (ADR-039). All Object Mode ops migrated to `AppController._opState` (StateMachine): `S_GRAB_ACTIVE`, `S_ROTATE_ACTIVE`, `S_FACE_EXTRUDE`, `S_MEASURE_PLACING`, `S_LINK_MODE`, `S_FRAME_PLACEMENT`, `S_MOUNT_PICKING`. Parallel `AppController._editOpState` (StateMachine) for Edit Mode with `EndpointDragState` as first state class. `_endpointDrag` ad-hoc flag eliminated. `src/core/StateMachine.js`, `src/core/editorStates.js`, `src/core/states/EndpointDragState.js` added.
- **2026-05-16**: Refactor + Bugfix — Phase 2: preview update pipeline. `SceneService.applyPreviewTranslation` and `applyPreviewRotation` established as canonical preview mutation methods. `AppController._applyGrabDeltaToAll`, `_applyRotate`, and `_objDragging` path now delegate entity-type dispatch to these service methods. Bugfix: `_applyStackSnap()` was directly mutating `corners` after `selObj.move()`, leaving `_position` stale at the pre-snap Z (ADR-040 violation). Fixed by calling `selObj.move(segStartPos, snapDelta)` with `snapDelta = currentDelta + (0,0,zOffset)`. CODE_CONTRACTS §"Solid Pose Mutation" updated with stack snap rule; §"Preview Pipeline" added.
- **2026-04-19**: Maintenance — ROADMAP sync + ADR-034 draft: ADR-029 Phases 1–3 / bugs #71/#72 / ADR-032–033 Completed table rows all found implemented-but-unmarked and updated. Design — CF placement philosophy: face-normal auto-alignment rejected; placement is arbitrary 3D (face/interior/edge/vertex); default pose = parent implicit local axes; pose changes only on stakeholder-relationship demand; integrator holds CF authority. `ADR-034-coordinate-frame-placement-policy.md` created (Draft). PHILOSOPHY elevation deferred pending 2+ unrelated contexts.
