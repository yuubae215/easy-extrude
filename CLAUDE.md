...
| CI / テストゲート / PR ワークフロー / rigor 遡及 / prefers-reduced-motion / E2E スモーク / pnpm test glob 化 | ADR-064 (Accepted, Phase 1+2 実装済; Phase 3–4 後続), ADR-062 (play 側の対), PHILOSOPHY #29, `.github/workflows/ci.yml`, `.github/workflows/deploy.yml` |
| DSL スキーマ昇格 / Layout·Context DSL JSON Schema 化 / conformance / additionalProperties:false / ドリフト束縛 / 形と意味の分離 / schema:validate / 未契約ワイヤ (Phase 3 後続) | ADR-064 Phase 2 (実装済), PHILOSOPHY #29, `schema/layout-1.0.schema.json`, `schema/context-0.4.schema.json`, `schema/README.md`, `src/schema/LayoutSchema.test.js`, `src/schema/ContextSchema.test.js`; 意味検査は `src/layout/LayoutValidator.js` / `src/context/ContextValidator.js` に残る |
