...
- **2026-05-16**: Bugfix — ADR-040 completion: `_objCtrlDrag` primary triple violation. The Ctrl+drag rotate path directly mutated `vertices[i].position` via inline quaternion math, leaving `_position`/`orientation`/`localCorners` stale. Subsequent G-key grab, R-key rotate, and face extrude would operate on stale primary triple values. Fixed by snapshotting `orientation`+`_position` at drag start and calling `Solid.rotate()` in pointermove for Solid objects. Non-Solid entities retain old corner-mutation path. No command push (ctrl+drag is intentionally non-undoable). CODE_CONTRACTS §Solid Pose Mutation Must Use Public API already covered the rule; no new rule added.
