...
- **2026-03-23**: CoordinateFrame auto-origin + axis labels — Removed orange wireframe selection sphere; added X/Y/Z text label sprites (CanvasTexture Sprite, colour-matched per axis) positioned past each arrowhead for immediate direction readability. `SceneService.createCuboid()`, `extrudeSketch()`, and `duplicateCuboid()` each auto-call `createCoordinateFrame(id, 'Origin')` so every 3D object gets an origin frame at creation (single entry-point design means 2D→3D transition is covered by `extrudeSketch` without special-casing). `createCoordinateFrame` gains optional `overrideName` param. MENTAL_MODEL updated.
- **2026-03-23**: CoordinateFrame visibility fix — `CoordinateFrameView.setObjectSelected()` toggles `depthTest`/`renderOrder` on all axis + sphere materials: selected → `depthTest: false` + `renderOrder: 1` (always visible through geometry); deselected → normal depth testing (frame may be occluded). MENTAL_MODEL updated.
