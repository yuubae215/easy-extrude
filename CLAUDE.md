...
- **2026-07-05** (3): Feature — **ADR-063 Phase 3 実装（ガイド付きインテーク・ウィザード）→ Accepted (Phase 1+2+3)**。純粋層 `WizardCatalog`（定義アセット `wizard/1.0` + FSM `null/step(k)/review` — `nextWizardState` はゲート不成立時に同一 state を返す = 不正遷移表現不能、各ステップの `formGaps` は IntakeAssist 述語の**同一関数参照**、11 テスト）。uiStore `context.wizard`（丸ごと置換・sole writer は `ContextController` — grasp FSM と同じ規律）、`WizardPanel` = ContextLayer negotiate の `'wizard'` タブ（IntakePanel の同一フォームを export して埋め込み — コミット境界は `onAddDocEntry` のみ、ウィザードは順序付きの器）。白紙 doc の初期タブを `'wizard'` へ。テスト 392/392（+11）、tsc・vite build クリーン。残: Phase 4–5（パラメトリックビューワ/統合）+ ADR-062 全フェーズ。
