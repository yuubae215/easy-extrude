...
- **`src/` はアイデアも重い処理も持たない。** 判定は §AI 向けガード の 3 軸
  (アイデア / 重さ / 公知の閉形式 — ADR-146) ただ 1 箇所で行う。**ここに軸を複製
  しない** (§1.1) — かつてこの行に在った話題の列挙は ADR-146 が退役させた。
  フロントは DSL で `ik_solvable` 等の **参照名を宣言**し、結果は契約経由で受け
  取って表示する。`src/` から `core/` を import する経路は存在しない
  (越境は必ず HTTP + 契約)。
...
Schema への *binding* として準拠テストで縛る — どちらも契約を *定義/拡張* しない。
**response** 契約 (デプロイ境界) の変更は Schema + `contract-version.json` を **同一 PR で**
更新する意図的な版上げ行為のみ (response schema 変更に版上げが無ければ CI `contract-wall`
が fail — ADR-082/084 §4。デプロイ単位間のズレは従来どおりコアAPI が 400 で即拒否 —
ADR-074)。**request** 側は open payload / layoutVersion 統治の非対称なので、optional 追加は
contractVersion を上げない (ADR-083/084 の先例 — 壁は request スキーマを見ない)。
...
ある計算を `src/` に置いてよいかは **3 軸を順に問って**判定する (ADR-146)。
話題の列挙 (「IK / 干渉 / リーチ / 安定性」) で判定しない — 列挙に無い計算が来た日に
何も答えず、列挙に在る計算でも理由が違えば答えが違うため (ADR-053 が FK による到達
判定を front に許しているのがその例)。

1. **アイデアか** (コスト関数・重み・ランキング・ヒューリスティック・キュレーション
   された判断知) → **`core/` のみ**。**この軸が最優先** — 一意で軽くてもアイデアなら
   出さない。ワイヤに載るのは *ソルバが決定した事実* だけ (ADR-060 / 原則 #29)。
2. **重いか** (探索・組合せ・反復) → **`core/` のみ**。計算資源を端末へ押し付けない。
3. **公知の閉形式・一般ロジックか** (順運動学・DH 変換・幾何プリミティブ・単位変換・
   正規形) → **`src/` 可。ただし第二の源ではなく*導出*として** = 共有フィクスチャの
   準拠テストを**同じ PR で**置く (`fixtures/cross-language/` + 登録簿
   `src/robotics/CrossLanguageDerivation.test.js`)。§1.1 が禁じるのは*複製*ではなく
   **源が 2 つになること**で、源が 1 つであり続けることを機械が問うなら二つ目は導出
   である (BFF が schema から型を導出するのと同じ形)。

どの軸も真にならない計算は「**まだ決めていない**」— **「対象外」と書かない**
(原則 #29 の二状態。対象外と書けば次に問われたとき決着済みに見える)。

1 か 2 に当たるコードを `src/` に書こうとしたら**作業を中断**し、「それは `core/` の
責務です (フロントは宣言と表示のみ — 越境は契約経由)」と促して `core/` 側へ誘導する
こと。`core/` 内での解法実装は in-scope (ADR-075 の pure/副作用規律に従う)。
...
**response** 契約 (`*-response.schema.json`) を **contractVersion bump なしで** 編集しよう
としたら中断 — response 契約変更は Schema + `contract-version.json` + 両側の導出/準拠テストを
同一 PR で更新する版上げ行為としてのみ行う (ADR-082/074/084 §4。閉層に `optional` 兄弟を
生やさない統治 ADR-060 は従来どおり)。**request** 契約への optional 追加は版上げ不要だが、
`robot` 等の閉じたオブジェクトに足す場合はスキーマ更新自体は必要 (open payload で
素通しできるのは `additionalProperties:true` の階層のみ)。
...
**`/whiteboard [作業]`** — 実装前の一拍俯瞰 (核 §2)。**語彙の充足 (原則 #33 — 言いたい文が今の語で言えるか)** / Goal / 触る実体の状態表 (**基数 0·1·N
...
(**原則 #31**) — 台帳の基数列はそのための欄なので必ず埋める (ADR-090 の欠陥はここが
空だったこと)。同じ盲点はコードの外にも出る: 検査を書くときは *在るもの* を辿らず
**必要な種別を列挙して個数を検査**する (`.gsn` の支えの無い goal → `pnpm test:gsn`)。
