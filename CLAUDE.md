...
- **2026-07-12** (2): UX — **実体化/消滅ボクセルバーストの演出強化（ADR-065 Phase 2 の production-value 向上）**。ユーザ指示「Cube 出現をもっとクール（ディゾルブ）に、感動できる UI に」。実 solid の material dissolve は #4 で棄却 → 既存 overlay バーストを animation-fx アンチバニラゲートで引き上げ（#30 は「事実がすでに可視なほど演出は控えめ・消滅は最大 cue」= 既存 Tier F の production value 向上を明示的に許容; 事実源・発火条件・silence・reduced 静的キューは無改変）。純粋層: `voxelFrame` 属性別イージング化（dissolve=expo バースト+強タンブル、materialize=expo 収束+`easeOutBack` 着地ポップ; 既存テスト不変条件保持）+ 新 pure 4 本（`voxelDelay` 黄金比 per-voxel ステージャ / `localProgress` global→local 再マップ / `voxelFlash` 境界フラッシュ単発スパイク / `voxelEnvelope` 全体アルファ包絡）+ `MotionMath.easeOutExpo`。View: 24→40 ボクセル（依然 1 draw call）・`AdditiveBlending`・ステージャ再生・白フラッシュ lerp（per-voxel フェードは scale に fold）・reduced 静的経路分岐保持。tokens: materialize 520→640ms / dissolve 700→860ms。**追撃（ユーザ「生成の一点凝縮は分かりづらい、外形を作り上げる方向に」）**: materialize（実 cuboid = 8 corners）を radial 凝縮から**アウトライン構築** `WireframeAssembly` に刷新 — anchor が実 OBB corners を運び、`buildGeometry`+`EdgesGeometry`（MeshView と同一経路 = 回転一致）から 12 稜線を得て**稜線が 1 本ずつ描き上がる**（`localProgress` ステージャ）+ ボクセル火花が外から稜線上へ着地して組み上げ → 集合フラッシュ → envelope フェードで実 solid 顕現。overlay のみ・emissive 不可侵 #4・非決定なし・dispose 対称。非 cuboid（profile 等）は従来 `VoxelBurst` fallback（`createLandingEffect` factory で分岐）。dissolve の飛散は無改変。Evidence: unit 621 全 pass / typecheck clean / E2E 7 pass / build clean / 実機目視（materialize=稜線トレース+ブロック組み上げ→クリーン顕現、dissolve=飛散維持、reduced 静的、pageerror 0）。契約・schema・DSL 版・BFF 無改変。
- **2026-07-12**: UX — **スナップ係合フラッシュ実装（ADR-065 Phase 2 最終候補 → 全候補完了・プログラム完結）**。animation-fx スキル導入セッション: スキル品質ゲートを Motion Tier 規律の内側で適用（「静止禁止」ゲートのみ #30 装飾禁止が上書き）。新純粋 `SnapFeedbackMath.js`（engage/retarget のみ発火・保持/解放は機械固定の無音・stack key は着地 Z 量子化・`DURATION.snapFlash ≤ 300ms` テスト拘束）+ `SnapFlash.js`（Governor 経由・activeCamera ビルボード・reduced 静的リング）+ `GrabOperationHandler._syncSnapFx`（`_snapFxPrev` は controller-local 演出履歴）。事実源の弁別を #30 corollary に明記: 禁止は未確定*結果*の演出、係合*イベント*は既に起きた事実。Evidence: unit 594→616 / typecheck / contract 23 / E2E 6→7 pass（grab+stack 生死ガード追加）/ 実機目視（緑リング+エコー→減衰→消滅・reduced 静的・pageerror 0）/ build **+1.37KB gz**（累計 ≈+12.0KB、予算 ≤+20KB、依存 0）。契約・schema・DSL 版・BFF 無改変。
