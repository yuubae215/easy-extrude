...
| CI / テストゲート / PR ワークフロー / rigor 遡及 / スキーマ昇格 / DSL JSON Schema 化 / conformance / 未契約ワイヤ / prefers-reduced-motion / E2E スモーク / pnpm test glob 化 | ADR-064 (Proposed), ADR-062 (play 側の対), PHILOSOPHY #29, `.github/workflows/deploy.yml` |
...
- **2026-07-08**: Docs — **横断レビュー + ADR-064 起案（Rigor 側の遡及プログラム）→ Proposed**。全リポ横断調査で PHILOSOPHY #29 の非対称を確認: play 側は ADR-062 で全 UX 面へ展開済みだが rigor 側は grasp ワイヤに閉じたまま（CI テスト 0 本・PR トリガ無し、`test:contract` 未配線、`test:context` 手動列挙 = silent failure 構造、Layout/Context DSL が JS 定数で中立スキーマ成果物でない、ajv は grasp のみで scenes/WS/import 無契約、`prefers-reduced-motion` 0 件、E2E 無し）。ADR-064 = 4 フェーズの遡及: ① CI ゲート（PR ワークフロー + `pnpm test` glob 化 + `build` から wasm 分離）② DSL の JSON Schema 昇格（conformance + 同値テスト）③ 未契約ワイヤの二択宣言（scenes は検証、WS/import は期限つき対象外宣言）④ play の検証（reduced-motion 退行形 + Playwright スモーク）。GitHub Actions は public リポジトリにつき無料無制限（課金の崖なし）を確認。コード無改変。
- **2026-07-07**: Feature — **ADR-063 Phase 4+5 実装（パラメトリック 3D アセット + ビューワ / 統合）→ Accepted (全 5 フェーズ実装済)**。純粋層 `ParametricAssets`（`parametric/1.0`、パラメータスキーマ + `layout/1.0` fragment ビルダー×3 アセット、`clampParams` は total、`applyAssetCommit` は変数 upsert + fact 置換 = 再コミット非重複、13 テスト）。`ParametricPreviewView` ゴースト（**importFromJson 非経由** — 正準シーン/undo 無傷、楽観プレビュー/悲観コミット）。`ContextController` sole writer `context.assetViewer`、コミットは `createDocEditCommand` 1 本 = 数値だけが成果物（Goal 2）。Phase 5: wizard variables 步に `assetSource` 宣言で埋め込み、Gallery に Guided Intake カード（wizard 自動開始）、Empty Project を Expert 棚へ。テスト 434/434、tsc・vite build クリーン。契約/BFF/ドメイン/DSL 版無改変。**ADR-063 完了。**
- **2026-07-05** (5): Feature — **ADR-062 Phase 4+5 実装（測定器チェック面 + テンプレート構造プレビュー）→ Accepted (全フェーズ実装済)**。Phase 4: `ContextService.projectChecks()`（validator `checkResults` × doc ベイク済オペランドの唯一の join — Gap join と同型）→ `context.checks`、純粋層 `CheckFeedbackMath`（`checkStatusKeys`/`checkTransitions`/`unsettledCount`/`checkMeter` — 惜しさ曲線は ADR-061 `nearMissCloseness` を import で共有、blocked はメーター無し #11、14 テスト）、`ChecksPanel` = negotiate `'checks'` タブ（pass/fail/blocked バッジ + violation 逐語 + blocked→pass 緑フラッシュ + 生数値併記メーター）。新例 `cell_robotics_context.json`（pass/fail(惜しい 22vs25)/blocked 三態 — Questions 回答で blocked→pass がその場で閉じる）+ テンプレート Robotics Checks。Phase 5: `TemplatePreviewMath.structurePreview`（`canonicalForm` 事実の層カウント + 署名 8 桁、比較・ランキング無し — ADR-056 §3）→ ギャラリーカードの Why/How/What バー。テスト 421/421（+20）、tsc・vite build クリーン。契約/DSL 無改変。残: ADR-063 Phase 4–5（パラメトリックビューワ/統合）。
