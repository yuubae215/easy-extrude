...
| CI / テストゲート / PR ワークフロー / rigor 遡及 / prefers-reduced-motion / E2E スモーク / pnpm test glob 化 | ADR-064 (Accepted, Phase 1+2+3 実装済; Phase 4 後続), ADR-062 (play 側の対), PHILOSOPHY #29, `.github/workflows/ci.yml`, `.github/workflows/deploy.yml` |
...
- **2026-07-08** (2): Infra — **ADR-064 Phase 1 実装（CI ゲート化）→ Accepted**。`ci.yml` 新設（PR + main push、単一 `gate` ジョブ: unit → contract → typecheck → build）。`pnpm test` = glob `node --test "src/**/*.test.js"`（#11 の silent-failure 構造除去）。`build` から wasm 分離（`build:full` 新設）+ `deploy.yml` の Rust ステップ削除 = committed artifact 一本化（§1.1）。残: branch protection は管理者設定。
