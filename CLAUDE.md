...
- **2026-07-13**: UX — **ビューポート常設ステージ + 起動リビール（ADR-067 Accepted — ADR-066 Tier D の最初の適用）**。ユーザ指示「成長分野を見つけ、ゲームのような感動する UX へ」→ ギャップ分析で「全フレーム見続ける舞台そのもの（背景・奥行き・光・起動の瞬間）が素の Three.js のまま」= Tier D 開放後の最大の未開拓分野と特定。新純粋層 `StageMath.js`（14 テスト: 決定論ダスト配置/非整数比 2 周波ドリフト/層スタガー entry/フォグ密度 #27/フライト **p≥1 恒等**拘束）+ `SceneStage`（縦グラデ背景・FogExp2・フロアグロー・ダスト 2 層・リムライト; **SceneView 所有の永続ビュー**・`scene.background`/`fog` の単独書き手 #4・`_updateGridScale` 追従 #27・reduced は単一境界購読でドリフト凍結 = 静的ステージ残存 #30/#11）+ `BootReveal`（`MotionGovernor.spawn` 経由の一回性カメラ進入 1.8s → 既定構図に恒等着地; 最初のキャンバス pointerdown/wheel/context ロードで即着地・external-write ガードで fitCameraToSphere に譲る・`?demo=context` は張らない・reduced = 最終ポーズのみ）。`DURATION.bootReveal` 追加(COLOR 追加なし)。CODE_CONTRACTS §1 新行・ADR-066 References 相互リンク。Evidence: unit 635 全 pass / typecheck clean / contract 23 / E2E 7 pass / 実機目視（フライト着地・ダスト entry・reduced 静的・pageerror 0）/ build **+1.95KB gz**（累計 ≈+14KB、予算 ≤+20KB、依存 0）。契約・schema・DSL 版・BFF 無改変。
- **2026-07-12** (3): PHILOSOPHY — **#30 Motion Tier に Tier D「delight」を追加（遊び心の制約緩和）**。ユーザ指示「序盤の演出過剰抑止項目が遊び心の成長を妨げている、制約を外したい」。調査で当該項目は #30（序盤ではなく最新・ADR-065 由来）と特定し、齟齬を提示 → ユーザ選択「delight allowance を追加（構造・CODE_CONTRACTS リンク維持）」。ドキュメントのみ変更（コード無改変）: `docs/PHILOSOPHY.md` #30 に **Tier D — delight**（非命題的・意図的・予算内・reduced 対応の歓び演出を許容）を追加、routing test を「'nothing' → 不採用」から「'nothing' → Tier D へ委ねる」に反転（旧文を日付付きで引用＝ドリフトでなく意図的改訂を明示）、Forbidden を「無規律/欺瞞的」に再定義。2 構造 corollary（単一 reduced-motion 境界・transient 単一所有者）と Underlies フッタは無改変 = 参照 CODE_CONTRACTS 4 規則は有効のまま。volume corollary に「delight も scarcity 予算を消費」を追記。Index 行と CLAUDE.md doc-nav ポインタ同期（#19）。制約は「装飾禁止」→「未宣言・無統治・欺瞞的な動きの禁止」へ移行。Evidence: 目視レビュー（#30 通読で整合）。**ADR-066 起票（Accepted）** — ADR-065 Widening 1 を amend、旧 tier 表は履歴として残し相互リンク、README 索引 + #30 back-ref。ユーザ判断「生成/削除エフェクトは感動した・あってよい・UI は退屈であってはならない」を Context の一次証言として記録。
