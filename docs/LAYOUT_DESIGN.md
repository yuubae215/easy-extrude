...
| Grasp Search panel (ADR-054 — UI→DSL→BFF→grasp-search verification) | `fixed; inset:0` centred modal over a `rgba(0,0,0,0.6)` backdrop; **z-index 300** (above all edge panels — transient, PHILOSOPHY #26) | dialog `width: min(560px, 94vw); max-height: 88vh`; weights/topN input row + ranked candidate cards |
