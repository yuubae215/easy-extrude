...
**Related**: ADR-044 (5W1H Function Mapping), ADR-045 (External Layout API), ADR-037 (Body Frame), ADR-030 (SpatialLink), ADR-047 (Context Demo Layer), ADR-049 (Requirement / Conflict モデル — 本 DSL の L2.5 拡張), ADR-050 (Context-First Project Model — 本 DSL を正準アーティファクト化)
