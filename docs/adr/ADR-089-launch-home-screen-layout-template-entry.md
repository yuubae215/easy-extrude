...
2. **選択 → 単一の読込経路**: カード選択は `compileLayout(dsl) → importFromJson(scene, {clear:true})`
   ...
   増やさない。空プロジェクトは既定シーン（ボックス1個）へそのまま抜ける。着地は常に S-01。

   **既定シーンの整え（follow-up 2026-07-24）**: 空プロジェクトの着地先＝既定 boot シーン
   なので、その見栄えも本 ADR の責務に含める。従来の boot は原点中心の 2m キューブ
   （半分が地面下）＋ `[-2,2,0]` に孤立したロボット骨格で、初回の印象が雑然としていた。
   (a) スターターキューブを地面に載る 1m キューブに整える（`Solid.setPose` 経由 — §1.1、
   フィールド直書きしない）、(b) ロボット骨格は既定で**非表示**にする（ADR-087 の所有者＝
   `robot_base` の Outliner 目アイコン経由で状態一貫。フレーム自体は grasp のため残す。
   テンプレ読込後も同様に非表示 — テンプレ固有の robot Solid が孤立 UR5e 装飾に隠されない）。
   これらは boot 契約（"Cube" が選択済み・ツアー 1/5）を保つ範囲の整えで、e2e スモークと
   両立する。
