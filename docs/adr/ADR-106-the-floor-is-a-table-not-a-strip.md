...
goal ごとの支えの正本は `.gsn` 側 (ここには複製しない — §1.1)。**Accepted 時点で 6 goal は
実行された証拠を持つ** (`src/FloorContainerCensus.test.js` / `src/view/EdgeOccupancy.test.js` /
`e2e/smoke.spec.js`)。
...
- **`uiStore.context.loaded` の読み手が 0 個であることを根拠に、フィールドが 0 個。**
  (実装前: 書き手 1 / 読み手 0 = 未使用の第二の源)

## 実装で分かったこと (2026-08-02 — 原則 #19)

決定は変わらなかったが、実装が**予告になかった事実**を 3 つ出した。ADR に残すのは、
どれも「次に同じ形を踏む人が同じところで気づく」種類のものだからである。

1. **数えられる値域が存在しなかった。** `.gsn` の `RetirementCensusIsPlanned` は反証形として
   「`inspectorTab` は文字列でオープンな値域なので、宣言そのものが存在しない」を挙げていたが、
   **その反証形のほうが正しかった** — 値域は JSDoc の union コメントにしか無かった。
   したがって D5 は「退役値を消す」ではなく「まず値域を有界な宣言 (`src/view/FloorTabs.js`) に
   する」から始まった。数えられない形は数える前に直す。
2. **下端の所有者を先に置いた瞬間に、既存のパッチが 2 個出た。** `.gsn` の
   `BottomEdgeOwnerIsPlanned` は「Toast の `bottom:96px` がその形で書かれているかは実装時に
   確認する — 書かれていなければそれ自体がパッチ 1 個目」と予告していた。実測は **2 個**で、
   Toast は desktop と mobile で**同じ 96px** (mobile はツールバーだけで 86px を食う) を書き、
   `TourCard` は**原則 #26 をコメントで引用しながら** `left: 192` / `bottom: 38` を literal で
   書いていた。後者が重い — 規則を知っている人が書いた違反は、レビューでも読み飛ばされる。
   コメントは計算ではない。
3. **器を広げると、器の中の入口が画面からはみ出す。** `+ 追加` に Assets 群 (5 項目) が
   増えた結果、ポインタが画面下半分にあるとメニューの末尾が視野外へ落ちた — 項目は
   存在するのにクリックできない = 無言の行き止まり (原則 #11) であり、
   「発見可能性は成果物」(原則 #16) は*在ること*では満たされない。e2e が
...
5. **移設先を作ると、移設元にあった「場に入る」条件が余分だと分かる。** grasp / assets /
   ...
   タブの移設の副産物として効いた)。**器は責務だけでなく前提条件も汚染する。**
