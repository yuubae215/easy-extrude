...
- Status: Proposed (棚卸しの実測は本文 §力学。決定は「残しを第一級の宣言にし、母集団を導出して数え、満期を機械が読む」— 検査の実装は Phase 6)
- Date: 2026-08-04
...
- **検証 (証拠):** `docs/gsn/adr-109-a-deferral-is-a-declaration-not-a-memory.gsn`。
  実装 (Phase 6) の証拠は `scripts/check-deferrals.mjs` の 4 本の問い —
  (1) 宣言外の残しの個数 ≤ baseline、(2) 満期が過ぎた `PROVISIONAL_UNTIL` = 0 件、
  (3) 登録簿の全行が ticket を持つ = 未 ticket 0 件、(4) 実在しない残しの宣言 = 0 件。
  **現時点の証拠はすべて未来形である** (Proposed の理由)。
- **波及 (blast radius):** 新規 `docs/DEFERRAL_LEDGER.md` + `scripts/check-deferrals.mjs`
  + `package.json` の `test:deferrals`。既存コードの挙動は 1 行も変えない (`PROVISIONAL_UNTIL`
  の文字列 1 つを除く — 満期の付け替え)。`docs/NAVIGATION.md` に索引行。
