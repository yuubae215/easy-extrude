...
## UX 具体化 — 段1 ゴーストの視覚設計と門の堅さ（2026-07-03 設計追記, コード無改変）

設計軸は PHILOSOPHY #29 の徹底: **論理検証（門・契約・能力判定）は堅く、体感（グリフ・色・
アニメ）は遊び満載に**。遊び側はすべてワイヤ事実（`score`・`pose.kind:'endEffector'` の
frame）からの *クライアント導出* で、契約には一切足さない。

### A. 堅い側 — 門と能力判定（rigor）

1. **能力ゲートは検査で決める、推測しない**: ゴーストを出す条件は
   `candidate.pose?.kind === 'endEffector'` かつ frame の `position`（長さ 3 配列）・
   `orientation`（長さ 4 配列）の形検査に合格すること。それ以外（opaque・`jointSpace`・
   形不一致）は **出さない** — パネル既存の「spatial view unavailable」系の正直なキャプション
   に留める（PHILOSOPHY #11: 偽の姿勢を捏造しない）。ヒューリスティックな opaque 解釈は禁止。
2. **`frame` 基準系の仮置きは一点に封じる**: upstream 未確定の world / base-link 問題は、
   `GraspGhostView` 冒頭の単一定数 `FRAME_CONVENTION = 'world' /* assumed — upstream 未確定 */`
   に封じ、確定時の変更点を 1 箇所にする（§1.1）。確定までゴースト脇のキャプションに
   `frame: world (assumed)` を明示 — 仮定を隠さない。
3. **接近方向は規約から導出**: 接近ベクトルは end-effector frame の −Z 軸（グリッパ TCP 慣習）
   と *規約として文書化* して導出する。ワイヤに `approachVector` を要求しない（PHILOSOPHY #29
   の成長ガバナそのもの）。規約が実サービスと食い違ったら規約定数を直す — 契約は動かない。
4. **対象ハイライトは emissive を触らない**: 掴む対象の強調は、`MeshView` の
   `cuboidMat.emissive`（`_syncEmissive()` の単独所有 — CODE_CONTRACTS「Visual State
   Ownership」）に手を出さず、`GraspGhostView` が自前で持つ `EdgesGeometry` アウトライン
   （ゴースト色）を追加・破棄する。生成と破棄は同ビュー内で対称（PHILOSOPHY #9）。

### B. 遊び側 — グリフ・色・アニメ（すべて導出、1 ビューに閉じる）

1. **グリフ**: 様式化 2 指グリッパ（丸めた 2 フィンガー + パーム・バー、プリミティブのみ・
   メッシュ資産不要）。サイズは画面基準 ~40px 相当を狙い、シーン半径由来の world 上限で
   クランプ（PHILOSOPHY #27 のペア則）。接近ベクトルは対象へ向かって細るテーパー矢印。
2. **スコア → 色/質感**: `totalScore` を色相へ（高=青緑、低=アンバー）、3 つの真偽
   （`withinReach`/`ikSolvable`/`interferenceFree`）は全✓=実線 / どれか✗=破線の線種へ
   マップ。順位はスコアバー（ADR-057）が正で、ゴーストは順位を並べ替えない —
   score-first は不変。
3. **hover / select の二段体感**: 候補行 hover でゴーストが 40% 不透明度に 150ms
   フェードイン（プレビュー）。クリック選択で 90% へ確定し、**接近アニメ**: グリフが
   接近ベクトル上の 1.5 グリフ長手前から ease-out 400ms で滑り込み、着地で指がわずかに
   閉じる。再選択で再生。これが「どこを・どの向きから」が *体に入る* 瞬間で、本 ADR の
   Goal の中核体験。
4. **比較モード（任意・後続）**: 「top 3 を重ね表示」トグルで上位 N ゴーストを低不透明度 +
   ランクバッジで併置、選択中のみフル表示。候補間の空間差（同じ面か、逆側か）が一目になる。
   追加データ不要 — 既存 candidates 配列の導出。
5. **所有と寿命**: 以上すべて `GraspGhostView` 単独所有（Decision §3 のとおり）。
   `selectedRank`/hover の変化で生成・更新し、`exit`/`contextEnd`/`idle` 遷移で破棄。
   アニメ進行は view ローカル（FSM を増やさない — 下記）。

### C. §1.4 再判定（アニメ状態）

hover/選択/接近アニメで見かけの状態は増えるが、全て `GraspState.results.selectedRank` と
ポインタ hover からの **導出射影 + view ローカルなアニメ位相**であり、不正遷移でデータ破損は
起きない。よって本追記でも新 FSM は起こさない（§5 過剰モデリング禁止を維持）。

### 検証（この追記の証拠計画）

- A-1 の形検査・A-3 の接近ベクトル導出・B-2 のスコア→色マップは純粋関数に切り出し
  `node --test` 可能（THREE-free の数値部分）。
- A-2 は「`FRAME_CONVENTION` 参照が 1 箇所」を grep で機械検査可能。
