...
- References: ADR-057（Grasp UI — score-first / G3 を deferred）, ADR-053（ロボティクス測定器 / urdf-loader を deferred / `Kinematics.js` FK）, ADR-047（ゴースト系譜の視覚語彙）, ADR-054（UI→DSL→BFF→grasp）, ADR-045（Layout DSL = 宣言と幾何）, ADR-060（pose の kind 判別 union — 門1 の構造）, grasp-contract（response `pose` の所有 — 上流）
...
- **未充足（明示, §5）**: 段1 は**門1（上流の TCP フィールド）にブロック**される。よって本 ADR は
  Proposed で、実装着手は門1 の充足が前提。先に上流契約へ「pose の `kind:'endEffector'` 枝」（ADR-060）を提案し
  `contractVersion` を上げてから段1 に入る、が正しい順序（証拠なき完了禁止）。
