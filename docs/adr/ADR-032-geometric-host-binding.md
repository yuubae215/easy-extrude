...
良い例（遅延生成 / ADR-033 の方針）:
  Solid_Tray → CoordinateFrame を一切生成しない
...
CoordinateFrame の作成動機と auto-Origin の廃止については ADR-033 を参照。
...
class SpatialLink {
  constructor(id, sourceId, targetId, linkType) {
    this.id       = id
    this.sourceId = sourceId   // Annotated* entity ID (mounts の場合)
    this.targetId = targetId   // CoordinateFrame entity ID（幾何学的拘束では必須）
    this.linkType = linkType
  }
}
```

**幾何学的拘束（`GEOMETRIC_LINK_TYPES`）における targetId の扱い：**

- `targetId` は **必ず `CoordinateFrame` エンティティの ID** でなければならない
- Solid を直接 target にしない。Solid の「どの点か」を明示するために
  CoordinateFrame を作成してからリンクを張る（ADR-033 §2 参照）
- `sourceId` は Annotated\* エンティティの ID。
  Annotated\* は暗黙のローカル空間を持つため（ADR-033 §6）、
  別途 CoordinateFrame を作成する必要はない

位相的・意味的拘束（カテゴリ B / C）では `sourceId` / `targetId` は
任意のエンティティ ID を受け付ける。
...
| linkType | source に有効な型 | target に有効な型 | 備考 |
|----------|-----------------|-----------------|------|
| `mounts` | Annotated\* | **CoordinateFrame のみ** | target は Solid 直接不可 |
| `fastened` | Solid / Annotated\* | **CoordinateFrame のみ** | 同上 |
| `aligned` | CoordinateFrame | **CoordinateFrame のみ** | 同上 |
...
幾何学的拘束（上位3行）で `targetId` に Solid を直接指定することは
ADR-033 に反する。UI はこの validation を強制し、
ターゲット選択時に CoordinateFrame のみをハイライト表示する。
...
```
hostPose H = worldPoseOf(link.targetId)  // targetId = CoordinateFrame（必須）
                                          // CF の worldPose は parentId(Solid) の
                                          // worldPose × CF.translation で合成される
...
1. エンティティ選択 → `L` キー
2. ステータスバー「Click target frame」
3. **CoordinateFrame をクリック**（幾何学的拘束の場合、CF のみハイライト）
   ← Solid をクリックした場合は「Add a frame to this object first」とガイドを表示
...
| ユーザー操作 | 結果 |
|------------|------|
| **CoordinateFrame** をタップ | validation 通過 → `MountAnnotationCommand` |
| Solid をタップ（CF なし） | トースト「Add a frame to this object first」 |
| 空白をタップ | キャンセル |
| ステータスバー ✕ | キャンセル |
...
- ADR-033 — CoordinateFrame Phase C（auto-Origin 廃止・インタフェース契約モデル）
