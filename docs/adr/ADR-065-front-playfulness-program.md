...
- Status: Accepted (Phase 0+1+2+3 実装済 — 2026-07-10。Phase 2 は CommandStack
  着地系〔箱スポーン・押し出し・Move/Rotate settle・undo 巻き戻し・redo〕を実装、
  スナップ係合フラッシュと選択パルスは Phase 2 残として後続。Phase 3 は
  クローム刷新 + disabled-as-quest を実装（escape hatch トリガ不成立 =
  `@react-spring/web` 不採用 — 実装ノート参照）。Phase 4–6 未実装)
