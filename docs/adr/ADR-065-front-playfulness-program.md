...
- Status: Accepted (Phase 0+1+2+3+4+5 実装済 — Phase 5: 2026-07-11。Phase 2 は
  ...
  Phase 5 は Context/Grasp 深化〔grasp 三拍リビール・conflict セル解消の 3D
  対応物（領域 recolor→dissolve）・不確実バンド崩壊強化 + 永続ゴースト 3 view の
...
## 実装ノート — Phase 2 残候補: スナップ係合フラッシュ (2026-07-12)

**最後の候補の完了 = プログラム完結**。animation-fx スキル（`.claude/skills/animation-fx`）
導入後の初適用面。スキルの品質ゲート（linear 禁止・一斉動作禁止・境界と余韻・
reduced 静的代替）を、本 ADR の Motion Tier 規律の**内側**で満たす形で実装した
（スキルの「静止状態を作らない」ゲートのみ #30 装飾禁止が上書き — アイドル
モーションは Tier A 根拠がある面に限る。スキル既定 < リポジトリ憲法)。

- **Tier 判定**: Tier F（ドラッグ中の係合という*見えない*事実の提示 — #30 一文
  テスト: 「止まったとき、位置ジャンプが *ロック* だったのか・どの点に係合したのかが
  分からなくなる」）。**事実源の弁別**が本候補の核心: 禁止されるのは楽観プレビューの
  *結果*（未確定ポーズ）の演出であり、スナップ係合は**既に起きた相互作用事実**
  （ロックはドメインプレビューに現に適用された; その後のキャンセルは係合を
  無かったことにしない）。Governor 契約行に括弧書きでこの弁別を明文化。
- **音量設計（transitions only）**: 発火は 自由→ロック（`engage`、全強度）と
  ロック→別ターゲット（`retarget`、0.55 強度 × 0.65 尺 — ターゲット間ジャンプは
  既に可視なので新ロック点だけが新情報）のみ。同一ロック保持と解放は機械固定の
  無音（解放後の実体は自身がフィードバック）。stack チャネルの key は着地 Z のみ
  量子化 — 同一面を滑っても再発火しない（「この高さに載っている」事実は不変）。
- **純粋層**: 新 `src/view/SnapFeedbackMath.js` — `geometrySnapshot`/`stackSnapshot`
  （握り状態→比較可能スナップショット; malformed → null #11）、`snapTransition`
  （2 スナップショットの diff — RegionGhostMath 系譜）、`snapFlashDescriptor`
  （geometry = `fxSnap` 橙〔ステータスバーの Snap 表示と同一トークン化〕、
  stack = `fxGreen` 緑; 実体バウンズ比例 #27、バウンズ不明 → null）、
  `snapFlashFrame`（リング = easeOutBack オーバーシュート→セトル〔境界と余韻〕、
  エコー = 22% 遅延の第 2 波面〔一斉動作禁止〕、スパーク = 係合点へ**収縮**する
  独自時間軸のクリック感〔overlap〕; reduced = 静的リング保持）。
  **`DURATION.snapFlash = 260` ≤ 300ms をテスト拘束**（高頻度イベント = マイクロ帯）。
- **View/配線**: 新 `src/view/SnapFlash.js`（RippleEffect 系譜・Governor 経由のみ・
  `sceneView.activeCamera` を毎 tick 読むビルボード = ortho 切替に追従・emissive
  不可侵 #4・Math.random 不使用）。`GrabOperationHandler._syncSnapFx()` が
  `apply()` 末尾と quick-drag stack 経路で毎フレーム diff（`_snapFxPrev` は
  controller-local 演出履歴 — grasp hover / `_ghostData` と同規律; gesture 開始・
  confirm・cancel で consume & reset）。1 フレーム最大 1 発火（geometry 優先)。
- **E2E 生死ガード**: controller 層は checkJs 対象外（Yellow Card「excluded layer
  needs liveness guard」）のため、smoke に grab + S stack 係合テストを追加 —
  `apply()`→`_syncSnapFx`→`_spawnSnapFx` のどこかが dangling なら pointermove が
  throw して "Stack: ON" が描画されず fail する。
- **Evidence**: unit 594→616 全 pass（SnapFeedbackMath 22 本）/ typecheck clean /
  `test:contract` 23 pass（契約無改変の機械証明）/ E2E スモーク 6→7 pass
  （reduced 含む）/ 実機 Playwright 目視: stack 係合で緑リング+エコーの
  オーバーシュート→減衰、520ms 後に消滅、reduced は静的リング保持、pageerror 0 /
  `vite build` **+1.37KB gz**（388.34→389.71KB — プログラム累計 ≈+12.0KB、
  予算 ≤+20KB、依存 0）。契約・schema・DSL 版・BFF 無改変。

## 実装ノート — Phase 6 (2026-07-11)

**オンボーディングツアー（デスクトップ — named rule 6 の鋳造 + Widening 3 の初適用）**:

- **純粋層**: 新 `src/view/TourMath.js` — `TOUR_STEPS`（add → select → grab →
  edit → extrude のクエストトレイル; 各 step は committed 事実のみ読む `done`
  述語を持つ: Solid 数 / committed 選択 / `selectionMode` / 直近 CommandStack
  着地 `{label, phase}`）、`startTour`（既充足 step をスキップして最初の未充足
  step で開始）、`nextTourState`（前進のみ — 箱の undo は add クエストを復活
  させない; malformed 事実は**同一参照**を返し前進しない; 未知 step id は null
  へ退行 = **誤ヒントより無ヒント** #11）、描画射影 `tourStepDescriptor` /
  `tourAnchor` / `tourVisible`（カードとアンカーパルスが**単一の可視述語**を
  共有 §1.1）。
- **FSM（§1.4 トリガ成立 — 3 状態以上 + 誤ヒントが体験事故）**: uiStore トップ
  レベル `tour` = `null | {status:'active', step} | {status:'done'}`（判別共用体・
  丸ごと置換・sole writer AppController）。ツアーの進行はユーザに見えるアプリ
  状態であり演出の表示履歴ではない = ADR-062 §2 への意図的 carve-out（本文 §2
  の予告どおり CODE_CONTRACTS 行として明文化）。`STATE_TRANSITIONS.md` §tour。
- **事実源配線**: `AppController._updateTour()` を CommandStack 着地リスナー
  （`_spawnLandingFx` と同一 microtask; 着地は controller-local
  `_tourLastLanding` に捕捉）・`objectAdded`（コマンド無しの import 追加も
  拾う）・`activeChanged`・`setMode()` 末尾から呼ぶ。同一参照なら store 書込み
  スキップ。
- **DOM**: 新 `src/components/Onboarding/TourCard.jsx` — 左下コーナーカード
  （Outliner 180px + InfoBar 26px を**オフセットで回避** #26; トーストは
  bottom-center で非衝突）。クエスト番号 n/5・タイトル・kbd チップ・進捗ドット・
  ✕ Skip。entry は `enterMotion`（step keyed remount; reduced は静止出現）。
  done は緑 `TOUR COMPLETE` バナー。Context/demo/ギャラリーの overlay 中は
  `tourVisible` が**FSM を変異せず**非表示化（閉じると再開）。
- **アンカーパルス**: add クエスト中、Outliner「+ Add」ボタンが `activeGlow`
  で呼吸（Tier A アフォーダンス — 既存 `eaBreatheGlow` 語彙の消費; reduced は
  静的中点グロー）。`tourAnchor(tour)` 経由でカードと同一状態から導出。
- **永続線引き（named rule 6 / Widening 3 初適用）**: done/dismiss フラグのみ
  `localStorage.ee_tour` に永続（表示**設定**）; 進行そのものはどこにも永続
  しない。モバイルは従来の一括ジェスチャオーバーレイを維持（`pointer: coarse`
  分岐 — ツアーは fine pointer のみ seed）。
- **ADR 記載からの逸脱**: なし（記載どおり `TourMath.js` + `Onboarding/` +
  uiStore FSM スライス + STATE_TRANSITIONS 更新のみ; 新ドメインイベント 0・
  依存 0・契約無改変）。ヒント対象の「次のアフォーダンス」導出は事実スナップ
  ショットの純関数で、判定の再実装なし。
- **Evidence**: unit 582→598 全 pass（TourMath 16 本 — malformed→null / 前進
  のみ / スキップ / 同一参照 / Move undo 非完了を機械固定）/ typecheck clean /
  `test:contract` 23 pass（契約無改変の機械証明）/ E2E スモーク 6 pass
  （+1: boot→add クエスト→+ Add で grab へ前進→Skip 永続→reload 非再表示;
  既存 boot アサーションは tour 文言との strict-mode 衝突回避で `exact:true`
  化）/ 実機 Playwright 目視: boot カード + 「+ Add」呼吸グロー・add 後
  4/5 Enter Edit Mode・E 押し出しで TOUR COMPLETE 緑バナー + `ee_tour=done`・
  ✕ Close でカード消滅・reduced 静的表示 / `vite build` **+1.83KB gz**
  （386.51→388.34KB — プログラム累計 ≈+10.6KB、予算 ≤+20KB、依存 0）。

## 実装ノート — Phase 5 (2026-07-11)

**Context/Grasp 深化（振付のみ — 事実源・契約・DSL は無改変）**:

- **grasp 三拍リビール（接近 → 閉指 → スコア）**: 純粋 `GraspGhostMath.revealFrame`
  が新設のステージタイムライン（`REVEAL_TIMELINE` = 400/240/260ms、**合計 900ms ≤
  1s の情報遅延上限をテスト拘束** — Consequences §7）。committed select のみ再生し、
  hover プレビューは `elapsed=Infinity` → 最終段即表示（従来挙動保存）。beat 3 で
  グリフがニュートラル灰 `NEUTRAL_GLYPH_COLOR` からスコア色へ flood（純粋 `mixHex`）
  + キャプション出現。verdict 線種（solid/dashed）は**即時** — 遅延するのは演出的
  な色 flood のみで、パネルのスコアバーは常に即時（事実は待たせない）。
  **reduced は最終段へジャンプ**（`revealFrame(_, true)` → 全 beat 1; フェードも
  スナップ = 静的完成キュー #30/#11）。非有限クロックも最終段（#11 — malformed が
  事実を隠さない）。
- **conflict セル解消の 3D 対応物（領域 recolor→dissolve）**: 新純粋
  `src/view/RegionGhostMath.js` — `gapBandRects`（no-man's-land バンド矩形の導出を
  `RegionGhostView._buildIntersection` から抽出 = ライブ overlay と解消演出の
  **単一幾何源** 核 §1.1）、`regionResolveTransitions`（**2 つの committed
  projection の diff** による解消認識 — conflict/proposed + 空交差 → resolved/
  satisfied または交差非空; 変数消失は解消ではない、prev=null は遷移ではない #11;
  判定再実装なし ADR-062）、`resolveFrame`（recolor 35% → dissolve 65%; reduced は
  静的 settled 緑 #30）。新 transient `RegionResolveEffect`（RippleEffect 系譜、
  `MotionGovernor.spawn` 経由のみ、`DURATION.regionResolve = 700ms`）。配線は
  `ContextController._reproject` の ghost 分岐 → `_refreshRegionGhosts()`:
  **ghost モード中の doc 変化（承認の undo/redo・領域編集）で 3D ゴーストを再構築**
  — 従来は DOM Matrix だけ更新され 3D がステールだった潜在バグを同時に解消。
  旧 gap バンドの離脱を transient が語り、**新状態は下で即時描画**（事実遅延ゼロ）。
  `_ghostData` は controller-local 演出履歴（grasp hover と同じ規律 — store/ワイヤ
  禁止）。**副産物の honesty 修正**: Decision で解消しても交差の空事実は残るため、
  `state:'resolved'` の gap バンドは settled 緑 + 「✓ Gap settled by \<decision\>」
  ラベルで描画（従来は解消後も赤 + 「= conflict」のステール主張 #11）。
- **不確実バンド崩壊の演出強化**: `UncertaintyGhostView` の collapse 中、両端の
  extreme ワイヤフレームが**nominal へ収束**しながらフェード（「区間の両端が決定値
  で合意する」を運動が語る）。**reduced-motion 対応を新設**（従来この view は無条件
  アニメ）: idle パルス → 静的バンド、collapse → 即スナップ + 静的シェル保持
  （onSnapped は即発火 = 実 solid の出現を遅らせない）→ COLLAPSE_DURATION 後に
  done。単一境界 `prefersReducedMotion()` を collapse 開始ごとに再サンプル
  （Governor の per-spawn と同じ規律; GraspGhostView も select ごと）。
- **ADR 記載からの逸脱**: なし（記載どおり `GraspGhostMath` へ stage timeline、
  対象 3 view + controller 配線のみ）。grasp リビールの実機目視は上流ソルバ必須の
  ため未実施 — 純粋層テスト + hover と共有の描画経路（`revealFrame` の最終段 =
  hover の従来表示）で担保し、ここに明示する。
- **Evidence**: unit 561→582 全 pass（revealFrame/mixHex 8 本 + RegionGhostMath
  13 本）/ typecheck clean / `test:contract` 23 pass（契約無改変の機械証明）/
  E2E スモーク 5 pass（reduced 含む）/ 実機 Playwright 目視: Regions テンプレ承認 →
  Region Ghosts で settled 緑バンド、ghost モード中 undo → 赤 conflict バンド即時
  復帰（ステール解消）、redo → recolor→dissolve エフェクト → 静的 settled 緑、
  Tutorial 承認 → extremes 収束 collapse → workbench 出現、reduced → 即スナップ +
  静的シェル → クリーン終端 / `vite build` **+1.16KB gz**（385.27→386.43KB —
  プログラム累計 ≈+8.8KB、予算 ≤+20KB、依存 0）。
