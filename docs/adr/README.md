...
| [ADR-110](ADR-110-grasp-is-a-check-beside-its-subject-not-an-object-of-the-floor.md) | **把持探索は場の対象ではなく、主語の隣の検査である — 動詞を失った行の再分類** | Proposed (IA Phase 5.1。ADR-106 で `grasp` 行は場を開かなくなったのに `open-floor` の引数に残っている = ADR-108 D4 と同型の欠陥。`FLOOR_TARGETS` を 3 行にし、選択が無いときの経路は N パネルの空状態が**理由つき disabled** で持つ。理由は最低 2 種 (未選択 / ロボット 0 台 — ADR-090)。**実測により入口の個数は動かない** — 行は入口ではなく引数なので D2 の ratchet は上げない) | 2026-08-04 | ADR-108, ADR-105, ADR-106, ADR-090, ADR-099, ADR-107, ADR-109 |
| [ADR-111](ADR-111-the-outliner-has-a-semantic-side.md) | **Outliner に意味側を与える — 選べるのにどのナビゲータにも載っていない実体を無くす** | Proposed (IA Phase 3.6。ADR-107 が選択の種を広げた帰結として `d_ref` が「選べるのに左に無い」状態になった。幾何 ⇄ 意味の側を置き、意味側の住人は共有変数だけ。行は**窓**であって入口ではない (5 verb のまま)。意味側の 0 は最低 2 種として宣言する。数えるのは行ではなく**住所を持たない種の個数**) | 2026-08-04 | ADR-107, ADR-099, ADR-105, ADR-103, ADR-106, ADR-109 |
| [ADR-112](ADR-112-the-document-intake-address-becomes-permanent.md) | **文書の入口の住所を恒久にする — 満期の過ぎた暫定を、更新ではなく決着で畳む** | Proposed (IA Phase 5.2。ADR-106 D3 の暫定住所は満期を ADR-108 に置いたが、Phase 5 は入口を決めて**器の住所を決めなかった** — 満期は 2026-08-03 に無言で過ぎた。`DocIntakeLayer` を恒久とし、覆う範囲の規則を**面の名前から役割へ** (ADR-111 の意味側は覆えない)。`PROVISIONAL_UNTIL` の 7 箇所の写しを畳む。教訓: 満期は他人の判断に相乗りせず、自分の問いが決着する事象に置く) | 2026-08-04 | ADR-106, ADR-108, ADR-111, ADR-051, ADR-063, ADR-109 |
