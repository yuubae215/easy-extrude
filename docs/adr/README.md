...
| [ADR-109](ADR-109-a-deferral-is-a-declaration-not-a-memory.md) | **残しは宣言であって記憶ではない — 「後でやる」に母集団と満期を与える** | Proposed (IA Phase 6。残しを id / 所在 / **満期条件** / **ticket** / lane を持つ登録簿の行にし、母集団は登録簿ではなく**語彙から導出**して「宣言外の残しの個数」を ratchet で縛る。満期は機械が読み (指す ADR が Accepted になったら fail)、逆向きにも問う (実在しない残しの宣言 = 0 件)。実測 4 件 — 満期切れの `PROVISIONAL_UNTIL` (写し 7 箇所)・ADR-060 の完了済み宣言 3 件・段を持たない項目 2 件・宣言を持たない残し 1 件。原則 #31 の 3 例目で、原則は足さず**検査だけ**を足す) | 2026-08-04 | ADR-102, ADR-103, ADR-100, ADR-106, ADR-107, ADR-108, ADR-060, ADR-098 |
