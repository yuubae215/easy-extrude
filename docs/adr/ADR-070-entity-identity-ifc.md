...
- Status: **Accepted**(2026-07-15 実装済 — 決定2 は推奨どおり **A. 視覚 + 軽い意味** で確定、
  ラベル表示既定は推奨どおり **選択/ホバー時の段階開示**。B フル産業は別 ADR 候補のまま)
- Date: 2026-07-15
...
## Open questions → 決定(2026-07-15)

1. **IFC 深度**: **A(視覚 + 軽い意味)で確定**(ユーザ指示「最新の ADR の意思決定に従って
   実装」= 推奨案の承認)。B(フル産業)は具体像が出た時点で別 ADR(ADR-073 候補)。
2. **ラベル表示の既定**: **選択/ホバー時の段階開示で確定**(常時表示トグルは要望が出たら追加)。

## 実装記録(2026-07-15)

- `src/view/EntityLabel.js` **新規**: CF ラベル機構(div + NDC 射影 + rect キャッシュ +
  0.5px デッドゾーン)を抽出した共有ヘルパ。text/accent/wanted/updatePosition が各 1 owner。
  `Z.sceneLabel` トークン消費。class `ee-entity-label`(スモーク E2E のフック)。
- `CoordinateFrameView`: EntityLabel 消費に置換 + **情報化**: `⌖` グリフ + 名前、選択中は
  RPY 読み出し(`ZYX` = ROS RPY、整数度)を追記。`_syncLabelText()` が唯一の合成者。
- `MeshView` / `ImportedMeshView`: コンストラクタに view context(camera/renderer/container)
  追加。`setLabelText` / `setIfcTint`(基調色 owner — emissive は `_syncEmissive` 専有のまま、
  BODY_COLOR→IFC color lerp 0.65)/ `_syncLabel`(text + accent + wanted の唯一の owner;
  選択 or ホバーで開示)。ラベルアンカーは corners の重心XY+maxZ(表示専用導出 — #24)。
- `SceneService`: `_newMeshView()`/`_newImportedMeshView()`(view context を必ず配線する
  唯一の構築点)+ `_syncIdentityVisuals(obj)`(全創出経路 + `setIfcClass` から呼ぶ)。
- `AppController` アニメーションループ: Solid/Profile/ImportedMesh の
  `updateLabelPosition(activeCamera)` を追加。
- E2E: 選択エンティティのラベル表示 liveness テスト追加(10 pass)。契約・schema・DSL 版・
  BFF 無改変。
