...
## 実装時に変わったこと (2026-07-26)

起票時の設計と実装が食い違った点を、俯瞰を書き換えずに残す (判断の履歴を消さない —
原則 #19)。

1. **`placement` は保存フィールドにしなかった — 種の関数にした。** 起票時は「実体が
   状態を 2 つ増やす」と書いたが、方針は生成時に種で決まり遷移しないので、保存すると
   種と方針が二箇所に書ける (§1.1)。`placementOf(entity)` が毎回 `instanceof` から
   引く。増えた保存状態は `belowGradeIntent` の 1 つだけ。
2. **入口は 3 つではなく 5 つあった。** 起票時の力学表は Grab / クイックドラッグ /
   `_updateMountedAnnotations` を数えていたが、個数検査が 2 つ落とした:
   - **N パネルの Location 数値入力** (`onLocationChange`) が `obj.move()` を直接
     呼んでいた — §Options で「8 箇所目で同じ苦情が出る」と予言した経路が既に在った。
   - **タッチ再グラブ** (`pointerdown` / `S_GRAB_ACTIVE`) が **4 つ目のドラッグ平面
     実装**で、実体種の分岐がまったく無かった。指を置き直した瞬間に、mounts した
     注釈もカメラ正対面へ落ちていた。

   どちらも読んで見つけたのではなく検査が落とした。「列挙して個数を問う」形を
   選んだ理由の実証になっている (原則 #31)。
3. **`belowGradeIntent` の宣言は、フィールドを足すだけでは残らなかった。** ジェスチャ
   終端の `s.stackMode = true` (ADR-071 の毎ジェスチャ既定復帰) が、次のグラブで宣言を
   無効化し、補助が宣言された基礎をピットから持ち上げていた。`start()` が宣言を読んで
   Free で始まるようにして塞いだ。**状態を永続させるとは、その状態を読む場所をすべて
   直すこと**であって、フィールドを足すことではない。
4. **`grounded` は「床へ吸い付く」ではなく「床を割らない」にした。** 上向きの clamp
   だけを行い、屋根への着地は stack assist (ADR-071 の残った価値) が担う。床は方針、
   「他の実体の上に載せる」は補助、という役割分担がここで確定した。
5. **受け入れた挙動の変化:** `grounded` 実体の自由ドラッグ平面が world XY になったため、
   **自由ドラッグで Solid を空中へ持ち上げられなくなった** (上下は G→Z へ)。G2 が
   要求した変更そのものだが、「持ち上げられなくて不便」側に出る可能性は残る —
   反証は当事者の実利用のみ (GSN `EaseOfUseIsNotClosedByTests`)。
6. **書かなかった検査と、その理由:** 「注釈 3 種を `instanceof` で並べる分岐」を
   分類の再導出として落とす静的規則を書いて外した。実測で 8 箇所に当たり、その全部が
   ヒットテスト・N パネルの欄・ツールバーの有効判定という**配置と無関係な正当な分岐**
   だった。無視される規則は規則が無いより悪い (希釈 — 原則 #20)。分類の複製が害に
   なるのは pose を書くときだけで、それは入口の規則が既に塞いでいる。
