...
- **受け入れるコスト / 否定的:**
  ...
  - reveal-on-select は「選択しただけで画面が変わる」ので、多数選択時に
    ...
    する (`showFrameChain` の既存の強度規則を再利用)。**この緩和策は初回実装では
    CF 分岐にしか入っていなかった — 2026-07-29 に測って回収した (下記 §溢れの上界)。**
...
## 溢れの上界 — 仮説 `RevealOnSelectMayFloodTheView` の回収 (2026-07-29 実測)

上の「受け入れるコスト」に挙げた緩和策 (「contextual 軸の DIMMED を使い、選択本体
だけ FULL にする」) は、初回実装では **CF 分岐にしか入っていなかった**。geometry
分岐は子孫フレームを `CONTEXTUAL.FULL` で主張しており、これは 1 個選択のときの
ADR-087 の振る舞いをそのまま N へ持ち込んだ形である。**N を振らずに書いた行は、
N を振ったときの意味を誰も問うていない。**

主張の総量と強度の分布を N の関数として測った (3 つの選択の形):

| 形 | 到達 (主張される id の数) | FULL (修正前) | FULL (修正後) |
|----|---|---|---|
| A: 1 本の TF 木 (13) の中から N 個の CF | **13 — N に不変** | N | N |
| C: 木をまたいで N 個の CF (各木 2) | 2N | N | N |
| B: N 個の Solid を矩形選択 (各 3 フレーム) | 4N | **4N** (N=50 → 200) | **N** |

読み取れたこと:

- 仮説は **量ではなく強度の問題として当たっていた**。到達する id の数は形 A が
  示すとおり N に比例しない (主張は集合の**和**なので、同じ木の中で N 個選んでも
  総量は木の大きさで止まる)。溢れていたのは形 B の**フル強度の個数**だけである。
- 原因は「強度規則が分岐ごとに違った」こと。CF 分岐は最初から「当人 FULL・連鎖
  DIMMED」だったのに、geometry 分岐は連鎖まで FULL だった。同じ関係 (選ばれた
  ものと、そこから連なるもの) に 2 つの規則があった。

**回収 — 規則を 1 つにする。** `_claimContext` の両分岐を「**選ばれた当人は FULL、
そこから連なるものは DIMMED**」に揃えた。結果、不変条件が式で書ける:

```
FULL(claim)   === 選択集合          (ちょうど — 超えない)
DIMMED(claim) === ⋃ chains − 選択集合
```

つまり **フル強度で光るものの個数は選択の基数で抑えられる**。これが「N で溢れない」の
意味であって、起票時に予備として持っていた「連鎖は主張しない」へ狭める必要は
なかった: 到達 (どの id が現れるか) は表の中列どおり**不変**なので ADR-087 の
振る舞い (Solid を選ぶとそのフレームが出る) は保たれ、狭めたのは強度だけである。
連鎖を落とせば G2 を削るが、薄めるだけなら何も失わない。

1 個だけ選んだときにフル強度の応答が消えるわけでもない — 応答は選ばれた当人
(Solid ならそのメッシュ) であり、周りのフレームは最初から「それの文脈」であって
「これを選んだ」の手応えではない。CF 分岐がずっと DIMMED と呼んでいたものと同じ
関係である。

- **問い所 (Q3):** `src/controller/SelectionManager.test.js` の 3 本 —
  「フル強度で現れるのは選択集合ちょうど」「基数 N を振っても FULL の個数は選択
  基数のまま」「強度規則は分岐に依存しない」。**修正を戻すと 4 本 (既存 1 + 新規 3)
  が落ちることを確認済み。** N=25 を回帰に焼いたのは、1 個の fixture では
  4N と N が区別できないため (原則 #31 — 1 と N は別世界)。
- **残る主観:** 「DIMMED の 0.30 が薄すぎないか」「50 個選んだ画面が実際に読めるか」は
  依然として当事者の目でしか閉じない (dogfooding — **未取得**)。測ったのは個数と
  強度の割り当てであって、見え方そのものではない。
...
- ADR-107 (選択できるものが 2 種になる) — 本 ADR の「入口は 1 つ」を保ったまま、選択集合の**要素の種**を 1 から 2 へ広げる。`_ids: Set` は種を持たないので、種の分裂は実装を読んでも見えない (本 ADR が窓について見つけたのと同じ盲点の一段上)
