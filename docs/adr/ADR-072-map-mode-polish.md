...
## Addendum — タッチ ピンチズーム(2026-07-16・ユーザフィードバック)

ユーザ報告:「マップモードってズームできないよ」。調査(headless Chromium で
`__easyExtrude.mapState().frustumSize` を観測)の結果、**デスクトップのホイールズームは
機能している**(frustum 9.0 → 2.9 → 27.6)が、**タッチのピンチでは frustum が全く変化
しない**ことを確認 = ズーム不能はタッチ限定。根因: タッチにホイールは無く、ピンチを
担う OrbitControls は Map Mode(正射影)で無効化されているのに、Map Mode 側がピンチを
実装していなかった(= ADR-072 決定 1 でカメラを完全に引き取った overlay が、引き取った
ナビ機能のうち zoom をタッチで再提供し忘れていた)。

`MapModeController` に二本指ピンチズームを実装。`state.touchPoints`(pointerId→画面座標)
と `pinch`/`pinchLock` を追加:2 本目の指が着いた瞬間に指間距離と現 frustum を基準化し、
以後の距離比で `setOrthoZoom`(指を広げる→距離増→frustum 減=ズームイン; clamp
[2,500]・ホイールと同じ中心固定)。`pinchLock` は 2 本目着地から**全指が離れるまで**
単指の pan/draw を抑止(ピンチ後に残った 1 本が飛ばないため)。AppController の配線が
肝:①単指ドラッグ中の 2 本目タッチを捨てる guard(`_activeDragPointerId` 判定)を、
Map Mode 稼働時は `onExtraTouchDown` へ迂回;②`_onPointerMove` の「ドラッグ開始ポインタ
以外は無視」guard を、Map Mode では非ドラッグ指(= ピンチを駆動する 2 本目)を
map ハンドラへ通すよう例外化。OrbitControls には手を出さない(正射影中は無効のまま)。

検証: unit **653 pass** / `tsc` clean / `vite build` clean / E2E **12 pass**(新規
「map mode two-finger pinch zooms the ortho camera」= 合成 PointerEvent 二本指スプレッド
→ `mapState().frustumSize` の縮小を assert; Playwright にネイティブ pinch は無く
controller は checkJs 除外ゆえ合成イベントが唯一の liveness ガード)。単指タッチの
配置・pan がリグレッションしないことも touch context で確認。契約・schema・DSL 版・BFF
無改変。数値指定配置は引き続き別 ADR 候補として据え置き。
