...
- Status: Accepted (全フェーズ実装済 — Phase 1+2+3 2026-07-05, Phase 4 測定器 + Phase 5 テンプレート面 2026-07-05)
- Date: 2026-07-05
...
## 実装（Phase 4 + Phase 5, 2026-07-05）

**Phase 4 — 測定器（受入チェック面 + blocked→pass 遷移フラッシュ + margin 惜しさメーター）**

- 事実の出所: `validateContext().checkResults`（pass/fail/blocked の判定 — ADR-053 §9 の
  述語がベイク済みオペランドを縮退させた結果）と、doc の acceptance 述語が運ぶ
  測定オペランド（`targets[].margin` / `contacts[].clearance`）。`ContextService.projectChecks()`
  が両者を join（provenance Gap join と同型のサービス糊 — 判定側 validator・オペランド側 doc
  の両方を持つ唯一の場所）。`ContextController` が negotiate 開始 + `_reproject` の一経路で
  `context.checks` へ射影（PHILOSOPHY #5 — 質問回答でブロック解除された検査は同じ経路で
  blocked→pass に着地する）。
- 純粋導出 `src/view/CheckFeedbackMath.js`（THREE-free・14 テスト）: `checkStatusKeys`
  （ref:status の状態つきスナップショット — refs 署名だけでは status 反転を見落とすため）、
  `checkTransitions`（両側に存在する ref の status 遷移のみ; 片側出現は遷移ではない）、
  `unsettledCount`、`checkMeter`（robot_reach = 最悪 margin vs `marginMin` / collision_free =
  最悪 clearance vs `clearance`(既定0); `closeness` は **ADR-061 の同一曲線**
  `nearMissCloseness` を import — 「惜しさ」の曲線はファネルとチェック面で一本）。
  比較不能・オペランド不在は `null` degrade（blocked 中は未評価なのでメーター非表示 — #11）。
- `ChecksPanel.jsx` = ContextLayer negotiate の `'checks'` タブ（acceptance を宣言する doc
  のみ表示・バッジ = 非 pass 数）: 判定バッジ + violation 逐語表示 + blockedBy の Questions
  誘導 + メーター（生数値併記 — ADR-061 の規律）+ 未解決差分チップ + pass 遷移の緑着地
  フラッシュ（`usePrevOnChange`、履歴は component-local — §2）。
- 例 doc `examples/cell_robotics_context.json` + テンプレート **Robot Cell — Robotics Checks**:
  pass（到達余裕 31 vs 25）/ fail（クリアランス 22 vs 25 = 惜しい）/ blocked
  （`f_gripper.attrs.stroke` unknown — Questions タブで回答すると blocked→pass フラッシュ）
  の三態を初期状態で実演。ADR-053 の測定ループ（ベイク済オペランド→述語→判定→演出）が
  UI 内で閉じる。測定器の**インボーク UX**（chain/links を実シーンから引く pending/computing
  UI）は ADR-053 §11 の据え置きのまま — 本フェーズは判定事実の表示と演出のみ。

**Phase 5 — TemplateGallery の CanonicalForm 由来構造プレビュー**

- 事実の出所: `canonicalForm(doc)`（ADR-056 の版付き正規形 — 決定的 core が決めた構造）。
  純粋導出 `src/view/TemplatePreviewMath.js`（4 テスト）: `structurePreview(cf)` =
  5W1H 層別ノード数（Why→How→What 順・分率）+ `docSignature` 先頭 8 桁 + root 数。
  署名の再計算・doc 比較・類似ランキングは一切しない（ADR-056 §3 境界）; malformed → `null`。
- `ContextController.openTemplateGallery()` が bundled doc 群から一度だけ導出しメモ化
  （`templateGalleryPreviews`、file キー）; 導出失敗カードはプレビュー無し（#11）。
  `TemplateGallery` カードに Why/How/What 積み上げバー + 層カウント + `⌗署名` を表示
  （層カラーは WhyTreeView と同一）。

**検証**: `CheckFeedbackMath.test.js` 14 件 + `TemplatePreviewMath.test.js` 4 件 +
`ContextService.test.js` +2 件（projectChecks join / 空 degrade）、`test:context` **421/421**、
`tsc --noEmit`・`vite build` クリーン。契約 / BFF / Context DSL 版は無改変
（例 doc は既存 `context/0.4` の範囲内; 受け入れ基準どおり演出フィールドの契約混入なし）。
