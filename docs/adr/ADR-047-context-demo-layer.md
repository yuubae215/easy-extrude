...
**Status**: Accepted
**Date**: 2026-06-11
**Related**: ADR-046 (Context DSL), ADR-045 (External Layout API), ADR-040 (Solid Primary Triple), ADR-041 (RippleEffect 前例), ADR-037 (Origin CF), ADR-050 (Context-First 本番機能化 — demo→production 分離), ADR-051 (要件入力 — デモ挙動の透明化), ADR-053 (ロボティクス KPI メソッド — ゴースト系譜を RobotPoseGhost/CollisionHighlightView へ拡張), **ADR-106** (Inspector の**住所**を本番の場と揃える — 教える画面と使う画面が違うならチュートリアルではない)
