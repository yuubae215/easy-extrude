...
#### 実機で確かめたこと、および**確かめられなかったこと**

Playwright で実アプリを起動し、`Add ▸ Robot` した骨格の world 座標を
`cameraState()` / `viewState().worldHeight` から画面 px へ射影して直接クリックした:

```
z+50  -> robot_base      z+250 -> robot_base
z+150 -> robot_base      z+350 -> nothing (骨格の外)
```

**腕をクリックすると腕が選ばれることは、実機で確認した。**

ただし**同じ probe を ADR-140 以前のコード (origin/main の `HitTestService.js` /
`AppController.js`) に当てると、同じ 3 点がすべて `robot_base` を返す。**
この起動シーンでは腕の後ろに Solid が無く、**欠陥を踏む構図になっていない**からで
ある。つまりこの実機確認が示したのは「腕が選べること」と「退行していないこと」で
あって、**修正そのものではない**。修正 (奥に Solid が在るときに腕が勝つ) を示して
いるのは今日のところ unit test だけである。

同梱セル (ペデスタルの上に腕が立つ構図 = 報告された症状そのもの) をブラウザで
読み込むには Negotiate から layout を投影する追加の UI 手順が要り、そこまでは
自動化していない。**「実機で確認した」と書けるのは上の 2 つだけ**なので、
そう書く (通ったことにしない — 核 §2)。
