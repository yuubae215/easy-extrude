...
- Status: Accepted (**2026-09-26 実装** — 決定は当事者合意 2026-09-26: 手の寸法の源を tcp 実体に / 3D の確認は当たりを色で言わない / 導出の把持幅は今回直さない / 深さは mm / contractVersion 6 → 7 / `faces` は自動移行して退役。`Retires:` の 2 番地は実装で消えた。実装で足した判断は下の「実装メモ」節)
...
## 残し (登録簿: `docs/DEFERRAL_LEDGER.md`)

- **未実装** — 導出 (仕様なし・`anywhere`) の把持幅を対象 box から測る修正 (**DEF-046**)。
  既存の答えが動くので当事者合意で別判断。
- **未実装** — 3D で面をクリックして進入面を入力する経路 (**DEF-047**)。選択の所有者
  (ADR-099) を面単位へ広げる判断。
- **未実装** — 仕様ごとの段別棄却内訳 (**DEF-048**)。response の閉層が太るので版上げを伴う。

## 実装メモ (2026-09-26 — 実装で足した判断)

本文の決定は変えていない。実装で初めて問われた点だけを書く。

- **手はパネルのフォームではなく tcp から読む。** D3 の「グリッパのパネルはこの 1 つを
  読み書きし」を字義どおりに実装した結果、ゲートの値 (`maxOpening` 等) も tcp の `hand` に
  移り、パネルの Grasped カードは tcp の編集器になった。ゲートの on/off スイッチは
  **宣言の有無そのもの** (未宣言 = gripper を送らない) に置き換えた — 手を宣言しながら
  ゲートを切れると、描かれた形が判定されない状態が作れてしまう (D3「判定されない形を
  描かない」)。DEF-033 の「ハンドはどの実体に属するか」はこれで決着 (カメラと重みは残る)。
- **文書が知る tcp の手は文書へ宣言する。** 実機で、シーンだけに書いた手が次の文書編集
  (把持仕様の保存) の再コンパイルで消えるのを観測した。ADR-129 の pose と同じ分岐
  (`ContextController.declareTcpHand` / `DocBuilder.setEntityHand`、文書が知らない tcp は
  `SceneService.setTcpHand` + `SetTcpHandCommand`)。
- **障害物は球として送られていた。** `GraspController` が ADR-133 以後も障害物を
  `{center, radius: mmToM(o.radius)}` へ写しており、箱の `radius` は undefined → `NaN` →
  JSON の `null` だった。D5 の手の干渉は障害物の形に依るので、ワイヤ整形を
  `domain/graspWire.js` の 1 箇所にまとめ、未知の形は throw にした。
- **取付けと形の矛盾は `core/` でも 400 にする。** フロントのゲートが一次の停止点だが、
  契約の送り手は他にも居うる。`DeclarationError` (ValueError の部分型) を新設し、
  エンドポイントが `invalid_declaration` で理由つき 400 に写す — 想定外 (500) と
  送り手が直せる誤りを型で分ける。
- **許容傾きで刻みが 1 つも残らないときは正対 (0) だけを使う。** 0 はどの許容傾きの
  内側にも在り、空にすると「仕様は在るのに候補 0」という宣言の誤りと見分けのつかない
  答えになる。
- **プリグラスプは面の外に置く。** 深さぶん奥の TCP から後退させると、浅い
  プリグラスプ距離では進入の始点が物体の中に入る。
- **面の語は英語 (`front` / `left` / `top` / `tilted`)。** D2 の例示は `上` / `前` だが、
  パネルの他の文言が英語なので揃えた。語の意味 (ROS: +X 前 / +Y 左 / +Z 上、30° 超は
  斜め) は本文どおり。
