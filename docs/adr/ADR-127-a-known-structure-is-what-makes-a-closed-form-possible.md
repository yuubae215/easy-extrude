...
- Status: Accepted (**`core/` 側実装済み 2026-08-14** — 運動学・解析解・ソルバ・契約宣言・配線。フロントが宣言を送る配線は DEF-030)
- Date: 2026-08-14
...
### 残し — 引き受けなかったもの (DEF-030)

- **フロントがまだ宣言を送っていない — 未着手。** 送るまで実挙動は変わらない。
  D3 の設計上これは安全 (既定のまま) だが、**この ADR の価値は届いていない**。
- **同じ 6 数が 2 箇所に在る。** URDF (フロントの描画) と、フロントが送ることになる
  宣言。ADR-088 は URDF を TCP seed の正本にしており、この ADR はその数を**ワイヤへ
  写す**ので、写しが古くなる余地が生まれる。引き受けたうえで、一致を検査に落とすのが
  DEF-030 の一部 (§1.1 — 冗長を隠さず、検査つきの冗長にする)。
- **ベースの姿勢 (回転) は契約に無い。** 現状 `robot.base` は位置だけ。傾けて据え付ける
  要件が出たら契約に足す判断が要る。無いものを既定で埋めていない。
- **D6 の規約がフロントの描画と一致しているかは未検証。** 自己整合な往復
  (候補 → 目標 → IK → FK → 目標) は通るが、**自己整合な誤った規約も往復は通る**。
  照合先はフロントの骨格描画で、DEF-030 に含む。
