...
## Registered Window Handlers Must Stay Defined

- **Principle**: `_bindEvents()` registers `this._handlers.X = e => this._onX(e)` on `window`. The arrow wrapper survives any refactor, so a deleted `_onX` method does not fail at startup — it throws `TypeError: this._onX is not a function` once per event, and window listeners swallow that silently. The feature simply stops working (PHILOSOPHY #11).
...
- **Concrete Rule**: when extracting logic out of AppController, keep a thin `_onX` dispatcher on AppController and move only the body into the handler class. `_onWheel` dispatch order: (1) `RotationHandler.cycleStepSize` when `S_ROTATE_ACTIVE && _ctrlHeld`, (2) `GrabOperationHandler.cycleGridSize` when `S_GRAB_ACTIVE && _ctrlHeld`, (3) fall through to OrbitControls wheel zoom — in BOTH projections and while a place tool is armed (ADR-103 removed the ortho-zoom branch: the wheel belongs to OrbitControls again). The step constants live on the handlers (`RotationHandler.ANGLE_STEPS`, `GrabOperationHandler.GRID_SIZES`).
...
- **Concrete Rules**:
  - `AppController._onPointerDown/Move/Up` delegate to `_demoCtrl.onAuthorPointerDown/Move/Up(e)` **after** `_hitTest.updateMouse(e)` (so `_mouse` is current — touch has no preceding move) and **before** the op-state / selection branches. Each `onAuthor*` returns `true` **only** when it consumed the event (a handle was hit / a drag is in progress); a miss returns `false` so OrbitControls still orbits.
