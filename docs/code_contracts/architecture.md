...
- **Concrete Rule — a declared axis ANNOUNCES itself (ADR-132)**: `declareExplicitVisible()` emits `explicitVisibilityChanged`, and the Outliner row follows that event. Passing the value at `addObject` time is **not enough**, because the row is seeded when `objectAdded` fires and some entities are not yet what they will be: `addRobot()` creates the base frame (emitting `objectAdded`) *before* stamping `robotRole`, so at seeding time it classifies as an ordinary CoordinateFrame and takes that kind's `false`. The axis was corrected immediately afterwards and the mesh followed — but the row kept the stale snapshot and read "Show" over a drawn arm. **Do not fix this class of bug by reordering the caller**: a rule that says "write the axis before the entity is announced" is a convention, and a convention held by N call sites is broken by one (原則 #23). The obligation belongs to the write (原則 #18 / #32). Regressed by the ADR-096 e2e (`the row never lies`), which now asks the *add* path as well as boot — the add path had no check at all, which is why this survived from ADR-096 until the boot seed was removed and it became the only path.
...
## Tour FSM Is App State in the uiStore; Its Persistence Is a Setting, Never History (ADR-065 Phase 6)

The desktop onboarding tour (`uiStore.tour`, quest card + anchor pulse) is the one deliberate uiStore carve-out from ADR-062's "no presentation state in the store": the open quest is user-visible APP state (a discriminated union like `context.grasp`/`context.wizard`, replaced wholesale), not presentation history. Contracts:

- **Pure transitions, sole writer.** All step eligibility lives in `src/view/TourMath.js` (`TOUR_STEPS` / `startTour` / `nextTourState` — `node --test` lane). `AppController` is the sole writer via `_updateTour()`, fed by the fact sources (CommandStack landing, `objectAdded`, `activeChanged`, `setMode`); `TourCard` and the Outliner "+ Add" anchor pulse only read. A no-change transition returns the SAME state reference so the controller can skip the store write.
- **A wrong hint is worse than none (#11).** Malformed facts hold the current state (never advance on garbage); a corrupt state (unknown step id) degrades to `null`; steps the facts already satisfy are skipped, and progress never regresses (undoing the added box does not resurrect the add quest — the affordance was demonstrated).
