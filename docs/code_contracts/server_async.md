...
- **Principle**: The I/O contract between the BFF and the grasp-search core API is owned by the neutral JSON Schema package `@easy-extrude/grasp-contract` (in-repo at `packages/grasp-contract` since ADR-082, consumed as a `workspace:*` dependency). The BFF **derives** types and runtime validators from the schema; it never **defines or extends** the contract. Constraint solving (IK / collision / reach / ranking) is the backend layer's (`core/`) responsibility, never the BFF's. To change the contract, edit the schema together with a `contractVersion` bump in the same PR (CI contract-wall enforces the pairing).
- **Concrete Rule**:
...
`@easy-extrude/grasp-contract` (in-repo `packages/grasp-contract`, `workspace:*` — ADR-082) is the contract's source of truth; the BFF derives types/validators (`server/src/grasp/contract.js`, generated `.d.ts`) and never defines/extends it. `POST /api/grasp/search` validates + version-checks both ends (inbound mismatch → 400, upstream mismatch/non-conforming → 502, unreachable → 503) and delegates solving to the grasp-search core API. `pnpm test:contract` detects code-vs-contract and instance-vs-contract drift.
