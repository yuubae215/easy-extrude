...
Detail file for `docs/CODE_CONTRACTS.md` Section 4.
...
## Ground Markers Must Not Straddle Z=0; Layering Over Region Fills Uses the Transparent Queue

- **Principle**: A 3D marker standing on the ground plane interacts with two rendering traps. (1) **Straddling**: a z-centered body is half-buried; at glancing camera angles its top face, side wall, and any z=0 overlay parts visually separate into misregistered shapes ("two overlapping discs"). (2) **Queue + polygonOffset**: Three.js draws all opaque meshes before all transparent ones, and `polygonOffsetFactor` scales with the polygon's *screen-space depth slope* — for a near-edge-on ground fill the offset can reach tens of mm, pulling a region fill (transparent, drawn later) in front of an opaque marker standing many mm above the plane. The fill's color then blends OVER the marker (green Zone × purple Anchor → blue-grey ghost).
