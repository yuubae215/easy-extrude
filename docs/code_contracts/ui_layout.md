...
- **Concrete Rules** (desktop):
  - **Left edge**: the Outliner (180px, z:90, opaque, always visible) owns `left:0`. `LinkNetworkView` sits beside it at `left:188px` (`setMobile(false)`); on mobile the Outliner is a drawer, so `left:8px` is free.
...
- **Bug history (2026-06-12)**: the Link Network panel rendered fully behind the Outliner (z:50 < 90, same `left:8px`); the N panel and world gizmo rendered behind the Context DSL demo inspector. Both were silent overlaps — no error, the element simply "disappeared".
