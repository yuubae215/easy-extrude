...
| `onOpenGraspPanel` | Header **Context ▾ → Grasp Search…** / ⋯ menu | **(ADR-057)** `openGraspPanel()` — seed `context.grasp` with the loaded layout summary and `setGraspPanelOpen(true)`; warns and does not open when there is no renderable layout |
| `onRunGraspSearch({weights, topN})` | GraspSearchPanel **Run** button | **(ADR-057)** `runGraspSearch(params)` — read `getCompiled().layoutDsl`, ensure a JWT'd `BffClient`, Step A `compileLayout` (round-trip verify) then Step B `graspSearch` (BFF stamps `contractVersion` + delegates to the external service); push status/candidates into `context.grasp`. A query, not a doc mutation — not on the CommandStack. 400/502/503 surface their reason as a toast + the error envelope |
| `onCloseGraspPanel` | GraspSearchPanel ✕ / backdrop | **(ADR-057)** `closeGraspPanel()` — `setGraspPanelOpen(false)` |
