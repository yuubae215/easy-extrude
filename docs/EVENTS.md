...
| `onRunGraspSearch({weights, topN, camera?, gripper?})` | GraspSearchPanel (grasp tab) **Run** button | **(ADR-057/081)** `GraspController.runGraspSearch(params)` — read `getCompiled().layoutDsl`, ensure a JWT'd `BffClient`, then the linear FSM `compiling` (Step A `compileLayout` round-trip verify) → `solving` (Step B `graspSearch`, BFF stamps `contractVersion` + delegates) → `results` \| `error{stage}`. `camera`/`gripper` are the domain-card declarations (ADR-081 Decision 5), gap-checked by `GraspDeclarationCatalog` predicates before Run enables and passed into the request's open payload **verbatim** (declaration only — solving stays in `core/`); a disabled card passes `null` and the key is omitted (that gate then passes vacuously). Replaces `context.grasp` wholesale with each discriminated-union state. A query, not a doc mutation — not on the CommandStack. 400/502/503/BFF-down surface their reason as a toast + the error state |
| `onCaptureViewportCamera()` → `{position, viewAxis, fovHalfAngle\|null}\|null` | GraspSearchPanel vision card **📷 use current view** | **(ADR-081 Decision 5)** `GraspController.captureViewportCamera()` — a **synchronous value-returning** callback (unlike the fire-and-forget rows above): snapshots `SceneView.activeCamera` (position / matrixWorld / perspective fov) and derives the wire declaration through the pure `visionFromViewportCamera` (`GraspDeclarationCatalog`). Ortho camera (projection axis, ADR-103) → `fovHalfAngle: null` (the form keeps its previous value); no camera at all → `null`, which the panel reports inline ("viewport camera unavailable") — never a guessed declaration (#11) |
...
| `AddAnnotationCommand` | After the place tool's `_createAnnotation()` created the entity (ADR-072 / ADR-073 / ADR-103) | `detachObject()` + `setVisible(false)` (soft delete) |
