...
graph LR
  ...
  S2 --> S6
  S5P["ADR-131 ✅<br/>再生成は投影の差し替え<br/>(G-5 の前提)"] --> S5
  S6["段 G-6 ✅<br/>ADR-130<br/>探索の主語の所有者<br/>(入口が premise を運ぶ)"]
  S6 --> S7
  S5 --> S7
  S7["段 G-7 ✅<br/>ADR-132<br/>探索の幾何の源<br/>(画面に在るものについて解く)"]
```
...
| ~~**G-5**~~ | ADR-129 | **完了** (Accepted 2026-08-14) — D1/D2/D3 を 1 つの MVP として。実装中に**前提が 1 つ足りない**ことが分かり (再生成が未宣言実体を消す)、ADR-131 を先に閉じてから入れた | **G-2 / G-3** + **ADR-131** | DEF-031 / DEF-032 **決着** · DEF-033 / DEF-034 は開いたまま |
| **G-5 の前提** | ADR-131 | **完了** (Accepted 2026-08-14) — 再生成が未宣言実体を消していた。ADR-129 D1 の実装前に書き込み先の経路を辿って発見。ADR-129 自身が「`ref` を持たない実体は欠落ではなく状態」と書いた前提が、今日は成り立っていなかった | なし | — |
| ~~**G-6**~~ | ADR-130 | **完了** (Accepted 2026-08-14) — 探索の主語の所有者を 1 つに畳んだ。**ユーザーが G-5 の手順を実際に通そうとして見つけた欠陥**で、手順の 5 手目 (掴む対象を選ぶ) が自分のゲートを踏んで偽の阻止文を出していた | **G-2** — 対象の pick 行が在って初めて「選択は主語ではない」が言える | — |
| ~~**G-7**~~ | ADR-132 | **完了** (Accepted 2026-08-14) — 探索の幾何を live scene から取り、宣言だけ文書から `ref` で join する。**これもユーザーが実際に押して見つけた欠陥**で、G-6 と同じ日の同じ手順の続きである: 入口が「文書が無い」を「失うものが無い」と読み替えて `cell_robotics` を読み込み、**ユーザーがモデリングしたシーンを全消し**していた。根は主語の源が 2 つに割れていたこと (ロボット=scene / 対象=文書) で、`_graspTargets` の doc コメントが**自分でこの ADR を予告していた**。ADR-055 の φ⁻¹ (`decompileLayout`) に**初めて消費者が付いた** — 3 か月間、呼び出しは 0 件だった | **G-2** (対象の契約) + **G-5** (宣言が文書に住む — join する相手が居ることが前提) | DEF-035 |

**着手順の推奨 (2026-08-14 5 訂):** G-0 / G-2 / G-3 / G-5 / G-6 / G-7 は完了。次は
**G-1 (ADR-121 重心)** → **G-4 (ADR-122 pickable と歩留まり)**。
...
| **G-5** | ADR-129 (宣言の書き込み経路 — 配置・据付姿勢・領域) | 2026-08-14 |
| **G-6** | ADR-130 (探索の主語の所有者 — 入口が premise を運ぶ) | 2026-08-14 |
| — | ADR-131 (再生成は投影の差し替え — G-5 の前提) | 2026-08-14 |
| **G-7** | ADR-132 (探索の幾何の源 — 画面に在るものについて解く) | 2026-08-14 |
