...
## Commit observation metadata — 未刻印 / 刻印済 / 空洞 / 刻印不能 (ADR-092 → ADR-116)

コミット 1 個が持つ観測トレーラ (`Model-Effort` / `Task-Class`) の有無と中身。
第 3・第 4 の状態は**不在の終端**と**在るのに何も言っていない状態**なので、
どちらも状態に見えない (原則 #31) — 明示的に置く。

**ADR-116 で主経路が変わった。** 刻印はコミット生成の**後** (PostToolUse の amend) から
**前** (`prepare-commit-msg`) へ移り、「刻印不能」へ落ちる辺は主経路から消えた。

```
   PreToolUse ── .git/claude-commit-context を置く (根拠。マーカーの存在 = 帰属)
        │
        ▼
   git commit ──▶ prepare-commit-msg ──▶ ┌──────────┐
   (Claude)       主経路・SHA 不動        │  刻印済   │
        │         連鎖しても刻める         └──────────┘
        │                                      ▲
        │  マーカー無し / 古い / hook 未配線        │ PostToolUse (amend --only)
        ▼                                      │ 予備経路・冪等
   ┌──────────┐ ───────────────────────────────┘
   │  未刻印   │
   └──────────┘ ───▶ ┌────────────┐
        │  予備経路でも  │  刻印不能   │ ← test:trailers が個数で数える
        │  push 済み     └────────────┘   (baseline 14 — 主経路の配線前に出たもの)
        ▼
   ┌──────────┐
   │   空洞    │  トレーラは在るが transcript が読めず unknown/unknown
   └──────────┘  被覆率の分子に入るので刻印済と分けて数える (baseline 0)
```

- **権威**: 主経路 `.githooks/prepare-commit-msg`、予備経路
  `.claude/hooks/commit-trailers.sh` (PostToolUse)。**両方とも導出はしない** —
  規則は `scripts/commit-meta.mjs` の純粋関数 1 箇所を参照する (§1.1)。二重実装
  すると遡及分類と新規分類が静かにズレる。予備経路は冪等 (同じ値なら no-op) なので
  二重刻印にはならない。
- **帰属の判別はマーカーの鮮度ただ 1 つ。** 人が自分の端末で打った commit には
  マーカーが無いので、主経路は何もしない。マーカーは使い切り (刻んだら消す) で、
  鮮度は 300 秒。窓を広げると人のコミットを巻き込み、狭めると刻み漏れが増える —
  どちらの側に倒れても `pnpm test:trailers` が個数で見せる。
- **禁止遷移**: **公開済みコミットの刻印** (予備経路のみ)。`git branch -r --contains
  HEAD` が非空なら何もしない — amend は SHA を変えるので、push 済みを書き換えると
  履歴が分岐する。主経路はコミット生成前なのでこの遷移自体が存在しない。
- **guard**: 主経路は merge / squash のメッセージ源で素通り。予備経路は rebase /
  merge 進行中・detached HEAD・マージコミット・直近 300 秒に新しいコミットが無い
  (= コマンド失敗) のいずれかで素通り。判定のどこかが失敗しても fail-open
  (統治の hook がユーザーの作業を壊さない)。
- **副作用の告知**: 予備経路の刻印は SHA を変えるため、`additionalContext` と
  `systemMessage` で `before → after` を告げる (原則 #11 — 黙って変えない)。
  主経路は SHA を動かさないので告知が要らない。
- **残る欠測** (数える対象): 1 つの Bash 呼び出しで 2 つ以上 commit すると、
  マーカーは使い切りなので 2 本目以降は主経路で刻まれない (予備経路が HEAD だけ
  拾う)。`core.hooksPath` が張られていない clone も同様に予備経路だけになる。
