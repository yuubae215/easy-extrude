...
| ~~**Phase 3**~~ | ~~`SceneModel` を集約ルートに昇格。`SceneService` (ApplicationService) を追加~~ (完了 2026-03-20, ADR-011) |
| **Phase 4 (次)** | `Sketch.extrude()` が `Cuboid` を返し SceneService がエンティティを差し替える。`Sketch` から `move()`/`extrudeFace()` を削除し次元遷移モデルを正しくする (ADR-012 小) |
| **Phase 5** | グラフ基底ジオメトリモデル — Vertex / Edge / Face を明示的に保持。`dimension` フィールド廃止。G→V / G→E / G→F 選択の統一 (ADR-012 本体) |
| **Phase 6** | ドメインイベント (EventEmitter) を使い View が Model を直接購読 |
