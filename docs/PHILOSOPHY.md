...
|---------------------|---------------------------------------------|---------------------|
| A fixed physical relation must be written as a fixed transform, never as two independently re-spanned bases | 2026-09-23 · `core/…/engine/ur_solver.py` / `src/robotics/graspPoseGauge.js` · The candidate frame and the flange target were each built by spanning a basis from their own +Z (`basisFromZ(−a)` vs `basisFromZ(+a)`) and rolling it. A tool bolted to the flange means the two differ by ONE constant rotation — but the two spans disagreed by `2·roll`, so the jaw closing axis spun at twice the roll relative to the flange. Both frames looked plausible alone; ADR-147 even pinned "rotating 180° does not match" as a finding, when it was the symptom. Fixed by deriving the flange from the candidate frame (x kept, y/z flipped, ADR-150 D5). **Asked at:** `CrossLanguageDerivation.test.js` — the fixture must equal the candidate frame under the fixed mount for every pose | — (the check is the test; no prose rule) |
| A tree stores only the edges whose both ends are declared facts — a value composed through edges the tree does not hold is a derivation, never an edge (TF: store `tool0 → tcp`, not `base → tcp`) | 2026-09-23 · `src/view/robotSkeleton.js` (`TCP_LOCAL_SEED`) / `SceneService.addRobot` · The scene's TF tree had no arm-joint edges, so the `robot_base → tcp` edge stood in for six joints composed at the rest pose + the tool — a derived value stored as a fact. It could not follow the previewed arm (the dogfooder saw the label leave the tool tip) and sat 150 mm short once the tool was bolted on. Fixed by storing only the mount `tool0 → tcp` and composing through the injected rest flange pose (ADR-151). Distinct from principle #24 (a derived value fed back as a cycle's input) — here nothing cycles; the derived value simply became a second source | [A Robot's TCP Stores Only the Tool Mount; Its World Pose Is Derived (ADR-151)](code_contracts/architecture.md#a-robots-tcp-stores-only-the-tool-mount-its-world-pose-is-derived-adr-151) |
...
| **原則 #31 の GSN 版 — 「支えが 0 個の主張」は数えられるが、「支えが 1 個あって穴も 1 個ある主張」には欄が無い** | 2026-08-14 · `scripts/check-gsn-debt.mjs` (ADR-126) / `docs/gsn/adr-120-unevaluated-is-not-zero.gsn` · ADR-120 の goal `ScoresAreComparableAcrossRequests` には**スタブレーンの `solution` が 1 つ**吊られており、その隣に「実ソルバではまだ示せていない」を書いた `assumption` (= DEF-013) が並んでいた。G2 が数える母集団は `support-exploring` / `support-unexplored` **ラベルを持つ goal** = 支えが一つも無い主張なので、この goal は**支えられている側**に分類され、宣言済みの穴は個数にも満期にも入らない。実際に DEF-013 を片付けても負債は 30 のまま動かず (実測)、逆に言えば**穴が開いたままでも 30 は動かなかった** — ADR-126 が「宣言済みなら永久に緑」を閉じたつもりの当の場所に、一段細かい「部分的に宣言済みなら永久に緑」が残っている。二人の生産者 (`core/` とスタブ) のうち片方だけが支えている主張は、木の上では「支えられた主張」と見分けがつかない。**昇格の扱い**: 原則 #31 は既に在るのでこれはその適用域の話であり、原則行は増やさない (ADR-092 の希釈判断)。降ろし先の候補は G2 の母集団を「ラベルを持つ goal」から「**満期 trigger を持つ `assumption` を子に持つ goal**」へ広げること — ただし今日の 1 例では *assumption を穴として書く* 記法自体が普及しておらず、母集団が実質 1 件なので ratchet にならない。2 例目 (別の木で、solution の隣に宣言された穴が見落とされた事例) が出たら検査へ降ろす。 | `pnpm test:gsn-debt` G2 (母集団の定義そのものが問い所) |
| **導出を宣言で上書きできるようにする変更は加算ではない — 導出は「利用者が知らないこと」を知っており、それが黙って荷重を支えている** | 2026-08-14 · `src/domain/graspFeature.js` (ADR-128 / ADR-119 D2-D3) · 掴む場所は 100% 導出だった: `FACES_BY_GRIPPER_KIND` (ADR-118) が**ハンドが実際に触る面**を選び、`core/` は平行ジョーの開き幅を**そのサンプル点の広がり**から測る。ADR-119 D2 が「ここを掴め」と言えるようにした瞬間、**1 面だけ宣言する**という完全に自然な操作が幅をほぼ 0 にし、閉じられないハンドをゲートが通す — **ADR-118 が直した当の欠陥が、宣言という正面玄関から復活する**。しかもリクエストは終始正しい形をしており、応答は「候補 0 件」という*正しい形の答え*になる。注目すべきは、導出が守っていた不変条件が**どこにも不変条件として書かれていなかった**こと: 表は「どの面をサンプルするか」に見えて、実は「幅が測れる面の組を保証する」を担っていた。宣言側はその二つ目の役割を知らないまま表を迂回できる。修正は対向面を黙って足すことではなく (それは D3 が禁じる静かな拡大)、**gap として止めて理由を出す**こと。候補となる価値: **導出の置き換えを許す前に、導出が何を保証していたかを名指しできなければならない** — 言えないなら、それを置き換えられるほど理解していない。**降ろし先**: 原則行を増やすとプロンプト希釈と衝突するため (ADR-092 の先例)、原則化せず `docs/code_contracts/architecture.md` の規則 + `graspFeatureGaps()` という**問う場所**にした。2 例目 (別機能で、宣言が導出の黙った不変条件を無効化した事例 — 例えば手動配置がスナップの保証を、手書きの経路が生成器の保証を壊す形) が出たら昇格。 | A Declaration Can Invalidate an Invariant the Derivation Was Holding (ADR-128) |
| **冗長な予備経路は、主経路の死を隠す — 計数は*結果*を数えており*経路*を数えない** | 2026-08-08 · `.claude/hooks/commit-trailers.sh` (ADR-116) · 観測トレーラの書き手を 2 つにした (主経路 = `prepare-commit-msg` / 予備 = PostToolUse の amend)。主経路はマーカーを書く `node -e` がモジュールのパスを argv で渡していたため `isMain` が誤爆し、**一度も動いていなかった**。にもかかわらず予備が全部拾うので ADR-115 の被覆率は 100% のままで、その日に入れたばかりの計数は**緑を出した**。気づけたのは予備経路の副作用の告知 (「SHA が変わりました」) が毎コミット出ていたからで、検査ではない。候補となる価値: 冗長化は可用性を上げるが**可観測性を下げる** — 結果だけを数える検査は、経路が 1 本減ったことを原理的に見ない (原則 #31 の同型: 死んだ経路は検査対象のノードを持たない)。フォールバックを足す変更は、同じコミットで**経路ごとの検査**を足して初めて閉じる。**降ろし先**: 原則を増やすとプロンプト希釈と衝突するため (ADR-092 の先例) 原則化はせず、経路を実際に走らせるテストを作った。2 例目 (別のフォールバック — リトライ・キャッシュ・代替 API など — が主経路の死を隠した事例) が出たら昇格。 | 統治 hook から node のモジュールを使うときはパスを env で渡す (ADR-116) |
