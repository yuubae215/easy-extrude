...
| DEF-043 | `docs/adr/ADR-150-the-tool-is-bolted-to-the-flange-and-the-arm-appears-once.md` · `core/easy_extrude_core/scene/orchestration.py` | ピック検証が「単体ワーク」と「トレー内の全ワーク」に分かれたとき: 満期=PATH:packages/grasp-contract/schema/pick-sequence-response.schema.json 。当事者依頼 (2026-09-23) の 4 点目。`core/` の `/pick-sequence` (ADR-078 — 最上面から取る→除去→再評価) は実装済みだが、JSON Schema 契約・BFF ルート・front は**どれも配線されていない**。合意は「`/pick-sequence` を契約・BFF・UI まで配線する」案で、新しい response 契約 = contractVersion の版上げを伴うので ADR-150 と同じ PR に入れず別 PR にした。取る順序の方針はアイデア (ADR-146 軸 1) なので front に写さない | ADR-150 | app |
| DEF-044 | `docs/adr/ADR-150-the-tool-is-bolted-to-the-flange-and-the-arm-appears-once.md` · `src/view/robotSkeleton.js` | シーンの `tcp` CoordinateFrame がツール先端に seed されたとき: 満期=GREP:src/view/robotSkeleton.js::TOOL_LENGTH_M 。ADR-150 D4 でツール (150mm) はフランジに付き、IK は TCP = ツール先端で解くようになったが、`tcp` フレームの seed (`TCP_LOCAL_SEED`, ADR-088) は**フランジ位置のまま** — 休止姿勢でシーンの TCP マーカーはツール先端の 150mm 手前に立つ。動かすと seed の導出・既存シーンの `tcp`・`tcpOrientation` の意味 (ADR-084 §3) に波及するので、ADR-150 の範囲では触れていない | ADR-150 | app |
