...
| DEF-044 | `docs/adr/ADR-150-the-tool-is-bolted-to-the-flange-and-the-arm-appears-once.md` · `src/view/robotSkeleton.js` | TCP がツールの取付けから導出されるようになったとき (ADR-151 の実装): 満期=GREP:src/view/RobotStage.js::_attachTcpMarker 。ADR-150 D4 でツール (150mm) はフランジに付き IK はツール先端で解くが、シーンの `tcp` フレームの seed (`TCP_LOCAL_SEED`, ADR-088) はフランジ位置のまま。当事者の指摘 (2026-09-23) で、プレビュー中に `tcp` が腕に追従しないことも同じ根 (TCP の源が 3 つ) だと分かり、**ADR-151 (Proposed) が源を 1 つに畳む設計として起票された** — 本行はその実装で消える | ADR-151 | app |
