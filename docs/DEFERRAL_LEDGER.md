...
| DEF-040 | `src/robotics/ApproximateReachPreview.js` · `docs/adr/ADR-146-a-closed-form-may-be-copied-an-idea-may-not.md` | GitHub Pages の腕プレビューの精度が実測で不足と判断されたとき: 満期=GREP:src/robotics/ApproximateReachPreview.js::inverseKinematics 。ADR-146 D6。UR 解析解 IK は D3 の一意性軸に乗る (公知の閉形式 — Hawkins 2013、探索でもアイデアでもない) ので、**ADR-144 が §1.1 を理由に退けた却下案 B は、ADR-146 の下では理由を失っている**。それでも今日移植しないのは、移植が *ADR-144 の近似プレビュー (サンプリング探索・位置のみ・姿勢未一致) を退役させるか*という別の判断を含むからで、置き場所の規則を記録するという ADR-146 の Goal の外にある。**却下の理由が差し替わったことと、結論が維持されることは別の事実**なので両方書く — 理由だけ書き換えて結論を黙って残すと、次に読む人は「まだ §1.1 で禁じられている」と読む | ADR-146 | app |
