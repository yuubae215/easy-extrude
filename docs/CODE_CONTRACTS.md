...
| TC Drag Blocked on Fastened-Source CF | `_updateFastenedFrames()` overwrites `source.rotation` (and `source.translation` when the parent is another CF) every frame — any delta from TC `objectChange` is silently discarded, causing the TC proxy gizmo to drift from the CF. In `dragging-changed` start, detect via `isFastenedSource()`, set `_tcFastenedBlocked = true`, show toast. In `objectChange`, return early if `_tcFastenedBlocked`. In `dragging-changed` end, sync proxy via `_syncMobileTransformProxy()` and return without pushing a command. |
