...
| TC Gizmo Does Not Block Object Selection | No TC hit-guard in `_onPointerDown`. `_hitAnyObject()` tests only domain cuboids — TC gizmo meshes never appear in `result`. TC drag is handled by TC's own pointer listeners independently. Empty-space touch tap (`result === null` + `pointerType === 'touch'`) calls `_clearObjectSelection()` + `_setObjectSelected(false)` for standard deselect behavior. |
