...
| TC Gizmo Hit Guard Before Object Selection | Raycast against `_tc.getHelper()` when TC is attached; **visible** handles always take priority — `if (tcHits.some(h => h.object.visible)) return` unconditionally, even when another entity is also hit (matches Blender/Maya/Unreal). **Invisible** picker meshes (`visible=false`) never block selection (PHILOSOPHY #22b). |
