...
| TC Gizmo Hit Guard Before Object Selection | In `_onPointerDown`, raycast against `_tc.getHelper()` before `_hitAnyObject()` when TC is attached; return early only when a **visible** handle is hit (`tcHits.some(h => h.object.visible)`); `Mesh.raycast()` ignores `visible=false` so invisible pickers block all taps on the object body if `length > 0` is used |
...
| CoordinateFrame Tap Selection | `_hitAnyObject()` never hits a CF (cuboid = null); `_onPointerDown` must call `_hitAnyCoordinateFrame()` **first** (before `_hitAnyObject()`); CFs render on top of parent Solids so cuboid-first would return the Solid, preventing CF long-press context menus and breaking CF→CF "fastened" link on mobile. Only visible groups (showFull/showDimmed) are tested. (PHILOSOPHY #22) |
