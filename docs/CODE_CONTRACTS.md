...
| Semantic Move Guardrail (checkMoveGuardrail) | Before G-key grab (`_startGrab()`) and before mouse quick-drag (`BEGIN_QUICK_DRAG` in `_onPointerDown`), call `SceneService.checkMoveGuardrail(_selectedIds)`. If it returns `{ blocked: true }`, show the message as a `'warn'` toast and return — do not enter the grab/drag state. The guardrail fires when any selected entity has a `fastened` or `mounts` SpatialLink to an entity **not** in the current selection; if the linked peer is also selected (moving the whole assembly), the guardrail does not fire. After the guardrail, **both call sites** must also check `hasFastenedChild(id)` for each selected Solid directly (not via the guardrail): fastened links live on child CF IDs, not the Solid ID — the link-loop inside `checkMoveGuardrail` misses them, and without the block the drag preview and `_updateFixedJointFrames()` fight every frame causing oscillation. This second check is a solver-conflict guard (like `_startRotate` and TC drag), not a semantic-intent guard — the two concerns must stay separate. | Tension Skips Topological Links | `getLinkDragTension()` must skip links whose `jointType === null` — those links carry no kinematic constraint and drag resistance has no physical meaning for them. Applying tension to them wrongly impedes free movement (PHILOSOPHY #11) and creates asymmetric damping (free toward linked entity, up to 85% resistance away) that users perceive as jitter/oscillation. |
