...
| Edge-Anchored Panels Must Coordinate Occupancy | Persistent fixed panels on the same screen edge must offset past each other, never layer. Left: Outliner owns `left:0`; `LinkNetworkView` sits at `left:188px` desktop. Right: NPanel shifts to `right:280px` while the Context Inspector (ADR-047) is open; gizmo offset is owned solely by `AppController._updateGizmoOffset()` (`16 + 200·nPanel + 280·inspector`, single uiStore subscription) — never call `setRightOffset()` from toggle handlers |
