...
| Rotate Pivot Must Use `_position` Directly | `_startRotate()`, `_applyRotate()`, and the mobile re-touch path in `_onPointerDown` must all set / fall back to `obj._position.clone()` — never `getCentroid(obj.corners)`. `getCentroid` averages 8 world-space vertices; far from the origin the FP rounding in those large coordinates is fed back into `_position` each frame, compounding into visible drift (PHILOSOPHY #24 manifestation c). `obj._position` is the ADR-040 authoritative primary triple and carries zero rounding error. |
