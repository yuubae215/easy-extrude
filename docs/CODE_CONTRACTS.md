...
| TC Gizmo Hit Guard Before Object Selection | Run scene hit tests (`_hitAnyCoordinateFrame`, `_hitAnyObject`) **first**; apply TC guard only when `!targetId \|\| targetId === scene.activeId` (tap on active object or empty space). If a different entity is hit, skip the guard entirely — the gizmo has no authority over other scene objects (PHILOSOPHY #22b). Guard blocks only when a **visible** handle is hit (`tcHits.some(h => h.object.visible)`). |
...
| TC Drag Blocked on Fastened-Source CF | `_updateFastenedFrames()` overwrites `source.translation/rotation` every frame — any delta from TC `objectChange` is silently discarded, causing the TC proxy gizmo to drift from the CF. In `dragging-changed` start, detect via `isFastenedSource()`, set `_tcFastenedBlocked = true`, show toast. In `objectChange`, return early if `_tcFastenedBlocked`. In `dragging-changed` end, sync proxy via `_syncMobileTransformProxy()` and return without pushing a command. |
