...
| Preview Pipeline (applyPreviewTranslation / applyPreviewRotation) | Entity-type dispatch for drag-preview mutations belongs in `SceneService.applyPreviewTranslation(segStartCorners, segStartPositions, worldDelta)` and `SceneService.applyPreviewRotation(obj, snap, deltaQ)`. `AppController` computes the delta/quaternion from input events and calls the service — it must not replicate the entity-type branch. After a stack snap, call sites must re-update mesh views for the snapped objects. See `docs/code_contracts/architecture.md` §"Preview Pipeline". |
