...
| TC Gizmo Force-Update After Proxy Repositioning | Call `_tc.getHelper().updateMatrixWorld()` after `tc.attach()` in `_attachMobileTransform()`; call `_syncMobileTransformProxy()` from `_toggleTcMode()` |
| TC Gizmo Does Not Block Object Selection | No TC hit-guard in the `if (result)` branch — domain selection always proceeds. In the empty-space `else` branch, `if (this._tcDragging) return` before deselect/rect-selection: TC's `domElement` listener fires before the `window` listener, so `_tcDragging` is already set when an arrow outside the Solid is tapped. Empty-space touch tap (no TC, no entity) calls `_clearObjectSelection()` + `_setObjectSelected(false)` for standard deselect behavior. |
