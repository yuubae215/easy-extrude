...
**設計判断は ADR-109** (Proposed)。要点: 上の 3 段はどれも「前の段が正しく宣言した残し」
...
- [ ] `docs/DEFERRAL_LEDGER.md` — 行は id / 所在 / **満期条件** / **ticket** / lane。
      どの欄も空にできない (`—` の ticket は禁止 — 段の無い項目は誰も実装しない)。
- [ ] `scripts/check-deferrals.mjs` の 4 つの問い —
      (1) 宣言外の残しの個数 ≤ baseline (**超えても下回っても** fail)、
      (2) 満期の過ぎた `PROVISIONAL_UNTIL` **0 件**、
      (3) ticket 欄が空 / 実在しない行 **0 件**、
      (4) 実在しない残しを指す行 **0 件** (逆向き)。
- [ ] 母集団は**語彙から導出**する (登録簿を分母にしない — それが `place-list`)。
- [ ] `package.json` に `test:deferrals`、CI の gate ジョブへ。

**完了条件:** 「残しはもう無いか?」が記憶ではなく `pnpm test:deferrals` の返り値で
答えられる。ADR-060 の 5 項目 (3 完了 / 1 消滅 / 1 残存) が逆向き検査の最初の実データとして
落ちる。**限界も宣言する** — 語彙による導出は「宣言する気のある残し」にしか効かない。
