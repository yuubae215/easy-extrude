...
猶予は次の PR までしかない。**Phase 3 は出た。この辺は今まさに張っている** — 場と
N パネルは同時に見えるようになったので、「場で `d_ref` を選んでいる間、右パネルは誰の
ものか」は毎回起きる状態にある。踏んだ人が `selectedVariable` の第二の選択状態を書く前に
Phase 3.5 を出すこと。Phase 3.5 は Phase 4 / Phase 5 のどちらの前提でもないので、
