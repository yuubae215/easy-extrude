...
- **設計判断** → **ADR-103** (Map の再分類 — Phase 1、実装済み) /
  ...
  **ADR-111** (Outliner に意味側 — Phase 3.6、**Accepted・実装済み**) /
  **ADR-108** (入口は動詞であって対象ではない — Phase 5、**実装済み**) /
  **ADR-110** (把持は主語の隣の検査 — Phase 5.1、**Accepted・実装済み**) /
  **ADR-112** (文書の入口の住所を恒久に — Phase 5.2、**Accepted・実装済み**) /
...
**設計判断は ADR-111** (Accepted・実装済み 2026-08-05)。要点: Phase 3.5 が選択の種を 2 つにした結果、
...
- [x] 左のナビゲータに幾何 ⇄ 意味の側。既定は幾何 (初期頻度 — v8 注釈②)。
      幾何側のラベルは `Scene Collection` のまま — 側を足したことを理由に既存の
      概念を改名すると、同じものに 2 つ目の名前が生まれる (§1.1)。
- [x] 意味側の住人は**共有変数だけ**から始める。actors / requirements は*選択できない*ので
      載せない (載せると「載っている ⊃ 選べる」が逆向きに崩れる)。
- [x] 行のクリックは**窓**であって入口ではない — 既存の `onSelectVariable` を通るだけで、
      ADR-099 の 5 verb は 5 のまま。
- [x] 意味側の 0 を**最低 2 種**として宣言する (文書が無い / 文書に変数が 0 個)。
      1 つの「空」で両方を賄わない。`SEMANTIC_SIDE_KIND` + 宣言表で、未宣言の種は throw。
- [x] `outlinerSide` を `STATE_LEDGER` へ。2 値・基数 1 なので状態機械は起こさない
      (閾値未満でも記録する)。ADR が Accepted になったので行は本表へ**移した**。

**完了条件: 達成。** 住所を持たない選択可能種 **0 個** (母集団は `SELECTABLE_KINDS` の
枝から導出)、`SelectionOwnership.test.js` は**作り直さずに**通った、場を閉じたまま
意味側から変数が選べる (e2e)、2 種の 0 が別の文言を出す (unit)。

実装で分かったこと 2 点 (ADR-111 §実装で変わったこと): 「文書を採ったか」の導出が
ヘッダと 2 箇所に書かれる形になったので `documentAdopted` へ畳んだ (先に見つけたのは
`DiscoveryOutsideTheFloor.test.js`)。`contextSetVars` の書き手は場の中だけだったので
文書の辺へ移し、あわせて `contextEnd` が `variables: []` を書いていた ADR-105 D4 型の
欠陥を消した — **静的には見えず、捕まえたのは e2e** である。
...
**設計判断は ADR-110** (Accepted・実装済み 2026-08-05)。要点: ADR-106 が器を動かした結果、`FLOOR_TARGETS` の
...
- [x] `FLOOR_TARGETS` を 3 行に (`negotiate` / `author` / `region-ghosts` — 全行が器を開く)。
- [x] `grasp` は**移設**。行き先を `RETIRED_FLOOR_TARGETS` (`RETIRED_FLOOR_TABS` と同じ形の
      「引っ越し先の台帳」) で名指しし、逆向き検査が「戻っていないか」も問う。
- [x] 選択が無いときの経路を N パネルの**常設**スロットへ — 理由つき disabled。
      **住所は `REQUIRES` ではなく `EntityScopeChecks`** になった (ADR-110 §実装で
      変わったこと — ヘッダから出た前提をヘッダの語彙に足すと空回りする宣言が 1 行できる)。
- [x] 理由は**2 種** — 未選択 / ロボット 0 台 (ADR-090)。基数を渡さない呼びは throw する。
- [x] ratchet の baseline は**変更しなかった**。

**完了条件: 達成。** 器を開かない `open-floor` の引数 **0 個** (呼び出し閉包から導出)、
入口の個数は desktop 7 / mobile 7 のまま `HeaderEntranceCensus.test.js` を**書き換えずに**
通った、`Context ▾` から `Grasp search…` が消えて N パネルに理由つき disabled で在り、
ロボットを選ぶと開く (e2e)。0 台の理由文だけは e2e から焼いていない — 削除フローが
主語になるため unit 側で焼いた (境界の宣言)。

## Phase 5.2 — 文書の入口の住所を恒久にする (ADR-112)

**設計判断は ADR-112** (Accepted・実装済み 2026-08-05)。要点: ADR-106 D3 の暫定住所は満期を **ADR-108 (Phase 5)**
...
- [x] `DocIntakeLayer` を恒久の住所として確定 (根拠は今日も有効 — 非モーダル / 非常設)。
- [x] 覆ってよい面の規則を**面の名前から役割へ**。所有者は 1 つ、未宣言の役割では throw。
      **粒度は実装で変わった** — 可否は「いま見せている面」ではなく「器が**見せうる**面」
      (側を切り替えるスイッチ自体が覆われる側に住んでいる — ADR-112 §実装で変わったこと)。
- [x] `PROVISIONAL_UNTIL` の宣言を**すべて**畳んだ (消し残しは `pnpm test:deferrals` が数える)。
- [x] `STATE_LEDGER` の `文書の入口 (docIntake)` 行から暫定性の欄を畳む。
- [x] 入口 (`START` の `GUIDED_INTAKE`) は**動かさなかった**。

**完了条件: 達成。** repo 内の `PROVISIONAL_UNTIL` 宣言 **0 個**、未宣言の役割で throw、
入力中に左のナビゲータが覆われない (e2e)。登録簿の DEF-001 は満期 (`満期=ADR-112`) が
来たので行ごと削除し、`UNDECLARED_BASELINE` を実測値へ下げた。
