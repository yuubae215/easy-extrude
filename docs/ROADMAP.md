...
## 2D/3D Spatial Link Semantics (ADR-043)

2Dマップオブジェクト（AnnotatedLine / AnnotatedRegion / AnnotatedPoint）と3Dソリッドを
`SpatialLink` で結び、空間的な意味論（安全距離・領域包含・物流インターフェース・公差ツリー）を
設計段階でリアルタイム検証するシステム。ケヴィン・リンチ分類の5要素（Route / Boundary / Zone / Hub / Anchor）を
3Dキネマティクスの一等市民として扱う。ADR-043参照。

### Phase 1 — bounded_by: AnnotatedLine × Solid クリアランス検証 ✅ (2026-05-21)

| 実装項目 | 詳細 | ADR |
|---------|------|-----|
| `bounded_by` セマンティクス | `SEMANTIC_TYPES` + `TOPOLOGICAL_SEMANTIC_TYPES` に追加 | ADR-043 |
| `SpatialLink.properties` フィールド | `clearance`（mm）などの型固有メタデータを格納；シリアライズ・ロード対応 | ADR-043 |
| `_minDistPolylineToPoints()` | XY投影 2D 最短距離計算（純JS）；polyline セグメント × solid corners の全組み合わせ O(M×N) | ADR-043 |
| `SceneService._evaluateClearanceLinks()` | 毎フレーム評価；`link.violated` + `link.errorMessage` 更新；`SpatialLinkView.setViolated()` 呼び出し | ADR-043 |
| `SpatialLinkView` アラート演出 | `setViolated(true)` → 赤パルス点滅（0xEF4444/0xFF9999 lerp + sin波）；dashSize縮小 | ADR-043 |
| リンク作成UI 3プリセット | AnnotatedLine/Region → Solid 時に「500mm / 1000mm / no gap」を picker に表示 | ADR-043 |

### Phase 2 — Zone 包含チェック（AnnotatedRegion × Solid）✅ (2026-05-22)

| 候補タスク | 説明 | 前提 |
|-----------|------|------|
| `_xyPointInPolygon()` ヘルパー | Ray-casting (odd-even) アルゴリズム；XY投影；`SceneService.js` モジュールスコープ関数として追加 | ADR-043 §Phase2 |
| `contains` セマンティクスの評価化 | `_evaluateClearanceLinks()` を拡張；Solid の全コーナーが Zone ポリゴン内か毎フレーム判定；複数 `contains` リンクが同一 Solid を指す場合は論理 OR で合成 | ADR-029, ADR-043 |
| Zone 侵犯ハイライト | `MeshView.setConstraintViolated(bool)` — `_syncEmissive()` で赤 emissive tint（0x550000）を付与；選択状態（0x112244）と合成；`SpatialLinkView` 赤パルス | ADR-043 |
| Phase 1 `bounded_by` との統合 | `_evaluateClearanceLinks()` 内で `semanticType` で分岐；`detachSpatialLink()` で `contains` リンク削除時に tint をクリア | ADR-043 |

### Phase 3 — Hub タクトタイム見積もり（AnnotatedPoint × 物流経路）✅ (2026-05-22)

| 候補タスク | 説明 | 前提 |
|-----------|------|------|
| `SpatialLink.properties.deadline` | Hub に要求タイミング（秒）を格納 | ADR-043 §Phase2 |
| `SpatialLink.properties.speed` | 搬送機器の移動速度（m/s）を格納 | — |
| タクト見積もり演算 | `（Hubまでの経路長 ÷ speed） > deadline` → `violated = true` | ADR-043 |
| Hub 表示 | タクト達成時 = 緑；未達 = 赤点滅（0.6 s 周期）；N パネルに見積もり秒数を表示 | ADR-029 (AnnotatedPoint) |
| Route 経路長 | `AnnotatedLine` の全頂点間距離の積算（polyline 全長） | ADR-029 |

### Phase 4 — Anchor 公差ツリー検証（AnnotatedPoint × CF チェーン）✅ (2026-05-22)

| 候補タスク | 説明 | 前提 |
|-----------|------|------|
| Anchor を公差ツリーの SSOT に設定 | `AnnotatedPoint (Anchor)` → `CoordinateFrame` の `references` リンクを起点に毎フレーム位置誤差を評価；`L` キーピッカーに ±1/5/10 mm プリセットを表示 | ADR-034, ADR-037 |
| 位置誤差評価 | Anchor 世界座標 ↔ CF 世界座標（`_worldPoseCache`）間の 3D 距離を mm 換算；`link.properties.currentDistance` に記録；`link.violated` 更新 | ADR-043 |
| 矛盾検出 | 複数の `references` リンクから同一 CF を異なる公差で拘束 → `anchorToleranceConflict` イベント → toast 表示 | ADR-043 |
| N パネル表示 | `references` リンク行に `${dist}mm/±${tol}mm` を緑/赤で表示 | — |
| Anchor 視覚フィードバック | `AnnotatedPointView.setToleranceViolated(bool)` 追加；violated = true → マーカー/クロスヘア赤変色、クロスヘアパルス周期 4 s → 1 s に短縮 | ADR-043 |

### Phase 5 — クリアランス評価の高精度化・Wasm 移行

| 候補タスク | 説明 | 前提 |
|-----------|------|------|
| 3D クリアランスモード | XY 投影ではなくワールド空間での最短距離（垂直方向安全確認、クレーン等） | ADR-027 |
| Wasm 移行 | `_evaluateClearanceLinks` の O(M×N) 計算を Wasm に移植；シーン規模 50 links 超で必要 | ADR-027 |
| カスタムクリアランス入力 | リンク作成 picker のテキストフィールドで任意 mm 値を入力 | ADR-043 §Phase2 |
| 空間インデックス | 大規模シーン向けに BVH/kd-tree で最近傍セグメント探索を高速化 | ADR-043 §Phase5 |
...
| ~~🔴 High~~ | ~~**fastened 拘束 CF チェーン伝播 (ADR-035)**~~ ✅ 2026-05-12 — `_findAncestorChain(sourceId)` で root Solid を特定し corners を `dq × (corner − pivot) + newCFWorldPos` で剛体変換。`bodyRotation.premultiply(dq)` で Solid 姿勢を更新し、中間 CF チェーンを ROS TF 順伝播でインライン再伝播。DFS による cycle 検出 + `constraintCycleDetected` イベントも実装済み。加えて `FrameRotateCommand` が `_updateWorldPoses()` を即時呼び出すよう修正し、N-panel CF 回転ハンドラに `_isFastenedRotationBlocked()` ガードを追加（source chain の混乱を全 UI エントリポイントで防止）。 | Medium | ADR-035; CODE_CONTRACTS "Fastened Constraint Limitations" |
| ~~🔴 High~~ | ~~**fastened 拘束の回転伝播**~~ ✅ 2026-05-06 — `_updateFastenedFrames()` の dq 計算が `prevQuat⁻¹ × worldQuat`（ローカルフレームデルタ）になっていたため混合軸回転で子 Solid の向きが崩れていた。`worldQuat × prevQuat⁻¹`（ワールドフレームデルタ）に修正。 | Medium | ADR-032 §5; CODE_CONTRACTS "Fastened Constraint Limitations" (1) |
...
| ~~🟡 Medium~~ | ~~**A-2: Live coordinates during Grab**~~ ✅ — `X:${cx} Y:${cy} Z:${cz}` in `_updateGrabStatus()` (lines 5175-5181); already implemented | Low | — |
