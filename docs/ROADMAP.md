...
### Phase 2 — Zone 包含チェック（AnnotatedRegion × Solid）

| 候補タスク | 説明 | 前提 |
|-----------|------|------|
| `_pointInPolygon()` ヘルパー | Ray-casting or Winding-number アルゴリズム；XY投影 | ADR-043 §Phase2 |
| `contains` セマンティクスの評価化 | 現在は annotation-only；Solid の全コーナーが Zone ポリゴン内に収まるか毎フレーム判定 | ADR-029, ADR-043 |
| Zone 侵犯ハイライト | 侵犯コーナーの Solid ハイライト表示（赤フェイス）+ `SpatialLinkView` 赤アラート | ADR-043 |
| Phase 1 `bounded_by` との統合 | `bounded_by`（Zone 境界の外側クリアランス）と `contains`（Zone 内部包含）は目的が逆；セマンティクスで分岐 | ADR-043 |
