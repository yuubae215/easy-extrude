...
|----------|------|-----------|-------------|
| 🟡 Medium | **Undo / Redo Phase 2** — `FaceExtrudeCommand` + `ExtrudeSketchCommand` | Medium | ADR-022 |
| 🟡 Medium | **Undo / Redo Phase 3** — `AddSolidCommand` + `DeleteCommand` (requires `SceneService.restoreObject`) | Medium | ADR-022 |
| 🟡 Medium | **Undo / Redo Phase 4** — `RenameCommand` + `FrameRotateCommand` + `FrameMoveCommand` | Low | ADR-022 |
