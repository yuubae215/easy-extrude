...
### Phase 4 — Anchor 公差ツリー検証（AnnotatedPoint × CF チェーン）

| 候補タスク | 説明 | 前提 |
|-----------|------|------|
| Anchor を公差ツリーの SSOT に設定 | `AnnotatedPoint` → `CoordinateFrame` の `references` リンクを起点に TF チェーンを構築 | ADR-034, ADR-037 |
| 公差積み重ね計算 | Anchor → Root CF → … → Leaf CF の各 `translation` の絶対誤差上限を累積 | ADR-018, ADR-035 |
| 矛盾検出 | 複数の `references` リンクから同一 CF を異なる公差で拘束 → `constraintCycleDetected` に準じたイベント | ADR-035 |
| N パネル表示 | 公差ツリーの累積誤差上限を mm 単位で表示；違反ノードを赤でハイライト | — |
