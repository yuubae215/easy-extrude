...
### Phase C-1 — Auto-Origin 廃止（コード移行）
...
| `L` キーフロー内の「New frame at…」オプション | 幾何学的リンク作成時にターゲットフレームを同時命名・作成 | ADR-033 §5, ADR-032 §8 |

### Phase C-3 — Mobile 作成 UI

| Task | Details | ADR |
|------|---------|-----|
| 長押しコンテキストメニューに「Add interface frame ⊞」 | Solid / Annotated\* を長押し → フレーム名入力 → 作成 | ADR-033 §5 |
| SpatialLink 作成フロー内の「New frame on this object」 | 「Link to…」タップ後に端点フレームを新規作成する分岐 | ADR-033 §5, ADR-032 §9 |
...
### Phase H-1 — ドメイン層（linkType 拡張 + インデックス）
...
### Phase H-3 — コマンド（Undo / Redo）

| Task | Details | ADR |
|------|---------|-----|
| `MountAnnotationCommand(linkId, sourceId, targetId, verticesBeforeMount)` | `execute()` = 頂点変換 + SpatialLink 作成；`undo()` = 頂点復元 + SpatialLink 削除 | ADR-032 §7 |
...
| Task | Details | ADR |
|------|---------|-----|
| `L` キーフロー拡張：幾何学的拘束ではCF のみハイライト | `GEOMETRIC_LINK_TYPES` 選択時は CoordinateFrame 以外を半透明化 | ADR-032 §8 |
...
| マウントピッキング中のステータスバー | 「Tap target frame (or empty space to cancel)」；✕ でキャンセル | ADR-032 §9 |
