...
| 残しの**宣言** (deferral) | `宣言外` / `宣言済み・満期前` / `宣言済み・満期切れ` / `実在しない宣言` の **4**。うち 2 つは今日どこにも表現されていない — だから満期切れも空宣言も緑を出す | **`0..N`**。**正当な非ゼロを宣言させる**予算として ratchet の baseline を焼く (ADR-100 と同形 — 「宣言外は 0 であるべきだが今は N 件ある」を隠さない)。母集団は登録簿ではなく**語彙から導出**する (登録簿を分母にしたら `place-list`) | `docs/DEFERRAL_LEDGER.md` (予定) が宣言の正本、個数の問い所は `scripts/check-deferrals.mjs` の 4 つの問い | **ADR-109** (Proposed) / Phase 6 |
