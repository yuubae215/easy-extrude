...
| 文書の入口 (docIntake) | `wizard` / `intake` + 不在 (3) — **暫定住所**であることを `PROVISIONAL_UNTIL` が宣言する (宣言しない暫定は恒久になる) | **0..1** — 0 = 入口を開いていない = 既定。**開くのに場は要らないが文書は要る** (どちらのパネルも文書へ書くため)。権威は `ContextService.loaded` (ADR-106 D4) | `uiStore.docIntake`、書き手は `setDocIntake` ただ 1 つ。値域は `src/view/DocIntake.js` の宣言表で、未宣言の面では **throw する** (原則 #31) | **ADR-106** D3 · `src/components/Doc/DocIntakeLayer.jsx` · 最終的な住所は **ADR-112** (Phase 5.2) が決める — 満期を ADR-108 から張り替えた (ADR-108 は入口を決めて器の住所を決めなかった。ADR-109 §力学 2) |
...
|---|---|---|---|---|
| Outliner の**側** (`outliner.side`) | `geometry` / `semantic` の **2**。表示の側であって実体の状態ではない — 不正遷移が事故にならず導出でもないので、核 §1.4 の閾値を跨がず状態機械は起こさない (**閾値未満でも記録する**のがこの行の目的) | **`1`** — 側を持たない Outliner は無い。**0 が現れるのは側の*中身*のほう**で、意味側の 0 は最低 2 種として宣言する (文書が無い / 文書に変数が 0 個)。1 つの「空」で両方を賄うと ADR-105 が潰した「未検証と問題なしの混同」をナビゲータ側で再生産する | `uiStore.outliner.side` (予定)、書き手は 1 つ。側ごとの住人は宣言表が持ち、**未宣言の側で throw** する | **ADR-111** (Proposed) / Phase 3.6 |
| 把持探索の**入口の可用性** | `enabled` / `disabled(未選択)` / `disabled(ロボット 0 台)` の **3** — 理由が状態を割る。1 つの理由文で 2 つの 0 を賄うと、0 台の人に対してだけ静かに嘘になる | **選択が `0..1`・ロボットが `0..N`** の積。**2 つの 0 は別物**: 「選べるロボットが在るのに選んでいない」と「選べるロボットが無い」は、ユーザーが次に取る行動が違う (ADR-090 の 0 台問題の再演) | `availabilityOf()` が**判定と理由を同じ返り値**から返す (原則 #25 / #11)。前提は `REQUIRES` + `REQUIREMENT_REASON` + dispatch の 3 つ揃いで、欠けると throw する | **ADR-110** (Proposed) / Phase 5.1 |
