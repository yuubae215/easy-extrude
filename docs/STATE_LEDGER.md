...
| grasp-search | `idle` / `no-layout` / **`no-robot`** / **`no-target`** / `compiling` / `solving` / `results` / `error` (8) | 1 | `GraspController` — `uiStore.context.grasp` を丸ごと置換 (判別 union) | ADR-057 · ADR-090 (`no-robot`) · **ADR-117** (`no-target`) · `src/store/uiStore.js` |
| 掴む対象 (grasp target) の台数 | 基数そのものが状態: `none` / `single` / `multi` (3 — `TARGET_CARDINALITY`)。ロボット台帳行の**双子**で、前提のもう半分 | **`0..N`**。0 = 正当 (掴めるものが無いセルは在りうる) で `no-target` で停止し理由を出す。N は **明示選択**が必要 — 既定で先頭を選ばない (この starter の先頭 Solid はロボット自身の台座なので、既定は「自分の台座を掴む」を誰も選ばずに宣言することになる)。**ADR-117 以前この基数には欄が無かった**: 対象は request に載っておらず、`core/` が空サンプルから返す `candidatesGenerated: 0` が「正しい形の答え」として通っていた (原則 #31) | **Layout DSL** (`ContextService.getCompiled().layoutDsl` の `Solid`) — 解決は `domain/graspTargets.js: resolveGraspTargets()` の 1 箇所。選択の所有者は `GraspController._selectedTargetRef`、投影は `context.graspTargets` (読み取り専用・書き戻し無し) | **ADR-117** · `src/domain/graspTargets.test.js` · `src/controller/GraspController.test.js` |
| grasp スタブのシナリオ | `solve` / `allRejected` / `thin` / `empty` / `error503` / `error502` / `error400` (7 — `STUB_SCENARIO`)。アプリの状態ではなく**ビルドレーンの**状態。実ソルバが滅多に到達しない画面へ決定的に行くための軸 | **`0..1`** — 0 = 通常ビルド (レーンごと存在せず、`dist/` から tree-shake されていることを実測で焼く)。1 = `VITE_GRASP_STUB=1`。**未宣言の名前は既定へ倒れず画面上で拒否する** — 打ち間違いで「見ていない画面」の所見を書かせないため (原則 #31)。boot 中に throw するだけでは devtools を開いた人にしか届かないので、拒否はパネルに出る 400 として運ぶ (原則 #11) | `mocks/graspStub/scenarios.js` の宣言表 (`scenarioOrThrow`)。`src/` は値域を知らない | **ADR-117** · `mocks/graspStub/conformance.test.js` · `e2e/grasp-stub.spec.js` |
...
| 誘導入力ウィザード | `step` / `review` (2) + 不在 | 0..1 | `ContextController` — `uiStore.context.wizard` を丸ごと置換。**ADR-106 D3 で場から独立した**: 寿命は `startWizard` / `exitWizard` / `finishWizard` の対だけが持ち、場のライフサイクルリデューサ (`contextStart` / `contextEnd`) は触らない (器が寿命を持つと、器を閉じただけで入力が消える) | ADR-063 Phase 3 / ADR-106 D3 / STATE_TRANSITIONS §context.wizard |
| 文書の入口 (docIntake) | `wizard` / `intake` + 不在 (3)。住所は **ADR-112 で恒久**になり、暫定性の欄 (`PROVISIONAL_UNTIL`) は更新ではなく**削除**された — 決着した暫定の宣言を残すと「まだ残っている」という嘘を出し続ける | **0..1** — 0 = 入口を開いていない = 既定。**開くのに場は要らないが文書は要る** (どちらのパネルも文書へ書くため)。権威は `ContextService.loaded` (ADR-106 D4) | `uiStore.docIntake`、書き手は `setDocIntake` ただ 1 つ。値域は `src/view/DocIntake.js` の宣言表で、未宣言の面では **throw する** (原則 #31) | **ADR-106** D3 / **ADR-112** · `src/components/Doc/DocIntakeLayer.jsx` · `src/DocIntakeAddressCensus.test.js` |
| Outliner の**側** (`outlinerSide`) | `geometry` / `semantic` の **2**。表示の側であって実体の状態ではない — 不正遷移が事故にならず導出でもないので、核 §1.4 の閾値を跨がず状態機械は起こさない (**閾値未満でも記録する**のがこの行の目的) | **`1`** — 側を持たない Outliner は無い。**0 が現れるのは側の*中身*のほう**で、意味側の 0 は **2 種**として宣言済み (文書が無い / 文書に変数が 0 個 — `SEMANTIC_SIDE_KIND`)。1 つの「空」で両方を賄うと ADR-105 が潰した「未検証と問題なしの混同」をナビゲータ側で再生産する | `uiStore.outlinerSide`、書き手は `outlinerSetSide` ただ 1 つ。値域は書き込みで守られ、**未宣言の側で throw** する (`navigatorSideOrThrow`) | **ADR-111** · `src/view/NavigatorSides.js` · `src/NavigatorAddressCensus.test.js` |
| 把持探索の**入口の可用性** | `enabled` / `disabled(未選択)` / `disabled(ロボット 0 台)` の **3** — 理由が状態を割る。1 つの理由文で 2 つの 0 を賄うと、0 台の人に対してだけ静かに嘘になる | **選択が `0..1`・ロボットが `0..N`** の積。**2 つの 0 は別物**: 「選べるロボットが在るのに選んでいない」と「選べるロボットが無い」は、ユーザーが次に取る行動が違う (ADR-090 の 0 台問題の再演)。基数を渡さない呼びは **throw** する — 既定へ倒すと 0 台の人にだけ嘘が出る | `graspEntryFor()` が**判定と理由を同じ返り値**から返す (原則 #25 / #11)。0 の種は `GRASP_BLOCKED_KIND` + `GRASP_BLOCKED_REASON` の対で、未宣言の種は throw する | **ADR-110** · `src/view/EntityScopeChecks.js` · `src/FloorContainerCensus.test.js` |
| パラメトリック資産ビューア | `assetId` + `values` + 不在 (2) | **0..1** — 0 = 何も整形していない = 既定。**ADR-106 D3 で場から独立した**: 入口は `+ 追加` の Assets グループ、スライダーは N パネル。整形は文書を要求せず、**commit だけが**要求する (guard は書く場所へ移した) | `uiStore.context.assetViewer`、書き手は `contextSetAssetViewer`。確保と解放は `openAssetViewer` / `closeAssetViewer` の**対ちょうど 1 組** (原則 #9 — 器が寿命を持っていたころは `_disposeAssetPreview()` の余分な呼び出しが 2 箇所あった) | ADR-063 Phase 4 / **ADR-106** D3 · `src/components/Context/ParametricAssetPanel.jsx` |
| オンボーディングツアー | `active` / `done` + `null` (3) | 0..1 | `AppController` (純粋遷移は `TourMath`) | ADR-065 Phase 6 / STATE_TRANSITIONS §tour |
| **画面の主張 (stage)** | 段 `stage` に立つ面 — `launch-home` (工程レイアウト) / `context-template-gallery` (New Project) / 不在 (3)。旧 `home` (`open`+`null`) と旧 `templateGalleryOpen` (boolean) を**1 欄に統合**したもの | **`0..1` — 欄が 1 つなので 2 枚同時が表現不能**。0 = 何も画面を占めていない = エディタが見えている状態 (既定)。統合前は 2 つの独立 flag だったので**基数の欄が無く**、両方立ったときの勝者は `UIShell.jsx` のマウント順 (どちらのコンポーネントにも書かれていない事実) が決めていた — 「0 個・N 個は状態に見えない」の典型 (原則 #31)。負けた面は無言で見えなくなる (原則 #11) | `uiStore.stage`。書く入口は `claimStage(claim)` / `releaseStage(claim)` の**対ちょうど 2 つ** (原則 #1 / #9)。解放は**どの主張が手放すか**を名乗るので、遅れて来た close が後から立った別の主張を消せない。段と z-index の宣言は `src/view/ScreenClaim.js` ただ 1 箇所で、面は自分で z を選べない (原則 #26 の全画面版) | **ADR-113** · ADR-089 / ADR-051 · `src/ScreenClaimCensus.test.js` · `docs/gsn/adr-113-one-claim-on-the-screen.gsn` |
| 画面を占める面 (段) | `stage` / `coach` / `dialog` の 3 段。段は**面が自分で選ばない** — `tierZIndex()` が唯一の順序の所有者で、未宣言の段では throw する | **段ごとに `0..1`**、面の総数は `0..4`。`coach` (初回ジェスチャヒント) を `stage` と同居させないのは、4 秒で自動退場する面が stage を奪うと mobile 起動でホームが戻らなくなるため。`dialog` が別段なのは、下の面**について**問う面が下を消してはならないため。**同段の 2 枚同時は構造で不可能** (stage は 1 欄・dialog は `uiStore.modal` の判別 union) であって検査で防いではいない — 3 枚目を同じ段に足す変更はこの不可能性を壊しうる | `src/view/ScreenClaim.js` の `CLAIM_DECLARATION` / `TIER_Z`。母集団は走査で導出する (`position:'fixed'` + `inset:0`) ので、全画面を描いた日からそのファイルは数えられる | **ADR-113** · `src/ScreenClaimCensus.test.js` |
| モバイル `⋯` メニューの段 | `null` (閉) / `{level:'verbs'}` / `{level:'objects', verb}` の 3 — **閉が段を持てない**形。旧実装は `open` (boolean) と `verb` の 2 欄で、`{closed, verb:'export'}` が表現可能だった (そして実際に画面に出た) | **`0..1`** — 0 = 閉じている (既定)。**段は開くときに決まる** (`openMenu()` は必ず一段目) ので、閉じ方が何通り増えても「閉じるときに戻す」を思い出す必要がない | `src/view/OverflowMenuState.js` の遷移関数のみ (原則 #1)。`MoreMenu` は部分更新を書かない | **ADR-113** · ADR-108 · `src/view/OverflowMenuState.test.js` |
| **上端 (mobile ヘッダ) の幅占有** | 住人ごとに `収まる` / `切り落とされる` (2)。`overflow:hidden` が後者を**無言で**起こす (原則 #11) | **住人は `1..N` (今 8)、ただし数えるのは *切り落とされた個数* で正当値は `0`**。実測 438px 要求 / 320〜393px 供給で、320px では 2 個・393px では 1 個が画面外に在った。上端は長らく**高さ (40px) だけ**が資源として扱われており、幅には所有者が居なかった — 「入らなかった個数」は欄を持たないので、入口を 7 個数える census は緑を出し続ける (原則 #31)。**同じ症状は 2 度目**で、1 度目は特定の住人 (Map ボタン) への 1 箇所のパッチで直され、ADR-103 の退役でパッチごと消えていた (原則 #26 — パッチは自分が守っていた不変条件を持ち去る) | `src/view/EdgeOccupancy.js` の `TOP_EDGE_OCCUPANTS` / `HEADER_METRICS` — **描画側 (`Header.jsx` / `HeaderMenus.jsx`) も同じ表を読む**ので予算は現実の写しになる (§1.1)。未宣言の住人では `topEdgeWidth()` が throw する | **ADR-114** · `src/view/EdgeOccupancy.test.js` (宣言側) · `src/HeaderEntranceCensus.test.js` (JSX 由来の母集団) · `e2e/mobile-reach.spec.js` (320px で押せる箱の矩形を実測) |
| **タッチのカメラ操作割当** | 自由度ごとに `割当あり` / `割当なし` (2)。自由度は `orbit` / `dolly` / `pan` の 3 で閉じる (`camera.up` 固定なのでロール軸は無い) | **自由度は常に 3、割当は各自由度に `1`。出荷から ADR-114 まで `pan → 0` だった** — `touches` の直値は割り当てた 2 つを宣言するが、**割り当てなかったものは行を持たない**ので、コードを読んでも「pan が無い」は見えない。母集団を割当表ではなく**自由度の側**に置いて初めて数えられる (原則 #31) | `src/view/CameraGestures.js` の `TOUCH_DOF_ASSIGNMENT` ただ 1 つ。`SceneView` の `controls.touches` は `orbitControlsTouches()` による**翻訳**であって第二の宣言ではない。未宣言の自由度では throw | **ADR-114** · `src/view/CameraGestures.test.js` · `e2e/mobile-reach.spec.js` (走っているアプリの `touches.TWO`) |
| **double-tap の判定** | `受理` / `却下` (2)。却下は必ず理由を持つ (無言で捨てる枝は 0 個 — 原則 #11) | **判定条件の個数が ADR-114 まで `0`** — ブラウザの `dblclick` をそのまま信じていた。0 個の条件は if 文を持たないので、ハンドラを読んでも「判定していない」は見えない。とくに空振り (当たりも選択も無い) が `_focusSphere()` の**シーン全体フォールバック**へ落ちており、意図の最も薄いジェスチャが最大のカメラ跳躍を起こしていた | 純粋述語 `src/view/TapGesture.js: acceptDoubleTap()` ただ 1 つ (原則 #25 — ハンドラ内のインライン early-return に散らさない)。入力の記録は `AppController._recordTap()` の 2 スロット | **ADR-114** · `src/view/TapGesture.test.js`。**限界の宣言**: 述語の正しさしか示さない — ブラウザがいつ `dblclick` を合成するかは Playwright の合成タッチで再現できず (実測)、実機確認 1 回が残る |
| **drawer の backdrop** | `在` / `不在` (2) | **`0..1`** — 0 が正当なのは両 drawer が閉のとき、および desktop (drawer が存在しない)。ADR-114 以前は `nPanelVisible` と 2 欄で書き手も 2 箇所あり、`N` キー経路と outliner の「もう片方を閉じる」行は開閉だけを書いていた → **backdrop 無しで開いた drawer** が作れ、画面外のトグルでしか閉じられなかった (上端の幅の欠陥と噛み合って「しまえない」を二重にしていた) | `AppController._syncDrawerBackdrop()` — **drawer から導出**する唯一の書き手 (原則 #4)。開閉の唯一の入口は `_toggleNPanel()` (原則 #1) | **ADR-114** · `e2e/mobile-reach.spec.js` (開く→**閉じる**、および暗転を叩いて閉じる) |
...
| コミットの観測メタデータ | `未刻印` / `刻印済` / `刻印不能` (3 — 最後は push 済みで amend 不可になった終端) に加え、**`空洞` を 4 つ目として分ける** (トレーラは在るが `unknown/unknown` = transcript が読めなかった)。3 状態のときは「在るか無いか」で足りたが、*在るのに何も言っていない*状態は被覆率の分子に入ってしまうので刻印済と同一視できない | コミット 1 個につき **`0..1`**。0 は 3 種類ある: (a) 人間のコミット (帰属として正しい — **分母にも入れない**)、(b) 書き手が実在する前のコミット (境界の外)、(c) `commit && push` 連鎖で刻む隙間が無かったもの = **欠測**。**(c) だけが数える対象で、実測 14 件 / 母集団 56 件 (被覆 75%)**。ADR-092 はこれを「`report` の `with Model-Effort: N / 総数` で母数に現れる」としたが、**その数を読む機械はどこにも居なかった** — 印字と検査は別の事実で、14 件はそのあいだに無言で積み上がった (ADR-115 力学 1)。今は `MISSING_BASELINE = 14` の ratchet が超過も下回りも落とす。**母集団の境界は導出する** (`.claude/hooks/commit-trailers.sh` を追加したコミット) — 日付で近似すると同日の hook 導入前 3 件を欠測に数えて 71% と誤る (実際に踏んだ)。**母集団が空であること自体が 0 件の欠測に化ける**ので、境界を決定できなければ検査が落ちる (shallow clone — 原則 #31 を道具の側に当てた欄) | **主経路 = `.githooks/prepare-commit-msg`** (コミット生成の**前**に刻むので連鎖が無関係、SHA も動かない — ADR-116)、**予備経路 = `.claude/hooks/commit-trailers.sh`** の PostToolUse (`--amend --only`。`core.hooksPath` が張られていない clone 用・冪等なので二重刻印しない)。**どちらも導出はしない** — 規則の正本は `scripts/commit-meta.mjs` の純粋関数 1 箇所。**帰属の判別はマーカー (`.git/claude-commit-context`) の鮮度ただ 1 つ**で、書式の正本は `buildCommitContext`/`parseCommitContext` の対 (書き手 = PreToolUse、読み手 = git hook が別々に `key=value` を実装すると第二の源になる)。**数え方の正本は `scripts/check-commit-trailers.mjs`** (`classifyCommit` が `missing` と `foreign` を別の値で返し、両者を混ぜない) | **ADR-116** · **ADR-115** · ADR-092 (§3 の機構は主経路から降りた) · STATE_TRANSITIONS §Commit observation metadata · `scripts/check-commit-trailers.test.mjs` / `scripts/commit-meta.test.mjs` |
...
| 発見の集約 (discovery summary) | **未検証** / **検証済み** の 2 (kind 判別の union — `DISCOVERY_KIND`)。「検証済み」の内側に 3 カウンタが入る。**`null` / `0` / 不在で代用しない** — それらは推論させる形で、この行が消そうとしているものそのもの。検査側は 4 種 (`CHECKS_KIND`: 未検証 / 検査対象なし / 全部パス / 不合格あり) | **文書 `0..1` × 検査 `0..N` の直積で 0 が 3 種類に割れる**: ①文書なし = **未検証** (正当かつ頻出 — `_doc` の初期値は `null` で、起動直後・ホーム・テンプレ選択前はすべてここ)、②文書あり検査 0 件 = **検査対象なし**、③検査ありで全部 pass = **全部パス**。3 つは**次の一手が全部違う**ので、型で別の値にした。「✓ 全部パス」を名乗れるのは③だけ (①②で出すのは嘘、消すのは原則 #15 違反)。未宣言の種は宣言表が **throw** する | **導出値 — 保存しない。** 権威は `ContextValidator` の結果 + 文書で、`agendaCounters()` / `projectChecks()` が読み時に合成し、`ContextService.discoverySummary()` / `.checksSummary()` が union へ畳む。**書き手は 1 つ** = `uiStore.contextSetDiscovery`。ADR-105 以前は 2 つで、退出リデューサが `{0,0,0}` へ戻していた (UI ライフサイクルは導出されたドメインの事実の書き手ではない — 原則 #4 / #24)。配線は場ではなく**文書**の辺 (`contextLoaded` / `contextChanged`) に繋ぐ。描き手は `ctx.active` を読まない (個数で検査) | **ADR-105** (実装済み) D2 / D3 / D4 · `docs/gsn/adr-105-unexamined-is-not-clear.gsn` · `src/context/DiscoverySummary.js` · `src/DiscoveryOutsideTheFloor.test.js` |
| entity-scope 検証の可用性 | 実体種ごとに**宣言**された可用性 (`ENTITY_SCOPE_BY_KIND`: `generic` / `frame` / `link` の 3)。状態ではなく**選択からの導出**だが、種の集合が動く欄なのでここに記録する。未宣言の種で **throw** する | **選択が `0..1`** — 0 = 何も選んでいない (正当・頻出。入口は消えず「Nothing is selected.」と理由を出す)。種の側は `0..N` で、母集団は `NPanel.jsx` の `nPanelData?.type` 分岐から**導出**する (手で並べない — ADR-102)。ロボットの識別は**宣言された役割** (`robotRole`) で行い、名前では行わない (名前はロボット間で一意でない — ADR-090 D1) | 導出のみ。純粋 `src/view/EntityScopeChecks.js` の `graspEntryFor()` が判定と**理由**を同じ返り値で返す (原則 #11)。`ctx.active` も文書の有無も読まない — 軸は選択だけ (個数で検査) | **ADR-105** D5 · ADR-085 (無フォームの grasp 入口) · `src/DiscoveryOutsideTheFloor.test.js` |
...
| 場の器 (下部の展開パネル) | `閉` / `開` (2) — **`mode` (negotiate / author / ghost) と直交**。器の開閉は「合意しに来たか」であって、どの投影を見ているか・何を選んでいるかとは別軸。閾値未満だが記録する (核 §1.4 — 記録しなければ累積しない) | **`0..1`** — 0 = 場が閉じている = **正当かつ既定**。ADR-106 以降は「文書は在るが場は閉じている」が通常状態である (場は必要なときだけ開き、常設しない)。**文書の有無は別軸**で、権威は `ContextService` ただ 1 つ。`uiStore.context.loaded` は**書き手 1 / 読み手 0 の写し**だったので**退役済み** — 書かれるだけで読まれない第二の源は、使われた瞬間にドリフトする (§1.1)。消せるうちに消した | `uiStore.context.active` — 書き手は `contextStart` / `contextEnd` の 2 verb。**器は他パネルの可視性を書かない** (実測 0 個): 排他 7 箇所 (`setNPanelVisible(false)` × 3 / `setForceHidden` × 4) は住所の衝突を回避するために書かれたもので方針ではなく、住所を移した結果**書く理由を失って消えた**。下端も共有資源なので占有計算は `src/view/EdgeOccupancy.js` の `bottomEdgeOffset()` ただ 1 箇所が持ち、段 (`BOTTOM_TIER`) は未宣言で throw する (原則 #26 / #31 — 右端の `_updateGizmoOffset()` と同じ役割) | **ADR-106** (実装済み) D1 / D2 / D4 / D6 · `docs/gsn/adr-106-the-floor-is-a-table-not-a-strip.gsn` · `src/components/Context/ContextLayer.jsx` · `src/view/EdgeOccupancy.js` · `src/FloorContainerCensus.test.js` |
| 場のタブ (`inspectorTab`) | 器に入っている**責務の種類**が状態。ADR-106 以降は「解消」(Matrix / Cluster / Floor / Questions) と「記録」(Why / Overview) の 2 種のみ — 「発見」と「入力」は器を出る | **`1`** — 常にちょうど 1 枚が選ばれる (0 枚は無い)。効くのは選択の基数ではなく**値域の基数**で、最大 11 → **7** (描かれるタブ 6 + author モードの `conflicts`) へ縮んだ。退役した値 (`checks` / `grasp` / `assets` / `wizard` / `intake`) を enum に残さない — **退役の腐敗は違反を*見逃す*のではなく緑を出す** (ADR-103 の `DS_PENDING` が廃止後 3 リリース enum に残った件と同型)。**移設先を名指ししないまま消す値は 0 個** (原則 #11 / #16 — 移設の証拠は「消えたこと」ではなく「着いたこと」) | `uiStore.context.inspectorTab`、書き手は `contextSetTab` ただ 1 つ。**値域の正本は `src/view/FloorTabs.js`** で、`contextSetTab` は未宣言値で **throw する** — JSDoc の union は散文であって数えられず、3 リリース残った退役値も「合法」に読める (数えられない形は数える前に直す)。退役値は行き先を持った表 (`RETIRED_FLOOR_TABS`) として残り、行き先の実在を逆向きに問う | **ADR-106** (実装済み) D3 / D5 · ADR-105 D3 / D5 (出ていく 2 タブの行き先) · `src/view/FloorTabs.js` · `src/FloorContainerCensus.test.js` |
| 選択の**要素の種** | `実体 (3D に姿を持つ)` / `文書の共有変数` の 2。**基数ではなく種**なので、台帳の基数列にも `mode` 欄にも現れない — `Set<id>` は種を持たないため実装を読んでも見えない (原則 #31 の一段上の同型)。選択そのものは `empty` / `entities(N≥1)` / `variables(N≥1)` の **kind 判別 3 枝**で表し、**混在は表現不能** | **`0..N` (種ごと)**。**0 個の表現は 1 通りだけ** (`empty` 枝) — 「空の entities」と「空の variables」が別物として書ける形は、同じ事実に 2 つの表現を与える (§1.1)。**混在の基数は常に 0** で、これは guard ではなく型で保証する: 混在を許すと N パネルが「どちらの詳細を出すか」を決めねばならず、その決定に自然な答えが無い (= 決定の不在を既定値で埋める形)。**選択可能な種は 3D の姿を宣言する** — 変数の姿は未確定帯 (ADR-050 の系譜)、未宣言の種は表が throw する (無言の禁止を種のレベルで保証 — 原則 #11)。**実装で姿の帰結が 3 通りに割れた**: 領域 Variable = 未確定帯 / スカラー = 束縛実体の DIMMED / **どちらも無い変数 = 0** — 最後の 1 つは正当だが、既定値で埋めず N パネルが「まだ空間に姿を持たない」と**述べる** (3D が 0 でも画面は無言にならない) | `SelectionManager._sel` — 入口は ADR-099 の 5 verb のまま (**種が増えても入口は増えない**)。値の構築点は `makeSelection` **ただ 1 つ**で、union のリテラルを外で組み立てる経路は 0 個 (検査あり)。Outliner の意味側・場の行列の変数見出し・未確定帯のクリックは**窓**であって入口ではない。実体 id は素の文字列、文書 `ref` は `variableRef(ref)` トークンで**形が違う** (命名規約に頼らない — 原則 #21 の同型)。種ごとの 3D の姿と N パネルの body は `src/view/SelectionKinds.js` の 2 表が持ち、未宣言の種で throw する | **ADR-107** (Accepted・実装済み) D1 / D3 / D4 / D5 / D6 · ADR-099 (入口の census — **作り直さずに通った**) · ADR-096 (`contextual` 軸) · `docs/gsn/adr-107-selection-has-two-kinds.gsn` · `src/domain/selection.js` · `src/SelectionKindDeclarations.test.js` · `src/SelectionOwnership.test.js` |
| **ヘッダの常設入口** | **状態ではなく母集団** — 「動詞の入口か否か」の 2。`mode` でも `status` でもないので実装を読んでも状態に見えないが、**個数が動く欄**なので台帳に載せる (原則 #31)。入口が生えるのは 1 つずつで、1 つ足すのは常に安い — 累積を数える所が無ければ有界でない成長が誰にも見えない (ADR-060 の契約版と同型) | **desktop 7 / mobile 7 ちょうど**。**0 は無い** (ヘッダは常設)。**接続状態で動かない** — ADR-108 実装で条件つき表示の入口を 0 個にしたので、BFF の有無で基数が変わらなくなった (消える入口は「入口の個数」を接続状態の関数にする)。上界は ratchet が **超えても下回っても** fail させる (ADR-100 と同形) | `src/components/Header/Header.jsx` の 2 つのレイアウト関数。分類の権威は `src/view/HeaderEntrances.js` の `HEADER_ENTRANCES` **ただ 1 つ**で、母集団は JSX から構文で導出する (手書きの入口リストを持たない — ADR-102)。動詞ごとの入口が 2 個以上になるのは `MULTI_ENTRANCE_VERBS` に理由つきで宣言したものだけ | **ADR-108** D1 / D2 / D4 · ADR-100 (ratchet) · ADR-102 (母集団の導出) · `docs/gsn/adr-108-entrances-are-verbs-not-objects.gsn` · `src/HeaderEntranceCensus.test.js` |
| 「始める」の**種** | `layout-template` / `context-template` / `story` / `quests` / `guided-intake` の **5**。遷移を持たない分類なので状態機械の節は起こさない — 効くのは**未宣言の種を表現不能にする**方 | 種は **`0..N`** で今日 5 個、起動中の物語は `0..1`。**「入口が 0 個」だった種が実際に 1 つあった**: Tour は初回に種が撒かれるだけで `✕` が永続化すると二度と戻らず、*引数として列挙*させるまでその 0 は誰にも見えなかった (ADR-108 §実装で変わったこと 1)。前提を満たさない種は**消さず**理由つき disabled — 消すと「5 種ある」が文書採択状態の関数になる | `START_ENTRY_BY_KIND` (`src/view/HeaderEntrances.js`) ただ 1 つ。`startEntryFor()` が**未宣言の種で throw** する (`EXPLICIT_DEFAULTS` / `PLACEMENT_BY_KIND` / `SELECTION_SHAPE_BY_KIND` と同じ既定表の規律)。母集団は `START_KIND` enum から導出し、枝を足して行を足さなければ検査が落ちる | **ADR-108** D3 · ADR-089 / ADR-051 / ADR-047 / ADR-065 / ADR-063 (各種の出自) · `src/HeaderEntranceCensus.test.js` · `e2e/smoke.spec.js` |
| ファイル動詞 × **対象** | 動詞 `export` / `import` の 2 × 対象 `scene` / `server` / `context` の 3。**直積は入口ではなく引数の値域**に置く — かつては 6 つの平坦なボタンとして*入口の個数*になっていた | 動詞ごとに入口 **ちょうど 1**、対象は **`1..N`** (今日 3)。**サーバ対象は BFF 未接続でも 0 個にならない** — 理由つき disabled で在り続ける (原則 #15 / #11)。対象を足しても入口は増えないので、成長は引数の値域に吸収される | `FILE_TARGETS_BY_VERB` (`src/view/HeaderEntrances.js`) ただ 1 つ。`fileTargetsFor()` は未宣言の動詞で throw。可用性は `availabilityOf()` が**判定と理由を同じ返り値**から出す (呼び手が理由を別経路で組み立てられない — 原則 #25) | **ADR-108** D1 · ADR-050 (`.ctx.json` が正準 — 却下案 C が壊そうとした区別) · `src/HeaderEntranceCensus.test.js` |
| 残しの**宣言** (deferral) | `宣言外` / `宣言済み・満期前 (機械可読)` / `宣言済み・満期前 (散文のみ)` / `宣言済み・満期切れ` / `実在しない宣言` の **5**。初版は 4 で数えていた — **満期の可読性という軸が無かった**ので、「満期を書いた」と「満期が読まれる」が同じ顔をしていた (ADR-109 D6 が割った) | **`0..N`**。**正当な非ゼロを 2 つ宣言させる**予算: 宣言外 **31 箇所** と、機械が読めない満期 **6 件**。どちらも超過・下回りの両方で fail する (ADR-100 と同形)。母集団は登録簿ではなく**語彙から導出**する (登録簿を分母にしたら `place-list`) | `docs/DEFERRAL_LEDGER.md` が宣言の正本 (`満期=ADR-NNN` が機械可読 trigger の唯一の形)、個数の問い所は `scripts/check-deferrals.mjs` の 5 つの問い — **CI gate で毎 PR 走る** | **ADR-109** · `docs/gsn/adr-109-a-deferral-is-a-declaration-not-a-memory.gsn` · `scripts/check-deferrals.mjs` |

---

## 提案中の実体 (段を持つが未実装 — ADR が Proposed の間だけここに居る)

**なぜ別表なのか:** 更新規則は「コードと同じコミットで」なので、まだコードの無い実体を
上の台帳に混ぜると「実装済みの状態」と「提案された状態」が同じ顔で並ぶ (原則 #31 —
未着手の枝と完了した枝が散文では区別できない、ADR-094 が名指しした形)。**しかし記録
しなければ累積せず、核 §1.4 の閾値判定がプロンプト単位に退化する。** よって分けて載せ、
ADR が Accepted・実装済みになった時点で行を上の台帳へ**移す** (複製しない — §1.1)。

`docs/ia-redesign/03-implementation-order.md` が「状態と基数 → `STATE_LEDGER` §提案中の
実体」と名指ししていた節がこれである。**その名指しは 2026-08-04 まで宛先を持っていなかった**
(参照先の無いポインタは、ADR-109 が数える残しの 5 例目として記録)。

| 実体 | 状態集合 (個数) | **基数** | 権威 (単一の書き手) | 段 / ADR |
|---|---|---|---|---|

**現在 0 行** (2026-08-05)。**0 であることを宣言する** — 空の表は「提案が無い」とも
「表を更新し忘れた」とも読めるので、推論させない (原則 #31)。ここに居た 2 行
(`outliner.side` / 把持探索の入口の可用性) は ADR-111 / ADR-110 の実装とともに
上の台帳へ**移した** (複製していない — §1.1)。
